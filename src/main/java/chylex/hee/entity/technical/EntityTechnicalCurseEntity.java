...
public class EntityTechnicalCurseEntity extends EntityTechnicalBase implements ICurseCaller {
    ...
    public EntityTechnicalCurseEntity(World world, EntityLivingBase target, CurseType type, boolean eternal) {
        ...
        this.usesLeft = (byte) (target instanceof EntityPlayer
                ? ((eternal ? 2 : 1) * type.getUses(EnumCurseUse.PLAYER, rand))
                : (eternal ? -1 : type.getUses(EnumCurseUse.ENTITY, rand)));
    ...
    public void onUpdate() {
        if (worldObj.isRemote) {
            ...
            if (curseType != null) {
                ...
                if (rand.nextInt(dist > 16D ? 3 : 2) == 0) HardcoreEnderExpansion.fx.curse(
                        worldObj,
                        posX + (rand.nextDouble() - 0.5D) * targetWidth * 1.5D,
                        posY + rand.nextDouble() * targetHeight,
                        posZ + (rand.nextDouble() - 0.5D) * targetWidth * 1.5D,
                        curseType);
            }
        ...
        if (target != null) {
            ...
            if (curseType.handler.tickEntity(target, this) && usesLeft != -1
                    && (--usesLeft <= 0 || (CurseEvents.hasAmulet(target) && --usesLeft <= 0))) {
