        ...
        if (ticksExisted == 1 && appearTimer == 0) {
            ...
            for (int xx = minX; xx <= maxX; xx++) {
                for (int zz = minZ; zz <= maxZ; zz++) {
                    if (BlockPosM.tmp(xx, yy, zz).getBlock(worldObj) == BlockList.dungeon_puzzle
                            && BlockPosM.tmp(xx, yy, zz).getMetadata(worldObj) != BlockDungeonPuzzle.metaDisabled)
            ...
            if (BlockPosM.tmp(this).getBlock(worldObj) != BlockList.dungeon_puzzle) setDead();
                ...
                for (EntityPlayer player : (List<EntityPlayer>) worldObj.getEntitiesWithinAABB(
                        EntityPlayer.class,
                        AxisAlignedBB.getBoundingBox(minX, posY, minZ, maxX, posY + 3D, maxZ))) {
                    if (tmpPos.set(player.posX, tmpPos.y, player.posZ).getBlock(worldObj) == BlockList.dungeon_puzzle) {
                        ...
                        player.setPositionAndUpdate(
                                tmpPos.x + 0.5D,
                                worldObj.getTopSolidOrLiquidBlock(tmpPos.x, tmpPos.z),
                                tmpPos.z + 0.5D);
                        ...
                        PacketPipeline.sendToAllAround(
                                ...
                                64D,
                                new C22EffectLine(
                                        FXType.Line.DUNGEON_PUZZLE_TELEPORT,
                                        player.posX,
                                        player.posY,
                                        player.posZ,
                                        prevX,
                ...
                fiend.setLocationAndAngles(
                        tmpPos.x + 0.5D + (rand.nextDouble() - 0.5D) * 18D,
                        worldObj.getTopSolidOrLiquidBlock(tmpPos.x, tmpPos.z) + 10,
                        tmpPos.z + 0.5D + (rand.nextDouble() - 0.5D) * 18D,
                        rand.nextFloat() * 360F,
                        0F);
