...
public class EntityTechnicalCurseBlock extends EntityTechnicalBase implements ICurseCaller {
    ...
    public void onUpdate() {
        if (worldObj.isRemote) {
            ...
            if (curseType != null) {
                ...
                boolean forceRenderFX = client.getEntityId() == ownerEntityID
                        || (client.getHeldItem() != null && client.getHeldItem().getItem() == ItemList.curse_amulet);

                if (!forceRenderFX) {
                    for (EntityLivingBase entity : (List<EntityLivingBase>) worldObj
                            .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(1.75D, 0.1D, 1.75D))) {
                        if (entity == client) {
                            disappearTimer = 120;
                            break;
                        }
                ...
                if (forceRenderFX || (disappearTimer > 0 && --disappearTimer > 0)) {
                    for (int a = 0; a < 1 + rand.nextInt(dist > 16D ? 2 : 3); a++) HardcoreEnderExpansion.fx.curse(
                            worldObj,
                            posX + (rand.nextDouble() - 0.5D) * 3D,
                            posY,
                            posZ + (rand.nextDouble() - 0.5D) * 3D,
                            curseType);
                }
            }
        ...
        for (EntityLivingBase entity : (List<EntityLivingBase>) worldObj
                .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(1.5D, 0.1D, 1.5D))) {
            if (entity.getUniqueID().equals(owner) || entity instanceof IBossDisplayData) continue;
            else {
                newAffectedEntities.add(entity);

                if (curseType.handler.tickEntity(entity, this) && usesLeft != -1
                        && (--usesLeft <= 0 || (CurseEvents.hasAmulet(entity) && --usesLeft <= 0))) {
