...
public class EntityBlockEnhancedTNTPrimed extends EntityTNTPrimed {
    ...
    public void onUpdate() {
        if (!worldObj.isRemote && ticksExisted == 1)
            dataWatcher.updateObject(18, EnhancementEnumHelper.serialize(tntEnhancements));
        ...
        if (fuse-- == 5 || (fuse == 0 && tntEnhancements.contains(TNTEnhancements.NO_FUSE)))
            dataWatcher.updateObject(17, Byte.valueOf((byte) 1));
    ...
    private void explode() {
        EnhancedTNTExplosion explosion = new EnhancedTNTExplosion(
                worldObj,
                this,
                posX,
                posY,
                posZ,
                tntEnhancements.contains(TNTEnhancements.EXTRA_POWER) ? 5.2F : 4F);
    ...
    public void setDead() {
        ...
        if (!isDead && worldObj.isRemote && dataWatcher.getWatchableObjectByte(17) == 1) {
            tntEnhancements = EnhancementEnumHelper
                    .deserialize(dataWatcher.getWatchableObjectString(18), TNTEnhancements.class);
            explode();
        }
    }
    ...
    private static final class EnhancedTNTExplosion extends Explosion {
        ...
        public void doExplosionA() {
                        ...
                        if (x == 0 || x == dist - 1 || y == 0 || y == dist - 1 || z == 0 || z == dist - 1) {
                            ...
                            for (float mp = 0.3F; affectedDistance > 0F; affectedDistance -= mp * 0.75F) {
                                ...
                                if (block.getMaterial() != Material.air) {
                                    float resistance = exploder != null ? exploder
                                            .func_145772_a(this, worldObj, testPos.x, testPos.y, testPos.z, block)
                                ...
                                if (affectedDistance > 0F && (exploder == null || exploder.func_145774_a(
                                        this,
                                        worldObj,
                                        testPos.x,
                                        testPos.y,
            ...
            int minX = MathHelper.floor_double(explosionX - explosionSize - 1D),
                    maxX = MathHelper.floor_double(explosionX + explosionSize + 1D);
            int minY = MathHelper.floor_double(explosionY - explosionSize - 1D),
                    maxY = MathHelper.floor_double(explosionY + explosionSize + 1D);
            int minZ = MathHelper.floor_double(explosionZ - explosionSize - 1D),
                    maxZ = MathHelper.floor_double(explosionZ + explosionSize + 1D);
            ...
            for (int a = 0; a < entities.size(); ++a) {
                ...
                if (entityDist <= 1D) {
                    ...
                    if (totalDist != 0D) {
                        ...
                        double blastPower = (1D - entityDist)
                                * worldObj.getBlockDensity(locationVec, entity.boundingBox);
                        if (damageEntities) entity.attackEntityFrom(
                                DamageSource.setExplosionSource(this),
                                ((int) ((blastPower * blastPower + blastPower) / 2D * 8D * explosionSize + 1D)));
