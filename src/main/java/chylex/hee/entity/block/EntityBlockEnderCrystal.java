    ...
    public boolean attackEntityFrom(DamageSource source, float damage) {
        if (isEntityInvulnerable()) return false;
            ...
            if (worldObj.provider.dimensionId == 1)
                WorldDataHandler.<DragonSavefile>get(DragonSavefile.class).destroyCrystal(crystalKey);

            Entity tar = source.getEntity();

            if (crystalType == TNT) {
                if (tar instanceof EntityPlayer) {
                    int limiter = 4 + worldObj.difficultySetting.getDifficultyId(), topblock = DragonUtil.getTopBlockY(
                            worldObj,
                            Blocks.end_stone,
                            MathUtil.floor(posX),
                            MathUtil.floor(posZ),
                    ...
                    for (EntityEnderman enderman : (List<EntityEnderman>) worldObj.getEntitiesWithinAABB(
                ...
                for (int a = 0; a < 8; a++) {
                    ...
                    tnt.fuse = (int) (58 + (posY - DragonUtil.getTopBlockY(
                            worldObj,
                            Blocks.end_stone,
                            MathUtil.floor(posX),
                            MathUtil.floor(posZ),
                            MathUtil.floor(posY))) / 2);
                    ...
                    for (pos.z = tmpPos.z - 4; pos.z <= tmpPos.z + 4; pos.z++) {
                        for (pos.y = terY; pos.y <= tmpPos.y; pos.y++) {
                            if (pos.getBlock(worldObj) == BlockList.obsidian_falling) {
                                ...
                                EntityBlockFallingObsidian obsidian = new EntityBlockFallingObsidian(
                                        worldObj,
                                        pos.x + 0.5D,
                                        pos.y + 0.1D,
                                        pos.z + 0.5D);
                                obsidian.motionX = (vec[0] + (rand.nextFloat() * 0.5F - 0.25F)) * 2.25F
                                        * rand.nextFloat();
                                obsidian.motionZ = (vec[1] + (rand.nextFloat() * 0.5F - 0.25F)) * 2.25F
                                        * rand.nextFloat();
