...
public class EntityItemIgneousRock extends EntityItem {
    ...
    public EntityItemIgneousRock(World world, double x, double y, double z, ItemStack is) {
        ...
        if (thrower != null)
            thrownDirection = (byte) (MathHelper.floor_double((thrower.rotationYaw * 4F / 360F) + 0.5D) & 3);
    }
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (rand.nextInt(64 - Math.min(32, is.stackSize / 2)) == 0) {
                for (int attempt = 0; attempt < 4 + (is.stackSize / 8); attempt++) {
                    ...
                    tmpPos.move(
                            MathUtil.floor((rand.nextDouble() - 0.5D) * 4D),
                            MathUtil.floor((rand.nextDouble() - 0.5D) * 4D),
                            MathUtil.floor((rand.nextDouble() - 0.5D) * 4D));
                    ...
                    if (block.getMaterial() != Material.air) {
                        PacketPipeline.sendToAllAround(
                                ...
                                64D,
                                new C20Effect(
                                        FXType.Basic.IGNEOUS_ROCK_MELT,
                                        tmpPos.x + 0.5D,
                                        tmpPos.y + 0.5D,
                                        tmpPos.z + 0.5D));
                    }
            ...
            if (rand.nextInt(80 - Math.min(32, is.stackSize / 3)) == 0) {
                List<EntityLivingBase> nearbyEntities = worldObj
                        .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(3D, 3D, 3D));
                if (!nearbyEntities.isEmpty()) nearbyEntities.get(rand.nextInt(nearbyEntities.size()))
                        .setFire(1 + rand.nextInt(4) + getEntityItem().stackSize / 10);
            }
        }
        ...
        if (rand.nextInt(6) == 0 && worldObj.getBlock(ix, iy, iz).getMaterial() == Material.water) {
            HardcoreEnderExpansion.fx.bubble(
                    worldObj,
                    posX + 0.2F * (rand.nextFloat() - 0.5F),
                    posY + 0.2F * (rand.nextFloat() - 0.5F),
                    posZ + 0.2F * (rand.nextFloat() - 0.5F),
                    0D,
                    0.6D,
                    0D);
        }
            ...
            if (BlockDungeonPuzzle.canTrigger(meta)) {
                ...
                if (!worldObj.isRemote && onGround) {
                    worldObj.spawnEntityInWorld(
                            new EntityTechnicalPuzzleChain(worldObj, ix, iy - 1, iz, thrownDirection));
                    setDead();
                }
            }
        }
        ...
        if (rand.nextInt(30) == 0) {
            for (int a = 0; a < 2; a++) worldObj.spawnParticle(
                    "lava",
                    posX + 0.2F * (rand.nextFloat() - 0.5F),
                    posY + 0.2F * (rand.nextFloat() - 0.5F),
                    posZ + 0.2F * (rand.nextFloat() - 0.5F),
