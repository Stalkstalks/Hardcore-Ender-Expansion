...
public class EntityBossEnderDemon extends EntityFlying implements IBossDisplayData, IIgnoreEnderGoo {
    ...
    private byte healthRegenTimer = 10, lightningStartCounter = 30, lightningCounter, endermanSpawnTimer = 25,
            obsidianSpawnTimer = 69;
    ...
    protected void updateEntityActionState() {
        if (lastAttacker != null
                && (lastAttacker.isDead || !lastAttacker.playerNetServerHandler.func_147362_b().isChannelOpen()))
            lastAttacker = null; // OBFUSCATED get network manager
        ...
        if (lightningTarget != null) {
            if (ticksExisted % 18 == 0) {
                double xx = lightningTarget.posX + (rand.nextDouble() - 0.5D) * 1.5D, yy = lightningTarget.posY,
                        zz = lightningTarget.posZ + (rand.nextDouble() - 0.5D) * 1.5D;

                lightningTarget.attackEntityFrom(
                        new DamageSourceMobUnscaled(this),
                        DamageSourceMobUnscaled.getDamage(ModCommonProxy.opMobs ? 7F : 4F, worldObj.difficultySetting));
            ...
            if (--endermanSpawnTimer < -100) {
                ...
                for (EntityPlayer player : (List<EntityPlayer>) worldObj
                        .getEntitiesWithinAABB(EntityPlayer.class, boundingBox.expand(128D, 64D, 128D))) {
                    ...
                    for (attempt = 0; attempt < 40; attempt++) {
                        ...
                        for (iy = MathUtil.floor(player.posY) - 2; iy < player.posY + 3; iy++) {
                            if (worldObj.isAirBlock(ix, iy, iz) && worldObj.isAirBlock(ix, iy + 1, iz)
                                    && worldObj.isAirBlock(ix, iy + 2, iz)) {
                                for (int a = 0; a < (ModCommonProxy.opMobs ? 4 : 3); a++) {
                                    EntityMobAngryEnderman enderman = new EntityMobAngryEnderman(
                                            worldObj,
                                            ix + rand.nextDouble(),
                                            iy,
                                            iz + rand.nextDouble());
                                ...
                                EntityWeatherEffect bolt = new EntityWeatherLightningBoltDemon(
                                        worldObj,
                                        ix + 0.5D,
                                        iy,
                                        iz + 0.5D,
            ...
            if (--obsidianSpawnTimer < -120) {
                ...
                if (!list.isEmpty()) {
                    ...
                    for (int attempt = 0, placed = 0, xx, yy, zz; attempt < 25
                            && placed < 12 + worldObj.difficultySetting.getDifficultyId() * 2; attempt++) {
        ...
        for (int ix = MathUtil.floor(posX), iz = MathUtil.floor(posZ), yy = MathUtil.floor(posY); yy
                > posY - 22; yy--) {
        ...
        if (lastAttacker != null) {
            ...
            rotationYaw = DragonUtil
                    .rotateSmoothly(rotationYaw, (float) (Math.atan2(diffZ, diffX) * 180D / Math.PI) - 270F, 2F);
            rotationPitch = DragonUtil
                    .rotateSmoothly(rotationPitch, (float) (-(Math.atan2(diffY, distance) * 180D / Math.PI)), 8F);
