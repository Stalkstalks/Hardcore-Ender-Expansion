...
public class DragonAttackSummoning extends DragonSpecialAttackBase {
    ...
    public void update() {
        ...
        if (++summonTimer > 35 - Math.min(viablePlayers.size() * 4 + (ModCommonProxy.opMobs ? 5 : 0), 20)) {
            ...
            for (int amt = MathUtil.clamp(
                    MathUtil.ceil(viablePlayers.size() * (0.2D + rand.nextDouble() * 0.25D)),
                    1,
                    viablePlayers.size()), aggro = 0, total = 0; amt > 0; amt--) {
                EntityPlayer player = viablePlayers.remove(rand.nextInt(viablePlayers.size()));

                for (EntityMobAngryEnderman enderman : (List<EntityMobAngryEnderman>) dragon.worldObj
                        .getEntitiesWithinAABB(EntityMobAngryEnderman.class, player.boundingBox.expand(14D, 5D, 14D))) {
                ...
                if (aggro < getDifficulty() && total < 6 + getDifficulty()) {
                    ...
                    if (flying) {
                        if (lastStriked.adjustOrPutValue(player.getPersistentID(), (byte) -1, (byte) 0) <= 0) {
                            ...
                            dragon.worldObj.addWeatherEffect(
                                    new EntityWeatherLightningBoltSafe(
                                            dragon.worldObj,
                                            player.posX,
                                            player.posY,
                                            player.posZ));
                    ...
                    for (int a = 0; a < 3 + rand.nextInt(getDifficulty()); a++) {
                        double x = player.posX + (rand.nextDouble() - 0.5D) * 13D,
                                z = player.posZ + (rand.nextDouble() - 0.5D) * 13D;
                        int y = 1 + DragonUtil.getTopBlockY(
                                dragon.worldObj,
                                Blocks.end_stone,
                                MathUtil.floor(x),
                                MathUtil.floor(z),
                                MathUtil.floor(player.posY + 8));
                        ...
                        if ((getDifficulty() > 1 || ModCommonProxy.opMobs)
                                && rand.nextInt(100) < 5 + getDifficulty() * 10 + (ModCommonProxy.opMobs ? 25 : 0)) {
