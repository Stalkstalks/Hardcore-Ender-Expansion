...
public class DragonAttackDefault extends DragonSpecialAttackBase {
    ...
    public void update() {
        ...
        if (!stealthInProgress && dragon.target == null && (targetCooldown == 0 || --targetCooldown == 0)) {
            ...
            int attackChance = Math.min(
                    170,
                    (dragon.angryStatus ? 45 : 5) + getDifficulty() * 5
                            + Math.min(viablePlayers, 5) * 14
                            + (ModCommonProxy.opMobs ? 10 : 0)
                            + (50 - (healthPerc >> 1)));

            if ((nextTargetTimer < 80 && rand.nextInt(260 - attackChance) == 0) || nextTargetTimer <= 0
                    || --nextTargetTimer <= 0) {
                nextTargetTimer = 150 + rand.nextInt(40)
                        + (healthPerc >> 2)
                        + (dragon.angryStatus ? 0 : 35)
                        - getDifficulty() * 6
                        - Math.min(dragon.worldObj.playerEntities.size(), 5) * 5;
        ...
        if (!dragon.angryStatus) {
            if (!stealthInProgress && ++seesCheck > 20 && dragon.target == null) {
                for (EntityPlayer player : (List<EntityPlayer>) dragon.worldObj.playerEntities) {
                    ...
                    if (getVision(dragon, player)
                            && seesDragon.adjustOrPutValue(id, (byte) 1, (byte) 1) > 4 + (getDifficulty() >> 1)) {
                ...
                seesCheck = 0;
            } else if (stealthInProgress && overrideTarget != null
                    && seesCheck % 3 == 0
                    && dragon.dragonPartHead.getDistanceSqToEntity(overrideTarget) < 27D) {
