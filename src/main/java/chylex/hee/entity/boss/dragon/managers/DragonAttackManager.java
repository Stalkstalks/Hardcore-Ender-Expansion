...
public class DragonAttackManager {
    ...
    public void registerPassive(DragonPassiveAttackBase attack) {
        if (getPassiveAttackById(attack.id) != null) throw new IllegalArgumentException(
                "Tried to register passive dragon attack with already registered attack ID " + attack.id);
        passiveAttackList.add(attack);
    }

    public void registerSpecial(DragonSpecialAttackBase attack) {
        if (getSpecialAttackById(attack.id) != null) throw new IllegalArgumentException(
                "Tried to register special dragon attack with already registered attack ID " + attack.id);
    ...
    public List<EntityPlayer> getViablePlayers() {
        List<EntityPlayer> players = dragon.worldObj.getEntitiesWithinAABB(
                EntityPlayer.class,
    ...
    public boolean biteClosePlayers() {
        ...
        for (EntityPlayer player : (List<EntityPlayer>) dragon.worldObj.getEntitiesWithinAABB(
                EntityPlayer.class,
                dragon.dragonPartHead.boundingBox.expand(2.2D, 1.5D, 2.2D))) {
            ...
            if (dragon.worldObj.rand.nextInt(100) < rm) {
                player.addPotionEffect(
                        new PotionEffect(Potion.poison.id, 90 + 30 * diff, ModCommonProxy.opMobs ? 1 : 0));
    ...
    public NBTTagCompound writeToNBT() {
        ...
        tag.setByteArray(
                "attq",
                ArrayUtils.toPrimitive(specialAttackQueue.toArray(new Byte[specialAttackQueue.size()])));
        return tag;
    }
