    ...
    public void update() {
        ...
        if (++timer > 125 - getDifficulty() * 10) {
            ...
            for (EntityPlayer player : dragon.attacks.getViablePlayers()) {
                EntityEnderman enderman = DragonUtil.getClosestEntity(
                        player,
                        (List<EntityEnderman>) dragon.worldObj
                                .getEntitiesWithinAABB(EntityEnderman.class, player.boundingBox.expand(18D, 8D, 18D)));
                ...
                if (enderman == null) {
                    ...
                    for (int a = 0; a < 2 + rand.nextInt(3) + (getDifficulty() >> 1); a++) {
                        for (int attempt = 0; attempt < 6; attempt++) {
                            if (tmpPos.move(rand.nextInt(3) - 1, rand.nextInt(3) - 1, rand.nextInt(3) - 1)
                                    .isAir(dragon.worldObj))
                                break;
                        }
                    ...
                    for (int a = 0; a < 2 + rand.nextInt(3) + (getDifficulty() >> 1); a++) {
                        spawnBatAt(
                                enderman.posX + rand.nextDouble() - 0.5D,
                                enderman.posY + rand.nextDouble() * enderman.height,
                                enderman.posZ + rand.nextDouble() - 0.5D,
                                player);
                    }
                    ...
                    PacketPipeline.sendToAllAround(
                            enderman,
                            64D,
                            new C20Effect(FXType.Basic.ENDERMAN_BLOODLUST_TRANSFORMATION, enderman));
                }
            }
