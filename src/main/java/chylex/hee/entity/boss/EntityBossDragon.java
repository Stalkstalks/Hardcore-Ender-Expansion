...
public class EntityBossDragon extends EntityLiving
        implements IBossDisplayData, IEntityMultiPart, IMob, IIgnoreEnderGoo {
    ...
    public EntityBossDragon(World world) {
        ...
        dragonPartArray = new EntityDragonPart[] { dragonPartHead = new EntityDragonPart(this, "head", 6F, 6F),
        ...
        attacks.registerSpecial(
                new DragonAttackFireburst(this, 2, 10).setDisabledPassiveAttacks(ATTACK_FIREBALL, ATTACK_BITE));
        attacks.registerSpecial(new DragonAttackPunch(this, 3, 10).setDisabledPassiveAttacks(ATTACK_FIREBALL));
        attacks.registerSpecial(
                new DragonAttackSummoning(this, 4, 7).setDisabledPassiveAttacks(ATTACK_FIREBALL, ATTACK_BITE));
        attacks.registerSpecial(
                new DragonAttackBloodlust(this, 5, 7).setDisabledPassiveAttacks(ATTACK_FIREBALL, ATTACK_BITE));
    }
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
                250D + Math
                        .min(90, WorldDataHandler.<DragonSavefile>get(DragonSavefile.class).getDragonDeathAmount() * 15)
                        + (ModCommonProxy.opMobs ? 80D : 0D));
    }
    ...
    public void onLivingUpdate() {
        if (freezeAI) {
            ...
            if (noViablePlayers && ticksExisted % 10 == 0 && !attacks.getViablePlayers().isEmpty())
                noViablePlayers = false;
        ...
        if (!worldObj.isRemote) {
            ...
            if (angryStatus) {
                ...
                if (currentAttack.equals(defaultAttack)) {
                    if (nextAttackTicks-- <= 0 && target == null) {
                        ...
                        if ((currentAttack = attacks.pickSpecialAttack(lastAttack)) == null)
                            nextAttackTicks = (currentAttack = defaultAttack).getNextAttackTimer();
                        currentAttack.init();
                    }
                } else if (currentAttack.hasEnded() || forceAttackEnd) {
                    forceAttackEnd = false;
                    currentAttack.end();
                    nextAttackTicks = MathUtil
                            .ceil(currentAttack.getNextAttackTimer() * (0.5D + attacks.getHealthPercentage() / 200D));
            ...
            if (getHealth() > 0) {
                ...
                float wng = frozen ? HeeDebugCommand.overrideWingSpeed : currentAttack.overrideWingSpeed(),
                        curWng = getWingSpeed();
        ...
        if (worldObj.isRemote && MathHelper.cos(prevAnimTime * (float) Math.PI * 2F) <= -0.3F
                && MathHelper.cos(animTime * (float) Math.PI * 2F) >= -0.3F) {
            worldObj.playSound(posX, posY, posZ, "mob.enderdragon.wings", 5F, 0.8F + rand.nextFloat() * 0.3F, false);
        }

        prevAnimTime = animTime;

        if (getHealth() <= 0F) worldObj.spawnParticle(
                "largeexplode",
                posX + (rand.nextFloat() - 0.5F) * 8F,
                posY + 2D + (rand.nextFloat() - 0.5F) * 4F,
                posZ + (rand.nextFloat() - 0.5F) * 8F,
                0D,
                0D,
                0D);
        else {
            ...
            if (worldObj.isRemote) {
                if (newPosRotationIncrements > 0) {
                    ...
                    rotationYaw = (float) (rotationYaw + MathHelper.wrapAngleTo180_double(newRotationYaw - rotationYaw)
                ...
                if (target != null) {
                    targetX = target.posX;
                    targetZ = target.posZ;
                    targetY = target.boundingBox.minY + Math.min(
                            0.4D + Math.sqrt(Math.pow(targetX - posX, 2) + Math.pow(targetZ - posZ, 2)) / 80D - 1D,
                            10D);
                } else trySetTargetPosition(
                        targetX + rand.nextGaussian() * 2D,
                        targetY,
                        targetZ + rand.nextGaussian() * 2D);

                if ((target != null && target.isDead) || distFromTargetSq > 22500D)
                    forceAttackEnd = forceNewTarget = true;

                if (forceNewTarget || distFromTargetSq < 90D
                        || distFromTargetSq > 22500D
                ...
                motionY += yDiff * 0.1D;
                rotationYaw = MathHelper.wrapAngleTo180_float(rotationYaw);
                double d9 = MathUtil.clamp(
                        MathHelper.wrapAngleTo180_double(180D - MathUtil.toDeg(Math.atan2(xDiff, zDiff)) - rotationYaw),
                        -50D,
                        50D);

                Vec3 targetDiffVec = Vec3.createVectorHelper(targetX - posX, targetY - posY, targetZ - posZ)
                        .normalize();
                Vec3 rotationVec = Vec3.createVectorHelper(
                        MathHelper.sin(MathUtil.toRad(rotationYaw)),
                        motionY,
                        (-MathHelper.cos(MathUtil.toRad(rotationYaw)))).normalize();
                ...
                if (slowed) moveEntity(
                        motionX * moveSpeedMp * 0.8D,
                        motionY * moveSpeedMp * 0.8D,
                        motionZ * moveSpeedMp * 0.8D);
                else moveEntity(motionX * moveSpeedMp, motionY * moveSpeedMp, motionZ * moveSpeedMp);

                double motionMultiplier = 0.8D + 0.15D
                        * ((Vec3.createVectorHelper(motionX, motionY, motionZ).normalize().dotProduct(rotationVec) + 1D)
                                * 0.5D);
            ...
            float offsetAngle = MathUtil
                    .toRad((float) (getMovementOffsets(5, 1F)[1] - getMovementOffsets(10, 1F)[1]) * 10F);
            ...
            collideWithEntities(
                    worldObj.getEntitiesWithinAABBExcludingEntity(
                            this,
                            dragonPartWing1.boundingBox.expand(1.5D, 2D, 1.5D).offset(0D, -2D, 0D)));
            collideWithEntities(
                    worldObj.getEntitiesWithinAABBExcludingEntity(
                            this,
                            dragonPartWing2.boundingBox.expand(1.5D, 2D, 1.5D).offset(0D, -2D, 0D)));
            collideWithEntities(
                    worldObj.getEntitiesWithinAABBExcludingEntity(
                            this,
                            dragonPartTail3.boundingBox.expand(0.8D, 1D, 0.8D)));
            collideWithEntities(
                    worldObj.getEntitiesWithinAABBExcludingEntity(
                            this,
                            dragonPartHead.boundingBox.expand(0.6D, 1D, 0.6D)));
            ...
            dragonPartHead.setLocationAndAngles(
                    posX + moveX * 5.5F * angleCos,
                    posY + currentOffsets[1] - oldOffsets[1] + angleSin * 5.5F,
                    posZ - moveZ * 5.5F * angleCos,
                    0F,
                    0F);
            ...
            for (int part = 0; part < 3; part++) {
                ...
                float partYaw = MathUtil.toRad(rotationYaw)
                        + MathUtil.toRad((float) MathHelper.wrapAngleTo180_double(partOffsets[0] - oldOffsets[0]));
                float partYawSin = MathHelper.sin(partYaw);
                float partYawCos = MathHelper.cos(partYaw);
                float partMp = (part + 1) * 2F;
                tailPart.onUpdate();
                tailPart.setLocationAndAngles(
                        posX - ((yawSin * 1.5F + partYawSin * partMp) * angleCos),
                        posY + (partOffsets[1] - oldOffsets[1]) - ((partMp + 1.5F) * angleSin) + 1.5D,
                        posZ + ((yawCos * 1.5F + partYawCos * partMp) * angleCos),
                        0F,
                        0F);
            }
            ...
            if (!worldObj.isRemote) {
                ...
                if (currentAttack.id == 1)
                    slowed |= destroyBlocksInAABB(dragonPartWing1.boundingBox.expand(0.5D, 0.5D, 0.5D))
                            | destroyBlocksInAABB(dragonPartWing2.boundingBox.expand(0.5D, 0.5D, 0.5D));
    ...
    public boolean attackEntityFromPart(EntityDragonPart dragonPart, DamageSource source, float amount) {
        ...
        if (noViablePlayers && source.getEntity() instanceof EntityPlayer
                && !attacks.isPlayerViable((EntityPlayer) source.getEntity()))
            amount *= 0.1F;
        spawnCooldown = 0;
        ...
        if (shouldChangeTarget && ticksExisted - lastAttackInterruption >= 500) {
            ...
            trySetTargetPosition(
                    posX + (MathHelper.sin(yawRad) * 5F) + ((rand.nextFloat() - 0.5F) * 2F),
        ...
        if ((source.getEntity() instanceof EntityPlayer || source.isExplosion())
            ...
            hurtResistantTime = (dragonHurtTime = (byte) (hurtTime = 15)) + 10;
    ...
    protected void onDeathUpdate() {
        ...
        if (!worldObj.isRemote) {
            if (deathTicks == 1) {
                PacketPipeline.sendToDimension(
                        dimension,
                        new C01ParticleEndPortalCreation(MathUtil.floor(posX), MathUtil.floor(posZ)));
            ...
            } else if (deathTicks > 150 && deathTicks % 5 == 0)
                DragonUtil.spawnXP(this, 550 + (250 * (rewards.getFinalDifficulty() >> 2)));
                ...
                for (int ix = xx - portalSize; ix <= xx + portalSize; ix++) {
                    for (int iz = zz - portalSize; iz <= zz + portalSize; iz++) {
                        ...
                        if (distSq <= (portalSize - 0.5D) * (portalSize - 0.5D)) {
                            if ((iy < bottomY && distSq <= ((portalSize - 1) - 0.5D) * ((portalSize - 1) - 0.5D))
                                    || iy > bottomY)
                                continue;
                            for (int a = 0; a < rand.nextInt(amount); a++) worldObj.spawnParticle(
                                    "portal",
                                    ix + rand.nextDouble(),
                                    iy + rand.nextDouble() - 0.5D,
                                    iz + rand.nextDouble(),
                                    0D,
                                    0D,
                                    0D);
            ...
            for (int minX = 2; minX < 5; minX++) {
                for (int a = 0; a < rand.nextInt(amount); a++) worldObj.spawnParticle(
                        "portal",
                        xx + rand.nextDouble(),
                        bottomY + a + rand.nextDouble() - 0.5D,
                        zz + rand.nextDouble(),
                        0D,
                        0D,
                        0D);
    ...
    private void createEnderPortal(int x, int z) {
            ...
            for (int xx = x - portalSize; xx <= x + portalSize; xx++) {
                for (int zz = z - portalSize; zz <= z + portalSize; zz++) {
                    ...
                    if (distSq <= (portalSize - 0.5D) * (portalSize - 0.5D)) {
                        ...
                        if (yy < bottomY) {
                            if (distSq <= MathUtil.square((portalSize - 1) - 0.5D))
                                tmpPos.setBlock(worldObj, Blocks.bedrock);
                        } else if (yy > bottomY) tmpPos.setAir(worldObj);
                        else if (distSq > MathUtil.square((portalSize - 1) - 0.5D))
                            tmpPos.setBlock(worldObj, Blocks.bedrock);
        ...
        for (int dir = 0; dir < 4; dir++)
            tmpPos.set(x + Direction.offsetX[dir], bottomY + 2, z + Direction.offsetZ[dir])
                    .setBlock(worldObj, Blocks.torch);
    ...
    public double[] getMovementOffsets(int offset, float partialTickTime) {
        ...
        return new double[] {
                movementBuffer[index][0]
                        + MathHelper.wrapAngleTo180_double(movementBuffer[prevIndex][0] - movementBuffer[index][0])
                                * partialTickTime,
                movementBuffer[index][1]
                        + (movementBuffer[prevIndex][1] - movementBuffer[index][1]) * partialTickTime };
    }
    ...
    private void updateEnderCrystal() {
        if (healingEnderCrystal != null) {
            if (healingEnderCrystal.isDead) {
                ...
                setHealth(getHealth() + (ModCommonProxy.opMobs ? 2F : 1F));
        }

        if (rand.nextInt(10) == 0) {
            float dist = 30F + 4F * worldObj.difficultySetting.getDifficultyId() + (ModCommonProxy.opMobs ? 8F : 0F);
            healingEnderCrystal = DragonUtil.getClosestEntity(
                    this,
                    (List<EntityEnderCrystal>) worldObj
                            .getEntitiesWithinAABB(EntityEnderCrystal.class, boundingBox.expand(dist, dist, dist)));
        }
    }
    ...
    private void collideWithEntities(List<? extends Entity> list) {
        ...
        for (Entity entity : list) {
            if (entity instanceof EntityLivingBase || entity instanceof EntityBlockFallingObsidian) {
                ...
                if (entity instanceof EntityPlayerMP) PacketPipeline.sendToPlayer(
                        (EntityPlayerMP) entity,
                        new C06SetPlayerVelocity(event.velocityX, event.velocityY, event.velocityZ));
            }
        }
    }
    ...
    private boolean destroyBlocksInAABB(AxisAlignedBB aabb) {
                ...
                for (int zz = minZ; zz <= maxZ; zz++) {
                    ...
                    if (angryStatus && block == BlockList.obsidian_falling) {
                        ...
                        spawnParticles = true;
                            ...
                            || (!angryStatus && (block == Blocks.obsidian || block == BlockList.obsidian_falling
                                    || (block == Blocks.iron_bars
                                            && tmpPos.moveDown().getBlock(worldObj) == BlockList.obsidian_falling)))) {
                                                wasBlocked = true;
                                            } else
                        if (block.canEntityDestroy(worldObj, xx, yy, zz, this)
                                && MathUtil.distance(xx - cx, yy - cy, zz - cz)
                                        <= rad + (0.9D * rand.nextDouble() - 0.4D)) {
        ...
        if (spawnParticles) worldObj.spawnParticle(
                "largeexplode",
    ...
    private void setNewTarget() {
        ...
        for (double newTargetX, newTargetY, newTargetZ;;) {
            ...
            if (MathUtil.square(posX - newTargetX) + MathUtil.square(posY - newTargetY)
                    + MathUtil.square(posZ - newTargetZ) > 100D) {
    ...
    public void trySetTargetPosition(double newTargetX, double newTargetY, double newTargetZ) {
        TargetPositionSetEvent event = new TargetPositionSetEvent(
                target,
                targetX,
                targetY,
                targetZ,
    ...
    public String getCommandSenderName() {
        return hasCustomNameTag() ? getCustomNameTag()
                : StatCollector.translateToLocal(Baconizer.mobName("entity.dragon.name"));
    }
