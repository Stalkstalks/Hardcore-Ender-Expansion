...
public class EntityProjectilePotion extends EntityPotion {
    ...
    public Item getPotionItem() {
        if (potionItem == null || potionItem == Items.potionitem) {
            if ((potionItem = Item.getItemById(dataWatcher.getWatchableObjectShort(16))) == null)
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (!worldObj.isRemote && potionItem instanceof ItemAbstractPotion) {
            for (Entity entity : (List<Entity>) worldObj
                    .getEntitiesWithinAABB(Entity.class, boundingBox.expand(4D, 2D, 4D))) {
                double dist = getDistanceSqToEntity(entity);
                if (dist < 16D) ((ItemAbstractPotion) potionItem)
                        .applyEffectThrown(entity, mop.entityHit == entity ? Double.MAX_VALUE : dist);
            }
