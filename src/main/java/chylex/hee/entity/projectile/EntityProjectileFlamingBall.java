...
public class EntityProjectileFlamingBall extends EntityFireball {
    ...
    public EntityProjectileFlamingBall(World world, EntityLivingBase shooter, double x, double y, double z,
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            if (ticksExisted == 1 && rand.nextInt(4) <= 1)
                worldObj.playSound(posX, posY, posZ, "mob.ghast.fireball", 0.8F, rand.nextFloat() * 0.1F + 1.2F, false);
        }
        ...
        if (mop.entityHit != null && shootingEntity != null) {
            ...
            if (isLiving && rand.nextInt(ModCommonProxy.opMobs ? 3 : 4) == 0) {
                double[] vec = DragonUtil.getNormalizedVector(
                        shootingEntity.posX - mop.entityHit.posX,
                        shootingEntity.posZ - mop.entityHit.posZ);
                ((EntityLivingBase) mop.entityHit).knockBack(
                        shootingEntity,
                        0.9F,
                        vec[0] * 0.15D * (0.85D + 0.2D * rand.nextDouble()),
                        vec[1] * 0.15D * (0.85D + 0.4D * rand.nextDouble()));
            }

            mop.entityHit.attackEntityFrom(DamageSource.onFire, 3F);
            if (isLiving) {
                mop.entityHit
                        .attackEntityFrom(DamageSource.causeMobDamage(shootingEntity), ModCommonProxy.opMobs ? 8F : 5F);
            }
