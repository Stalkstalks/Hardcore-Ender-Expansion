...
public class EntityProjectileCurse extends EntityThrowable {
    ...
    public CurseType getType() {
        return curseType == null ? (curseType = CurseType.getFromDamage(dataWatcher.getWatchableObjectByte(16) - 1))
                : curseType;
    }
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            if (curseType != null) {
                for (int a = 0; a < 1 + rand.nextInt(2); a++) HardcoreEnderExpansion.fx.curse(
                        worldObj,
                        posX + (rand.nextDouble() - 0.5D) * 0.15D,
                        posY + (rand.nextDouble() - 0.5D) * 0.15D,
                        posZ + (rand.nextDouble() - 0.5D) * 0.15D,
                        curseType);
            }
        ...
        if (!worldObj.isRemote) {
            if (mop.typeOfHit == MovingObjectType.ENTITY) {
                for (EntityLivingBase entity : (List<EntityLivingBase>) worldObj
                        .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(4D, 2D, 4D))) {
                    if (getDistanceSqToEntity(entity) < 16D && !entity.getUniqueID().equals(throwerID)
                            && !(entity instanceof IBossDisplayData))
                        worldObj.spawnEntityInWorld(
                                new EntityTechnicalCurseEntity(worldObj, entity, curseType, eternal));
                }
            } else if (mop.typeOfHit == MovingObjectType.BLOCK) {
                int yy = BlockPosM.tmp(mop.blockX, mop.blockY, mop.blockZ).getBlock(worldObj)
                        .isReplaceable(worldObj, mop.blockX, mop.blockY, mop.blockZ) ? mop.blockY - 1 : mop.blockY;
                worldObj.spawnEntityInWorld(
                        new EntityTechnicalCurseBlock(
                                worldObj,
                                mop.blockX,
                                yy,
                                mop.blockZ,
            ...
            worldObj.playSound(
                    ...
                    posY,
                    posZ,
                    "hardcoreenderexpansion:mob.random.curse",
                    0.8F,
                    0.9F + rand.nextFloat() * 0.2F,
                    false);
            for (int a = 0; a < 40; a++) HardcoreEnderExpansion.fx.curse(
                    worldObj,
                    posX + (rand.nextDouble() - 0.5D) * 1.5D,
                    posY + (rand.nextDouble() - 0.5D) * 1.5D,
                    posZ + (rand.nextDouble() - 0.5D) * 1.5D,
                    curseType);
        }
    }
