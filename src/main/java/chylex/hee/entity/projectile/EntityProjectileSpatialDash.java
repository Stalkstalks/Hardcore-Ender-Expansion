...
public class EntityProjectileSpatialDash extends EntityThrowable {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            for (int cycles = enhancements.contains(SpatialDashGemEnhancements.INSTANT) ? 48 : 1; cycles
                    > 0; cycles--) {
                ...
                List<Entity> collisionList = worldObj.getEntitiesWithinAABBExcludingEntity(
                        this,
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (!worldObj.isRemote) {
            if (getThrower() instanceof EntityPlayerMP) {
                ...
                PacketPipeline
                        .sendToAllAround(player, 64D, new C21EffectEntity(FXType.Entity.GEM_TELEPORT_FROM, player));
                ...
                if (player.playerNetServerHandler.func_147362_b().isChannelOpen() && player.worldObj == worldObj) { // OBFUSCATED
                    ...
                    for (int yTest = y; yTest <= y + 8; yTest++) {
                        if ((block = tmpPos.setY(yTest).getBlock(worldObj)).getBlockHardness(worldObj, x, yTest, z)
                                == -1)
                            break;
                        ...
                        for (int xTest = x - 1; xTest <= x + 1; xTest++) {
                            for (int zTest = z - 1; zTest <= z + 1; zTest++) {
                                for (int yTest = y + 1; yTest <= y + 8; yTest++) {
                                    if ((block = tmpPos.set(xTest, yTest, zTest).getBlock(worldObj))
                                            .getBlockHardness(worldObj, x, yTest, z) == -1)
                                        break;
                    ...
                    if (tryAchievement && BlockPosM.tmp(x, y, z).getBlock(worldObj).isOpaqueCube())
    ...
    private boolean canSpawnIn(Block blockBottom, Block blockTop) {
        return (blockBottom.getMaterial() == Material.air || !blockBottom.isOpaqueCube())
                && (blockTop.getMaterial() == Material.air || !blockTop.isOpaqueCube());
    }
