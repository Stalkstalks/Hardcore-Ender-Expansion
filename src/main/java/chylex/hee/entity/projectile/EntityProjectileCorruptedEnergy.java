...
public class EntityProjectileCorruptedEnergy extends EntityFireball {
    ...
    public EntityProjectileCorruptedEnergy(World world, EntityLivingBase shooter, double x, double y, double z,
            Entity target) {
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote) {
            if (ticksExisted == 1) worldObj.playSound(
                    posX,
                    posY,
                    posZ,
                    "hardcoreenderexpansion:mob.endermage.attack",
                    1F,
                    0.6F + rand.nextFloat() * 1F,
                    false);
            return;
        }

        if (ticksExisted % 3 == 0) {
            for (EntityLivingBase e : (List<EntityLivingBase>) worldObj.getEntitiesWithinAABB(
                    EntityLivingBase.class,
                    boundingBox.offset(0D, 0.5D, 0D).expand(1D, 1D, 1D))) {
