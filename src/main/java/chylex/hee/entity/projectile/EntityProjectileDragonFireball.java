...
public class EntityProjectileDragonFireball extends EntityFireball {
    ...
    public EntityProjectileDragonFireball(World world, EntityLiving shooter, double xDiff, double yDiff, double zDiff,
            float speedMp, boolean random, float power) {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (ticksExisted > 100 && Math.abs(motionX) < 0.01D && Math.abs(motionY) < 0.01D && Math.abs(motionZ) < 0.01D)
            onImpact(null);
    }
