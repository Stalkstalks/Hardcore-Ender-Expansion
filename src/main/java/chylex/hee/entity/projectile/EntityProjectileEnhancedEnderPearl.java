...
public class EntityProjectileEnhancedEnderPearl extends EntityEnderPearl {
            ...
            if (pearlTypes.contains(EnderPearlEnhancements.RIDING)) {
                ...
                for (Object o : world.loadedEntityList) {
                    if (o instanceof EntityProjectileEnhancedEnderPearl) {
                        ...
                        if (pearl.ride != null
                                && pearl.ride.getCommandSenderName().equals(player.getCommandSenderName())) {
                            pearl.ride = null;
                            if (!pearl.pearlTypes.contains(EnderPearlEnhancements.NO_FALL_DAMAGE))
                                player.attackEntityFrom(DamageSource.fall, 5F);
                            pearl.setDead();
                            break;
                        }
                    }
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (dataWatcher.getWatchableObjectString(16).equals(clientPlayer.getCommandSenderName()))
                clientPlayer.setPosition(posX, posY + clientPlayer.height, posZ);
        }
    }
    ...
    protected void onImpact(MovingObjectPosition mop) {
        ...
        if (!worldObj.isRemote) {
            if (getThrower() != null && getThrower() instanceof EntityPlayerMP) {
                ...
                if (player.playerNetServerHandler.func_147362_b().isChannelOpen() && player.worldObj == worldObj) { // OBFUSCATED
                                                                                                                    // get
                                                                                                                    // network
                                                                                                                    // manager
                    ...
                    if (!MinecraftForge.EVENT_BUS.post(event)) {
                        if (pearlTypes.contains(EnderPearlEnhancements.EXPLOSIVE))
                            worldObj.newExplosion(this, posX, posY, posZ, 2.7F, false, true);
                        if (pearlTypes.contains(EnderPearlEnhancements.FREEZE)) {
                            for (EntityLivingBase entity : (List<EntityLivingBase>) worldObj
                                    .getEntitiesWithinAABB(EntityLivingBase.class, boundingBox.expand(5D, 3D, 5D))) {
                                double dist = entity.getDistanceSqToEntity(this);
                                if (dist <= 5D) entity.addPotionEffect(
                                        new PotionEffect(
                        ...
                        if (!pearlTypes.contains(EnderPearlEnhancements.NO_FALL_DAMAGE))
                            player.attackEntityFrom(DamageSource.fall, event.attackDamage);
                        if (pearlTypes.contains(EnderPearlEnhancements.FREEZE)) PacketPipeline
                                .sendToAllAround(this, 64D, new C20Effect(FXType.Basic.ENDER_PEARL_FREEZE, this));
                    }
                }
            }
    ...
    protected float getGravityVelocity() {
        return pearlTypes.contains(EnderPearlEnhancements.NO_GRAVITY) ? 0F
                : (super.getGravityVelocity()
                        * (pearlTypes.contains(EnderPearlEnhancements.INCREASED_RANGE) ? 0.75F : 1F));
    }
