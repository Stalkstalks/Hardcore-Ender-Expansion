...
public abstract class EntitySoulCharmFX extends EntityFX {
    ...
    private static final ResourceLocation tex = new ResourceLocation(
            "hardcoreenderexpansion:textures/particles/soul_charm.png");

    private double targetX, targetY, targetZ;
    private byte indexX, indexY, age, maxAge, breakCheckTimer = 10;

    public EntitySoulCharmFX(World world, double x, double y, double z, double targetX, double targetY,
            double targetZ) {
        super(world, x, y, z);
    ...
    @Override
    public void renderParticle(Tessellator tessellator, float partialTickTime, float rotX, float rotXZ, float rotZ,
            float rotYZ, float rotXY) {
        Minecraft.getMinecraft().renderEngine.bindTexture(tex);
        EntityPlayer player = Minecraft.getMinecraft().thePlayer;

        float left = indexX * 0.25F, right = left + 0.25F, top = indexY * 0.25F, bottom = top + 0.25F,
                x = (float) (prevPosX + (posX - prevPosX) * partialTickTime
                        - interpPosX
                        + (player.prevPosX - player.posX)),
                y = (float) (prevPosY + (posY - prevPosY) * partialTickTime
                        - interpPosY
                        + (player.prevPosY - player.posY)),
                z = (float) (prevPosZ + (posZ - prevPosZ) * partialTickTime
                        - interpPosZ
        ...
        tessellator.addVertexWithUV(
                x - rotX * particleScale - rotYZ * particleScale,
                y - rotXZ * particleScale,
                z - rotZ * particleScale - rotXY * particleScale,
                right,
                bottom);
        tessellator.addVertexWithUV(
                x - rotX * particleScale + rotYZ * particleScale,
