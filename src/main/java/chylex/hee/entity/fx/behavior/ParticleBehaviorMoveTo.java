...
public class ParticleBehaviorMoveTo implements IParticleBehavior {
    ...
    public ParticleBehaviorMoveTo(EntityFX particle, double targetX, double targetY, double targetZ,
            float speedMultiplier) {
        motionVec = Vec3.createVectorHelper(
                (this.targetX = targetX) - particle.posX,
                (this.targetY = targetY) - particle.posY,
                (this.targetZ = targetZ) - particle.posZ).normalize();
    ...
    public void update(EntityFX particle) {
        ...
        if (MathUtil.distance(targetX - particle.posX, targetY - particle.posY, targetZ - particle.posZ) < checkDist)
            particle.setDead();
    }
}
