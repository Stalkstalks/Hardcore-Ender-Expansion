...
public class FXEvents {
    ...
    public void onGetBlockHardness(BreakSpeed e) {
        ...
        if (heldItem != null && heldItem.getItem() == ItemList.scorching_pickaxe
                && (ItemScorchingPickaxe.isBlockValid(e.block) || e.block == BlockList.ravaged_brick)) {
            Random rand = e.entity.worldObj.rand;
            Minecraft mc = Minecraft.getMinecraft();
            for (int fx = 0; fx < 5 - 2 * mc.gameSettings.particleSetting; fx++) HardcoreEnderExpansion.fx.flame(
                    e.entity.worldObj,
                    e.x - 0.2D + rand.nextDouble() * 1.4D,
                    e.y - 0.2D + rand.nextDouble() * 1.4D,
                    e.z - 0.2D + rand.nextDouble() * 1.4D,
                    6);
        }
    }
            ...
            if (player == null) templeSmokeEnable = false;
                ...
                for (int a = 0; a < 30; a++) {
                    ...
                    HardcoreEnderExpansion.fx.omnipresent(
                            "largesmoke",
                            player.worldObj,
                            ItemTempleCaller.templeX + 1.5D + rand.nextDouble() - 0.5D,
                            ItemTempleCaller.templeY + 2.5D,
                            ItemTempleCaller.templeZ + 6.5D + rand.nextDouble() - 0.5D,
                            0D,
                            0D,
                            0D);
                }
