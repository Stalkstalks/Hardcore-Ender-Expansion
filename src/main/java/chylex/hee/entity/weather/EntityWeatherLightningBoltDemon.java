...
public class EntityWeatherLightningBoltDemon extends EntityLightningBolt {
    ...
    public EntityWeatherLightningBoltDemon(World world, double x, double y, double z, EntityBossEnderDemon caster,
            boolean shouldMakeFire) {
        this(world, x, y, z);
        ...
        if (!world.isRemote) {
            ...
            if (!shouldMakeFire
                    && world.doChunksNearChunkExist(MathUtil.floor(x), MathUtil.floor(y), MathUtil.floor(z), 10)) {
                BlockPosM tmpPos = BlockPosM.tmp();

                for (int testX = -2; testX <= 2; ++testX) {
                    for (int testY = -2; testY <= 2; ++testY) {
                        for (int testZ = -2; testZ <= 2; ++testZ) {
                            if (tmpPos.set(ix + testX, iy + testY, iz + testZ).getBlock(world) == Blocks.fire)
