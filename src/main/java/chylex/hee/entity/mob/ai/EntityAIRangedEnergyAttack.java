...
public class EntityAIRangedEnergyAttack extends EntityAIBase {
    ...
    public void updateTask() {
        entity.getNavigator().tryMoveToEntityLiving(
                target,
                entity.getDistanceSqToEntity(target) > 100D ? moveSpeed : moveSpeed * 0.5D);
        ...
        if (entity.getEntitySenses().canSee(target)) {
            if (attackShots > 0) {
                if (++attackShotTimer > (ModCommonProxy.opMobs ? 4 : 5)) {
                    ...
                    > 100 - entity.worldObj.difficultySetting.getDifficultyId() * 7 - (ModCommonProxy.opMobs ? 15 : 0))
                attackShots = 1;
    ...
    private void shootProjectile() {
        ...
        entity.worldObj
                .spawnEntityInWorld(new EntityProjectileCorruptedEnergy(entity.worldObj, entity, x, y, z, target));
    }
}
