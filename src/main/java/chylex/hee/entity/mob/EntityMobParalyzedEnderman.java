...
public class EntityMobParalyzedEnderman extends EntityEnderman {
    ...
    public void onLivingUpdate() {
        if (entityToAttack == null && ++newTargetTimer > 60) {
            ...
            if (rand.nextInt(10) == 0) {
                List<EntityLiving> nearby = worldObj
                        .getEntitiesWithinAABB(EntityLiving.class, boundingBox.expand(8D, 4D, 8D));
        ...
        if (entityToAttack != null && (entityToAttack instanceof EntityPlayer || rand.nextInt(30) == 0))
            entityToAttack = null;
    ...
    public String getCommandSenderName() {
        return hasCustomNameTag() ? getCustomNameTag()
                : StatCollector.translateToLocal(Baconizer.mobName("entity.brainlessEnderman.name"));
    }
}
