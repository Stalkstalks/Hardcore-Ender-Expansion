...
public class EntityMobScorchingLens extends EntityMob {
        ...
        if (!worldObj.isRemote && entityToAttack != null) {
            if (getDistanceSqToEntity(entityToAttack) > 180D) entityToAttack = null;
                ...
                worldObj.spawnEntityInWorld(
                        new EntityProjectileFlamingBall(
                                worldObj,
                                this,
                                posX + look.xCoord * 0.5F,
                                posY + look.yCoord * 0.5F + 0.5D,
                                posZ + look.zCoord * 0.5F,
                                entityToAttack.posX - posX,
                                entityToAttack.posY - posY + rand.nextDouble() * 0.4D,
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        if (recentlyHit) entityDropItem(
                new ItemStack(ItemList.essence, rand.nextInt(5 + looting), EssenceType.FIERY.getItemDamage()),
                0F);
    }
