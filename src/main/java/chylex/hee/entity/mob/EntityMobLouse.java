...
public class EntityMobLouse extends EntityMob implements IIgnoreEnderGoo {
    ...
    private void updateLouseData() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth)
                .setBaseValue(14D + (attrHealth > 0 ? 10D + 8D * attrHealth : 0D));
        getEntityAttribute(SharedMonsterAttributes.movementSpeed)
                .setBaseValue(0.7D + (attrSpeed > 0 ? 0.1D + 0.07D * attrSpeed : 0D));
        getEntityAttribute(SharedMonsterAttributes.attackDamage)
                .setBaseValue(7D + 3.5D * louseData.attribute(EnumLouseAttribute.ATTACK));
    ...
    public void onUpdate() {
        ...
        if (healAbility > 0 && --healTimer <= 0) {
            ...
            List<EntityLiving> list = worldObj
                    .getEntitiesWithinAABB(EntityLiving.class, boundingBox.expand(5D, 1D, 5D));
                ...
                for (int attempt = 0, amt = list.size(); attempt < 15; attempt++) {
                    ...
                    if (entity.getHealth() < entity.getMaxHealth() && !entity.isDead) {
                        ...
                        PacketPipeline.sendToAllAround(
                                this,
                                64D,
                                new C22EffectLine(FXType.Line.LOUSE_HEAL_ENTITY, this, entity));
                        break;
                    }
                }
            }
        ...
        if (entityToAttack != null && entityToAttack.posY > posY + 1.3D
                && MathUtil.distance(posX - entityToAttack.posX, posZ - entityToAttack.posZ) <= 3D
                && (getDistanceToEntity(entityToAttack) < 8D || canEntityBeSeen(entityToAttack))) {
            jump();
        }
    }
    ...
    public boolean attackEntityAsMob(Entity entity) {
        ...
        if (entity.attackEntityFrom(DamageSource.causeMobDamage(this), dmgAmount)) {
            ...
            if (knockback > 0) {
                entity.addVelocity(
                        (-MathHelper.sin(MathUtil.toRad(rotationYaw)) * knockback * 1.25F),
                        0.1D,
                        (MathHelper.cos(MathUtil.toRad(rotationYaw)) * knockback * 1.25F));
                motionX *= 0.6D;
                motionZ *= 0.6D;
            }
    ...
    private void teleport(int level) {
        ...
        if (worldObj.isRemote) {
            for (int a = 0; a < 64; a++) {
                worldObj.spawnParticle(
                        "portal",
                        posX + (rand.nextDouble() - rand.nextDouble()) * width,
                        posY + rand.nextDouble() * height,
                        posZ + (rand.nextDouble() - rand.nextDouble()) * width,
        ...
        for (int a = 0; a < 64; a++) {
            worldObj.spawnParticle(
                    "portal",
                    oldPosX + (rand.nextDouble() - rand.nextDouble()) * width,
                    oldPosY + rand.nextDouble() * height,
                    oldPosZ + (rand.nextDouble() - rand.nextDouble()) * width,
                    (rand.nextFloat() - 0.5F) * 0.2F,
                    (rand.nextFloat() - 0.5F) * 0.2F,
                    (rand.nextFloat() - 0.5F) * 0.2F);
        }
    ...
    protected void dropRareDrop(int lootingExtraLuck) {
        ...
        if (rand.nextInt(1 + (nearbyLice >> 3)) == 0) {
            ...
            if (!abilities.isEmpty() && rand.nextBoolean())
                entityDropItem(new ItemStack(ItemList.rune, 1, RuneType.VOID.ordinal()), 0F);
