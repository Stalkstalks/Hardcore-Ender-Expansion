...
public class EntityMobForestGhost extends EntityFlying implements IMob {
    ...
    public void onLivingUpdate() {
        ...
        if (--lifeLeft < -9 || target == null
                || target.isDead
                || DragonUtil.canEntitySeePoint(target, posX, posY, posZ, 1D, true)) {
            if (target != null) {
            ...
            if (dist >= 1.9D) {
                ...
                setPositionAndUpdate(
                        posX + Math.cos(Math.toRadians(rotationYaw)) * 0.35D,
                        posY,
                        posZ + Math.sin(Math.toRadians(rotationYaw)) * 0.35D);
            } else if (lifeLeft < 80) target.attackEntityFrom(
                    new DamageSourceMobUnscaled(this),
                    DamageSourceMobUnscaled.getDamage(ModCommonProxy.opMobs ? 9F : 5F, worldObj.difficultySetting));

            if (dist < 2.1D && lifeLeft < 80)
                target.addPotionEffect(new PotionEffect(Potion.blindness.id, 80, 0, true));
        }
    }
