...
public class EntityMobHauntedMiner extends EntityFlying implements IMob {
    ...
    private byte wanderResetTimer = -120, nextAttackTimer = ATTACK_TIMER, currentAttack = ATTACK_NONE,
            currentAttackTime;
    ...
    protected void updateEntityActionState() {
        ...
        if (target == null) {
            if (--wanderResetTimer < -120 || rand.nextInt(300) == 0
                    || (motionX == 0D && motionZ == 0D && rand.nextInt(20) == 0)) {
                wanderResetTimer = 0;
            ...
            if (rand.nextInt(50) == 0) {
                List<Entity> entities = worldObj
                        .getEntitiesWithinAABBExcludingEntity(this, boundingBox.expand(32D, 16D, 32D));
                            ...
                            for (int a = 0; a < inv.mainInventory.length; a += rand.nextInt(3) + 1) {
                                ...
                                if (item == ItemList.scorching_pickaxe || item instanceof ItemPickaxe)
                                    foundMiningStuff += 4;
                                        ...
                                        || item == Items.diamond
                                        || item == Items.redstone
                                        || (item == Items.dye && is.getItemDamage() == 4)
                                        || item == Items.emerald
                                        || item == Items.coal
                                    ...
                                    foundMiningStuff += 1 + (is.stackSize >> 3);
                                        ...
                                        && ItemScorchingPickaxe.isBlockOre(((ItemBlock) item).field_150939_a))
                                    foundMiningStuff += 1 + (is.stackSize >> 3);
                            }
            ...
            if (!worldObj.isRemote) {
                if (currentAttack != ATTACK_NONE) {
                        ...
                        case ATTACK_PROJECTILES:
                            if (currentAttackTime == 50) {
                                ...
                                worldObj.spawnEntityInWorld(
                                        new EntityProjectileMinerShot(
                                                worldObj,
                                                this,
                                                posX + look.xCoord * 1.5D,
                                                posY + 0.7D,
                                                posZ + look.zCoord * 1.5D,
                                                target));
                                ...
                                PacketPipeline.sendToAllAround(
                                        this,
                                        64D,
                                        new C08PlaySound(C08PlaySound.SPAWN_FIREBALL, posX, posY, posZ, 2F, 1.8F));
                            }
                                ...
                                if (attackLavaCounter == 0) {
                                    ...
                                    for (int attempt = 0; attempt < 64; attempt++) {
                                        ...
                                        for (int yAttempt = 0; yAttempt < 7; yAttempt++) {
                                            if (tmpPos.isAir(worldObj) && testPos.set(tmpPos).moveDown()
                                                    .getBlock(worldObj).isOpaqueCube()) {
                                    ...
                                    for (int px = -1; px <= 1; px++) {
                                        for (int pz = -1; pz <= 1; pz++) {
                                            ...
                                            tmpPos.set(
                                                    attackLavaCurrentX + px,
                                                    attackLavaCurrentY - 1 + attackLavaCounter,
                                                    attackLavaCurrentZ + pz);

                                            Block block = tmpPos.getBlock(worldObj);

                                            if (block == Blocks.flowing_lava || block == Blocks.lava) continue;
                                            else if (!MathUtil.floatEquals(
                                                    block.getBlockHardness(worldObj, tmpPos.x, tmpPos.y, tmpPos.z),
                                                    -1F)) {
                                    ...
                                    for (int a = 0; a < 5; a++)
                                        Blocks.flowing_lava.updateTick(worldObj, tmpPos.x, tmpPos.y, tmpPos.z, rand);
                        ...
                        case ATTACK_BLAST_WAVE:
                            if (currentAttackTime == 30) {
                                for (Entity entity : (List<Entity>) worldObj.getEntitiesWithinAABBExcludingEntity(
                                        this,
                                        boundingBox.expand(12D, 4D, 12D).offset(0D, -2D, 0D))) {
                                    double dist = MathUtil.distance(entity.posX - posX, entity.posZ - posZ);
                                    if (dist > 12D) continue;

                                    double[] vec = DragonUtil
                                            .getNormalizedVector(entity.posX - posX, entity.posZ - posZ);
                                    double strength = 0.4D + (12D - dist) * 0.2D;
                                    vec[0] *= strength;
                                    vec[1] *= strength;

                                    entity.attackEntityFrom(DamageSource.causeMobDamage(this), 13F);
                                    if (entity instanceof EntityPlayer) PacketPipeline.sendToPlayer(
                                            (EntityPlayer) entity,
                                            new C07AddPlayerVelocity(vec[0], 0.4D, vec[1]));
                                ...
                                PacketPipeline.sendToAllAround(
                                        ...
                                        24D,
                                        new C08PlaySound(
                                                C08PlaySound.HAUNTEDMINER_ATTACK_BLAST,
                                                posX,
                                                posY,
                                                posZ,
                                                1.5F,
                    ...
                    currentAttack = (MathUtil.distance(target.posX - posX, target.posZ - posZ) < 7.5D
                            && rand.nextInt(3) != 0) || rand.nextInt(6) == 0 ? ATTACK_BLAST_WAVE
                                    : (rand.nextInt(4) != 0 ? ATTACK_PROJECTILES : ATTACK_LAVA);
            ...
            if (target.isDead || (currentAttack == ATTACK_NONE && getDistanceToEntity(target) > 40D)) {
                target = null;
                if (currentAttack != ATTACK_NONE)
                    dataWatcher.updateObject(16, Byte.valueOf(currentAttack = ATTACK_NONE));
            }
        }
        ...
        if (MathUtil.distance(targetX - posX, targetZ - posZ) > 0.1D)
            renderYawOffset = rotationYaw = rotationYawHead = -MathUtil
                    .toDeg((float) Math.atan2(targetX - posX, targetZ - posZ));
    ...
    public void onLivingUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            if (attack != ATTACK_NONE && !dead) {
                ...
                look.rotateAroundY(MathUtil.toRad(36F));
                HardcoreEnderExpansion.fx.spell(
                        worldObj,
                        posX + look.xCoord * 1.5D + (rand.nextDouble() - 0.5D) * 0.2D,
                        posY + 0.7D,
                        posZ + look.zCoord * 1.5D + (rand.nextDouble() - 0.5D) * 0.2D,
                        0.9F,
                        0.6F,
                        0F);
                look.rotateAroundY(MathUtil.toRad(-72F));
                ...
                if (attack == ATTACK_BLAST_WAVE) {
                    if (currentAttackTime == 29) {
                        for (int flame = 0; flame < 180; flame++) HardcoreEnderExpansion.fx.flame(
                                worldObj,
                                posX + (rand.nextDouble() - 0.5D) * 0.2D,
                                posY + height * 0.5D,
                                posZ + (rand.nextDouble() - 0.5D) * 0.2D,
            ...
            List<Entity> nearEntities = worldObj.getEntitiesWithinAABBExcludingEntity(
                    this,
                    bottomBB.setBounds(posX - 1.65D, posY - 3, posZ - 1.65D, posX + 1.65D, posY, posZ + 1.65D));
    ...
    public void setRevengeTarget(EntityLivingBase newTarget) {
        if (target == null || (newTarget.getDistanceSqToEntity(this) < target.getDistanceSqToEntity(this)
                && !(newTarget instanceof EntityMobHauntedMiner))) {
            target = newTarget;
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (damaged && sourceEntity instanceof EntityLivingBase) {
            ...
            if (rand.nextInt(7) == 0 || (getHealth() <= 0F && rand.nextInt(3) != 0)) {
                ...
                List<EntityMobHauntedMiner> nearby = worldObj
                        .getEntitiesWithinAABB(EntityMobHauntedMiner.class, boundingBox.expand(48D, 30D, 48D)),
                        viable = new ArrayList<>();
                ...
                if (maxTargeted > 0 && !viable.isEmpty())
                    viable.get(rand.nextInt(viable.size())).setRevengeTarget((EntityLivingBase) sourceEntity);
            }
        }
