...
public class EntityMobEndermage extends EntityMob implements IIgnoreEnderGoo, IRangedAttackMob {
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (super.attackEntityFrom(source, amount)) {
            if ((lastAttacked == 0 || (rand.nextInt(3) != 0 && getHealth() - amount * 2D <= 0D))
                ...
                for (int attempt = 0; attempt < 15; attempt++) {
                    ...
                    if (MathUtil.distance(tmpPos.x + 0.5D - sourceEntity.posX, tmpPos.z + 0.5D - sourceEntity.posZ)
                            > dist * 4D) {
                        tmpPos.y = MathUtil.floor(posY) + 7;

                        for (int yAttempt = 0; yAttempt < 14; yAttempt++) {
                            if (!testPos.set(tmpPos).moveDown().isAir(worldObj) && tmpPos.isAir(worldObj)
    ...
    public String getCommandSenderName() {
        return hasCustomNameTag() ? getCustomNameTag()
                : StatCollector.translateToLocal(Baconizer.mobName("entity.endermage.name"));
    }
}
