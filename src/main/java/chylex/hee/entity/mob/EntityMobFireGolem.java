...
public class EntityMobFireGolem extends EntityMob {
    ...
    private static final AttributeModifier cancelMovement = new AttributeModifier(
            cancelMovementModifierUUID,
    ...
    public void onLivingUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            if (flameParticleAmount > 0) {
                ...
                for (int a = 0; a < amt; a++) HardcoreEnderExpansion.fx.flame(
                        worldObj,
                        xx + (rand.nextDouble() - 0.5D) * 0.2D,
                        posY + 0.8D + rand.nextDouble() * 0.1D,
                        zz + (rand.nextDouble() - 0.5D) * 0.2D,
                        4);
            }
        } else {
                ...
                if (rangedStatus == -1 && dist > 4D && dist < 10D && canEntityBeSeen(entityToAttack)) {
                    ...
                    if (++rangedStatus > (ModCommonProxy.opMobs ? 28 : 38)) {
                        ...
                        worldObj.spawnEntityInWorld(
                                new EntityProjectileGolemFireball(
                                        worldObj,
                                        this,
                                        posX + look.xCoord * 0.8D,
                                        posY + 1D,
                                        posZ + look.zCoord * 0.8D,
                                        entityToAttack.posX - posX,
                                        entityToAttack.posY + 0.2D - posY,
                                        entityToAttack.posZ - posZ));

                        PacketPipeline.sendToAllAround(
                                this,
                                64D,
                                new C08PlaySound(
                                        C08PlaySound.SPAWN_FIREBALL,
                                        posX,
                                        posY,
                                        posZ,
                                        1.5F,
                                        0.85F + rand.nextFloat() * 0.1F));
                    }

                    if (dataWatcher.getWatchableObjectByte(16) != flameParticleAmountNew)
                        dataWatcher.updateObject(16, Byte.valueOf(flameParticleAmountNew));
                }
            }
        }
    }
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        if (recentlyHit) entityDropItem(
                new ItemStack(ItemList.essence, rand.nextInt(4 + looting) + 1, EssenceType.FIERY.getItemDamage()),
                0F);
    }
