...
public class EntityMobAngryEnderman extends EntityMob implements IEndermanRenderer, IIgnoreEnderGoo {
    ...
    private static final AttributeModifier aggroSpeedBoost = new AttributeModifier(
            aggroSpeedBoostID,
            "Attacking speed boost",
            7.4D,
            0).setSaved(false); // 6.2 -> 7.4
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(ModCommonProxy.opMobs ? 40D : 32D); // maxHealth
                                                                                                               // 40 =>
                                                                                                               // 32
        getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(0.3D);
        getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(ModCommonProxy.opMobs ? 9D : 5D); // attackDamage
                                                                                                                // 7 =>
                                                                                                                // 5
    }
    ...
    public void onLivingUpdate() {
        ...
        for (int i = 0; i < 2; ++i) {
            worldObj.spawnParticle(
                    ...
                    posZ + (rand.nextDouble() - 0.5D) * this.width,
        ...
        if (!worldObj.isRemote && isEntityAlive()) {
            if (entityToAttack != null) {
                if (entityToAttack.getDistanceSqToEntity(this) > 256D && teleportDelay++ >= 30
                ...
                if (rand.nextInt(30) == 0) {
                    for (EntityPlayer player : (List<EntityPlayer>) worldObj
                            .getEntitiesWithinAABB(EntityPlayer.class, boundingBox.expand(6D, 4D, 6D))) {
    ...
    protected boolean teleportToEntity(Entity entity) {
        Vec3 targVec = Vec3.createVectorHelper(
                posX - entity.posX,
                boundingBox.minY + (height / 2F) - entity.posY + entity.getEyeHeight(),
                posZ - entity.posZ).normalize();
        double dist = 16D;
        return teleportTo(
                posX + (rand.nextDouble() - 0.5D) * 8D - targVec.xCoord * dist,
                posY + (rand.nextInt(16) - 8) - targVec.yCoord * dist,
                posZ + (rand.nextDouble() - 0.5D) * 8D - targVec.zCoord * dist);
    }
    ...
    public void onDeath(DamageSource source) {
        if (!worldObj.isRemote && worldObj.provider.dimensionId == 1
                && worldObj.getTotalWorldTime() - EntityBossDragon.lastUpdate < 20
    ...
    public String getCommandSenderName() {
        return hasCustomNameTag() ? getCustomNameTag()
                : StatCollector.translateToLocal(Baconizer.mobName("entity.angryEnderman.name"));
    }
}
