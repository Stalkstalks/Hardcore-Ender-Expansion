...
public class ItemBlockSlab extends ItemBlock {
    ...
    public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float hitX,
        ...
        if ((side == 1 && !isTopSlab || side == 0 && isTopSlab)
                && BlockPosM.tmp(x, y, z).getBlock(world) == field_150939_a) {
            if (world.checkNoEntityCollision(fullBlock.getCollisionBoundingBoxFromPool(world, x, y, z))
                    && BlockPosM.tmp(x, y, z).setBlock(world, fullBlock)) {
                world.playSoundEffect(
                        x + 0.5D,
                        y + 0.5D,
                        z + 0.5D,
                        fullBlock.stepSound.func_150496_b(),
                        (fullBlock.stepSound.getVolume() + 1F) * 0.5F,
                        fullBlock.stepSound.getPitch() * 0.8F);
                --is.stackSize;
            }

            return true;
        } else return tryPlaceSlab(is, player, world, x, y, z, side)
    ...
    public boolean func_150936_a(World world, int x, int y, int z, int side, EntityPlayer player, ItemStack is) {
        ...
        return BlockPosM.tmp(x, y, z).move(side).getBlock(world) == field_150939_a
    ...
    private boolean tryPlaceSlab(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side) {
        ...
        if (tmpPos.getBlock(world) == field_150939_a) {
            if (world.checkNoEntityCollision(fullBlock.getCollisionBoundingBoxFromPool(world, x, y, z))
                    && tmpPos.setBlock(world, fullBlock)) {
                world.playSoundEffect(
                        x + 0.5D,
                        y + 0.5D,
                        z + 0.5D,
