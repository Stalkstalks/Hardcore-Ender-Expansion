...
public class ItemBiomeCompass extends Item {
    ...
    public void onUpdate(ItemStack is, World world, Entity entity, int slot, boolean isHeld) {
        if (world.isRemote) {
                ...
                if (lastSavedX == Integer.MAX_VALUE && lastSavedZ == Integer.MAX_VALUE) {
                    ...
                    for (int x = entity.chunkCoordX - 96, z; x <= entity.chunkCoordX + 96; x++) {
                        for (z = entity.chunkCoordZ - 96; z <= entity.chunkCoordZ + 96; z++) {
                            ...
                            if (biome != -1) locations.get(biome).add(
                                    new ChunkCoordinates(
                                            x * 16 + (IslandSpawnChecker.featureSize >> 1),
                                            0,
                                            z * 16 + (IslandSpawnChecker.featureSize >> 1)));
                        }
                    }

                    lastSavedX = entity.chunkCoordX;
                    lastSavedZ = entity.chunkCoordZ;
                } else if (MathUtil.square(entity.chunkCoordX - lastSavedX)
                        + MathUtil.square(entity.chunkCoordZ - lastSavedZ) > 250F) {
            ...
            if (entity instanceof EntityPlayerMP) {
                ...
                if (!player.func_147099_x().hasAchievementUnlocked(AchievementManager.BIOME_COMPASS))
                    player.addStat(AchievementManager.BIOME_COMPASS, 1); // OBFUSCATED getStatisticsFile
            }
            ...
            if (!nbt.hasKey("seed1")) {
                ...
                nbt.setShort(
                        "seed2",
                        (short) (1
                                + WorldDataHandler.<DragonSavefile>get(DragonSavefile.class).getDragonDeathAmount()));
    ...
    public void registerIcons(IIconRegister iconRegister) {
        ((TextureMap) iconRegister).setTextureEntry(
                "hardcoreenderexpansion:biome_compass",
                (TextureAtlasSprite) (itemIcon = new TextureBiomeCompass("hardcoreenderexpansion:biome_compass")));
