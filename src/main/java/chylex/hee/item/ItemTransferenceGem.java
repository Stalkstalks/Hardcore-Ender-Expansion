...
public class ItemTransferenceGem extends ItemAbstractEnergyAcceptor {
    ...
    public boolean onItemUse(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float hitX,
        ...
        if (!world.isRemote && side == 1 && player.isSneaking()) {
            ...
            PacketPipeline
                    .sendToAllAround(player.dimension, x, y, z, 64D, new C20Effect(FXType.Basic.GEM_LINK, x, y, z));
            return true;
        }
    ...
    public boolean itemInteractionForEntity(ItemStack is, EntityPlayer player, EntityLivingBase entity) {
        if (entity instanceof IBossDisplayData
                || !EnhancementHandler.hasEnhancement(is, TransferenceGemEnhancements.BEAST)
                || player.isSneaking())
            return false;
    ...
    public ItemStack tryTeleportEntity(ItemStack is, EntityPlayer player, Entity entity) {
        ...
        if (gemData.isLinked() && entity.dimension == gemData.dim) {
            ...
            if (isLiving) ((EntityLivingBase) entity)
                    .setPositionAndUpdate(gemData.x + 0.5D, gemData.y + 1.001D, gemData.z + 0.5D);
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List textLines, boolean showAdvancedInfo) {
        if (is.hasTagCompound()) {
            ...
            if (clientCache.isLinked()) {
                ...
                if (showAdvancedInfo) textLines.add(
                        new StringBuilder().append(EnumChatFormatting.GRAY.toString()).append("DIM ")
                                .append(clientCache.dim).append(", X ").append(clientCache.x).append(", Y ")
                                .append(clientCache.y).append(", Z ").append(clientCache.z).toString());
            }
        }
    ...
    public void registerIcons(IIconRegister iconRegister) {
        ...
        for (int a = 0; a < iconArray.length; a++)
            iconArray[a] = iconRegister.registerIcon("hardcoreenderexpansion:transference_gem_" + (a + 1));
    }
    ...
    private static class GemData {
        ...
        private void set(NBTTagCompound nbt) {
            set(
                    nbt.hasKey("HED_Gem_Dim") ? nbt.getInteger("HED_Gem_Dim") : -999,
                    nbt.getInteger("HED_Gem_X"),
                    nbt.getInteger("HED_Gem_Y"),
                    nbt.getInteger("HED_Gem_Z"));
        }
