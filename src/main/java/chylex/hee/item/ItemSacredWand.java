...
public class ItemSacredWand extends ItemAbstractEnergyAcceptor {
    ...
    public static boolean attackEntity(ItemStack is, EntityPlayer player, EntityLivingBase entity,
            EntityProjectileSacredWand projectile) {
        ...
        if (cores.contains(WandCore.DEXTERITY)) {
            ...
            List<EntityLiving> closest = DragonUtil.getClosestEntities(
                    max,
                    entity,
                    entity.worldObj.getEntitiesWithinAABB(EntityLiving.class, entity.boundingBox.expand(4D, 4D, 4D)));

            for (EntityLiving e : closest) {
                if (e.getDistanceToEntity(entity) <= (enhancements.contains(SacredWandEnhancements.RANGE) ? 4D : 2.5D))
                    newAttacked.add(e);
            }
        ...
        if (cores.contains(WandCore.REPULSION))
            knockback += isMelee ? 1 + MathUtil.floor(Math.max(0D, 6D - player.getDistanceToEntity(entity) * 0.75D))
                    : 4;
        ...
        for (EntityLivingBase target : attacked) {
            if (target.attackEntityFrom(
                    isMelee ? DamageSource.causePlayerDamage(player)
                            : new EntityDamageSourceIndirect("player", projectile, player),
                    magic ? damage * 0.8F : damage)) {
                if (knockback > 0) {
                ...
                EnchantmentHelper.func_151385_b(player, target); // OBFUSCATED some kind of damage event (there was
                                                                 // another one for thorns, screw that one)
    ...
    public Multimap getAttributeModifiers(ItemStack is) {
        ...
        map.put(
                ...
                new AttributeModifier(field_111210_e, "Weapon modifier", 0F, 0));
        return map;
    }
