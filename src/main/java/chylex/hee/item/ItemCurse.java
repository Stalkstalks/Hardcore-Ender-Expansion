...
public class ItemCurse extends Item {
            ...
            if (!world.isRemote) {
                world.spawnEntityInWorld(
                        new EntityTechnicalCurseBlock(
                                ...
                                player.getUniqueID(),
                                type,
                                CurseType.isEternal(is.getItemDamage())));
                --is.stackSize;
    ...
    public boolean itemInteractionForEntity(ItemStack is, EntityPlayer player, EntityLivingBase entity) {
        ...
        if (!player.worldObj.isRemote) player.worldObj.spawnEntityInWorld(
                new EntityTechnicalCurseEntity(player.worldObj, entity, type, CurseType.isEternal(is.getItemDamage())));
        else {
            ...
            for (int a = 0; a < 40; a++) HardcoreEnderExpansion.fx.curse(
                    player.worldObj,
                    entity.posX + (itemRand.nextDouble() - 0.5D) * 1.5D,
                    entity.posY + (itemRand.nextDouble() - 0.5D) * 1.5D,
                    entity.posZ + (itemRand.nextDouble() - 0.5D) * 1.5D,
                    type);
        }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer player) {
        ...
        if (!world.isRemote) world.spawnEntityInWorld(
                new EntityProjectileCurse(world, player, type, CurseType.isEternal(is.getItemDamage())));
