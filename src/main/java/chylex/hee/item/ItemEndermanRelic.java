        ...
        if (!world.isRemote) {
            ...
            if (++timer > 8) {
                ...
                if (owner instanceof EntityPlayer) {
                    Class<?> cls;
                    for (EntityMob mob : (List<EntityMob>) world
                            .getEntitiesWithinAABB(EntityMob.class, owner.boundingBox.expand(2.75D, 1.5D, 2.75D))) {
                        cls = mob.getClass();
                        ...
                        if (cls == EntityEnderman.class || cls == EntityMobAngryEnderman.class) {
                            ...
                            if (mob.getLastAttacker() != owner && world.rand.nextInt(3) == 0) {
                                List<EntityLiving> otherEntities = world.getEntitiesWithinAABB(
                                        EntityLiving.class,
                                        paralyzed.boundingBox.expand(8D, 4D, 8D));
