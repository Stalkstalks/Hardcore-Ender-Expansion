...
public class ItemEnergyWand extends Item {
        ...
        if (player.isSneaking() && !world.isRemote) {
            if (ItemUtil.getTagRoot(is, false).hasKey("cluster")) {
                ...
                if (tile != null) {
                    ...
                    double dist = ItemUtil.getTagRoot(is, false).getShort("prevDim") == world.provider.dimensionId
                            ? MathUtil.distance(prevLoc.x - tmpPos.x, prevLoc.y - tmpPos.y, prevLoc.z - tmpPos.z)
                            : Double.MAX_VALUE;

                    if (dist > 8D) {
                        tile.data.setEnergyLevel(
                                tile.data.getEnergyLevel() * (1F - 0.5F * Math.min(1F, (float) dist / 256F)));
                        if (itemRand.nextInt(100) < tile.data.getHealthStatus().chanceToWeaken)
    ...
    public void addInformation(ItemStack is, EntityPlayer player, List textLines, boolean showAdvancedInfo) {
        if (ItemUtil.getTagRoot(is, false).hasKey("cluster")) textLines.add(
                I18n.format("item.energyWand.info.holding").replace(
                        "$",
                        DragonUtil.formatTwoPlaces.format(ItemUtil.getTagSub(is, "cluster", false).getFloat("lvl"))));
    }
