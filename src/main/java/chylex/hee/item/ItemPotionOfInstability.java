...
public class ItemPotionOfInstability extends ItemAbstractPotion {
        ...
        while (true) {
            ...
            if (potion instanceof TimedPotion) {
                ...
                return new PotionEffect(
                        timed.getPotionType().id,
                        (int) (rand.nextInt(MathUtil.ceil(timed.getMaxDuration() * 0.8D))
                                + timed.getMaxDuration() * 0.2D),
                        rand.nextInt(timed.getMaxLevel(false)));
            }
        }
    }
    ...
    public void applyEffectThrown(Entity entity, double dist) {
        ...
        if (dur > 20) ((EntityLivingBase) entity)
                .addPotionEffect(new PotionEffect(eff.getPotionID(), dur, eff.getAmplifier(), eff.getIsAmbient()));
    }
