...
public class TileEntityDecompositionTable extends TileEntityAbstractTable {
    ...
    private static final int[] slotsTop = new int[] { 0 }, slotsSides = new int[] { 1 },
    ...
    public void invalidateInventory() {
        ...
        if (items[0] != null && worldObj != null) {
            List<ItemStack> recipeIngredients = StardustDecomposition
                    .getRandomRecipeIngredientsFor(items[0], worldObj.rand);
            ...
            if (recipeIngredients != null) {
                ...
                int amt = Math.max(
                        1,
                        Math.round(
                                recipeIngredients.size() * (0.425F + worldObj.rand.nextFloat() * 0.415F)
                                        * (items[0].isItemStackDamageable()
                                                ? 1F - (float) items[0].getItemDamage() / items[0].getMaxDamage()
                                                : 1F)));

                ItemStack ingredient;
                for (int a = 0; a < amt; a++) {
                    (ingredient = recipeIngredients.remove(worldObj.rand.nextInt(recipeIngredients.size()))
                            .copy()).stackSize = 1;
    ...
    protected boolean onWorkFinished() {
        ...
        for (int slot = 2; slot < items.length; slot++)
            itemsCopy[slot - 2] = items[slot] == null ? null : items[slot].copy();
