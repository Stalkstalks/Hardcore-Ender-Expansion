...
public class TileEntityAccumulationTable extends TileEntityAbstractTable {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote && (channelCooldown == 0 || --channelCooldown == 0)
                && items[0] != null
                && storedEnergy >= EnergyChunkData.energyDrainUnit
                && items[0].getItem() instanceof ItemAbstractEnergyAcceptor) {
            ItemAbstractEnergyAcceptor item = (ItemAbstractEnergyAcceptor) items[0].getItem();

            if (item.canAcceptEnergy(items[0])) {
                if ((storedEnergy -= EnergyChunkData.energyDrainUnit) < EnergyChunkData.minSignificantEnergy)
                    storedEnergy = 0F;
        ...
        if (lastComparatorStatus == true
                && (items[0] == null || !(items[0].getItem() instanceof ItemAbstractEnergyAcceptor)
                        || !((ItemAbstractEnergyAcceptor) items[0].getItem()).canAcceptEnergy(items[0]))) {
