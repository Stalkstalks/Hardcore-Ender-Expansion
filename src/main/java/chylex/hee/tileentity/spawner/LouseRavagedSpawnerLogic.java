...
public class LouseRavagedSpawnerLogic extends CustomSpawnerLogic {
    ...
    protected AxisAlignedBB getSpawnerCheckBB() {
        ...
        return AxisAlignedBB.getBoundingBox(sx, sy, sz, sx + 1, sy + 1, sz + 1)
                .expand(spawnRange * 2D, 2.5D, spawnRange * 2D);
    }
    ...
    protected boolean checkSpawnerConditions() {
        ...
        return getSpawnerWorld().getEntitiesWithinAABB(
                EntityMobLouse.class,
                AxisAlignedBB.getBoundingBox(sx, sy, sz, sx + 1, sy + 1, sz + 1).expand(10D, 10D, 10D)).size() <= 9;
    }
    ...
    public static final class LouseSpawnData {
        ...
        public enum EnumLouseAttribute {
            ...
            ARMOR, // adds armor that absorbs damage, absorbing stops knockback, armor amount & regen speed increases
                   // per level
            ...
            if (parts.length != 3) this.level = 0;
                ...
                try {
                    ...
                    for (String s : parts[1].split("\\+")) {
                        ...
                        if (ordinal >= 0 && ordinal < EnumLouseAttribute.values.length)
                            attributes.add(EnumLouseAttribute.values[ordinal]);
                    }

                    for (String s : parts[2].split("\\+")) {
                        if (s.equals("nil")) break;
                        int ordinal = Integer.parseInt(s);
                        if (ordinal >= 0 && ordinal < EnumLouseAbility.values.length)
                            abilities.add(EnumLouseAbility.values[ordinal]);
                    }
