...
public class TowerEndermanSpawnerLogic extends CustomSpawnerLogic {
    ...
    protected AxisAlignedBB getSpawnerCheckBB() {
        return AxisAlignedBB
                .getBoundingBox(getSpawnerX(), minY, getSpawnerZ(), getSpawnerX() + 1, maxY, getSpawnerZ() + 1)
                .expand(spawnRange * 2D, 0.5D, spawnRange * 2D);
    }

    @Override
    protected boolean checkSpawnerConditions() {
        return getSpawnerWorld()
                .getEntitiesWithinAABB(EntityMobAngryEnderman.class, getSpawnerCheckBB().expand(32D, 256D, 32D)).size()
                < 32;
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        for (int a = 0; a < nbtEffList.tagCount(); a++)
            effects.add(PotionEffect.readCustomPotionEffectFromNBT(nbtEffList.getCompoundTagAt(a)));
