...
public class TileEntityEnhancedBrewingStand extends TileEntityBrewingStand
        implements IAcceptFieryEssence, IEnhanceableTile {

    private static final int[] topSlots = new int[] { 3 }, sideSlots = new int[] { 0, 1, 2 },
    ...
    public void updateEntity() {
        if (!worldObj.isRemote) {
            if (brewTime > 0) {
                --brewTime;
                if (brewTime > 1 && brewTime % 2 == 0 && enhancements.contains(EnhancedBrewingStandEnhancements.SPEED))
                    --brewTime;
    ...
    private boolean checkBrewingRequirements() {
        ...
        for (int a = 0; a < 3; a++) {
            ...
            if (!PotionTypes.canBeApplied(
                    slotItems[3],
                    slotItems[a],
                    enhancements.contains(EnhancedBrewingStandEnhancements.TIER)))
                return false;
            requiredPowder += PotionTypes.getRequiredPowder(slotItems[3].getItem(), slotItems[a]);
        }

        requiredPowder = (short) Math.min(
                requiredPowder * (potionCount == 2 ? 0.835F : potionCount == 3 ? 0.7F : 1F)
                        * (enhancements.contains(EnhancedBrewingStandEnhancements.COST) ? 0.65F : 1F),
                69);
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        enhancements = EnhancementEnumHelper
                .deserialize(nbt.getString("enhancements"), EnhancedBrewingStandEnhancements.class);
    }
}
