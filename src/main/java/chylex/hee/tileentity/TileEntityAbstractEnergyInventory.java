    ...
    public void updateEntity() {
        ...
        if (draining && (drainTimer == 0 || --drainTimer == 0)) {
            ...
            if (worldObj.provider.dimensionId == 1) {
                float newDrain = WorldDataHandler.<EnergySavefile>get(EnergySavefile.class)
                        .getFromBlockCoords(worldObj, xCoord, zCoord, true).drainEnergy(drain);
                if (!MathUtil.floatEquals(newDrain, drain)) PacketPipeline.sendToAllAround(
                        this,
                        64D,
                        new C10ParticleEnergyTransfer(
                                this,
                                xCoord + 0.5D,
                                yCoord + 96D,
                                zCoord + 0.5D,
            ...
            if (drain > EnergyChunkData.minSignificantEnergy) {
                ...
                if (!clusters.isEmpty()) {
                    ...
                    for (Iterator<TileEntityEnergyCluster> iter = clusters.iterator(); iter.hasNext();) {
                        if ((drain = iter.next().drainEnergy(drain, this)) < EnergyChunkData.minSignificantEnergy)
                            break;
                    }
                }
            }
