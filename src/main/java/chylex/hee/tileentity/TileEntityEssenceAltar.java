...
public class TileEntityEssenceAltar extends TileEntityAbstractSynchronized implements IEnhanceableTile {
    ...
    public ItemStack getShowedRuneItem() {
        return runeItemIndex < 0 || runeItemIndex >= runeItems.length ? null
                : runeItems[runeItemIndex] == null ? null : runeItems[runeItemIndex].getShowcaseItem();
    }
    ...
    public ItemStack createEnhancedItemStack() {
        return EnhancementHandler
                .addEnhancements(new ItemStack(BlockList.essence_altar, 1, essenceType.id), enhancementList);
    }
    ...
    public void readTileFromNBT(NBTTagCompound nbt) {
        ...
        enhancementList = EnhancementEnumHelper
                .deserialize(nbt.getString("enhancements"), EssenceAltarEnhancements.class);
    ...
    private void createActionHandler() {
        try {
            actionHandler = essenceType.actionHandlerClass.getConstructor(TileEntityEssenceAltar.class)
                    .newInstance(this);
    ...
    private void addOrRenewCache(EntityPlayer player, ItemStack is) {
        ...
        if (cache == null || is.getItem() != cache.item || is.getItemDamage() != cache.damage)
            playerItemCache.put(player.getCommandSenderName(), new ItemUseCache(is));
    ...
    public void onRightClick(EntityPlayer player) {
        ...
        int giveAmount = player.capabilities.isCreativeMode ? (player.isSneaking() ? 1 : 32)
                : Math.min(is.stackSize, player.isSneaking() ? 1 : 32);
        ...
        if (is.getItem() == ItemList.essence) {
            ...
            if (runeItems[runeItemIndex] != null && runeItems[runeItemIndex].selector.isValid(is)) {
                PacketPipeline.sendToAllAround(
                        this,
                        32D,
                        new C17AltarRuneItemEffect(this, runeItems[runeItemIndex].indexInArray));
    ...
    public void onBlockDestroy() {
        if (currentStage == STAGE_HASTYPE) worldObj
                .spawnEntityInWorld(createItem(this, new ItemStack(ItemList.essence, 1, essenceType.getItemDamage())));
        ...
        if (essenceLevel - (16 * essence16) > 0) worldObj.spawnEntityInWorld(
                createItem(
                        this,
                        new ItemStack(ItemList.essence, essenceLevel - (16 * essence16), essenceType.getItemDamage())));
    }
