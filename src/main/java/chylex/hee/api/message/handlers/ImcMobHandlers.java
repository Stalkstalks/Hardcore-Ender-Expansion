...
public final class ImcMobHandlers extends ImcHandler {
    ...
    private static final MessageHandler enderGooTolerantAdd = new MessageHandler() {
        ...
        public void call(MessageRunner runner) {
            if (GlobalMobData.setEnderGooTolerant(runner.getString("id")))
    ...
    private static final MessageHandler energySet = new MessageHandler() {
        ...
        @Override
        public void call(MessageRunner runner) {
            if (EnergyValues.setMobEnergy(
                    (Class<? extends EntityLivingBase>) EntityList.stringToClassMapping.get(runner.getString("id")),
                    (float) runner.getDouble("units")))
                MessageLogger.logOk("Added 1 mob to the list.");
            else MessageLogger.logFail("The mob was already in the list.");
        }
    };
    ...
    public void register() {
        ...
        register("HEE:Mobs:SetEnergy", energySet, RunEvent.LOADCOMPLETE).addProp("id", SpawnEntryValue.livingMobString)
