...
public class ItemPatternValue extends PreconditionComposite<ItemPattern> {
    ...
    private ItemPatternValue() {
        ...
        addCondition(
                "damage",
                Optional.of(IntArrayValue.condition(IntValue.range(0, Short.MAX_VALUE)), ArrayUtils.EMPTY_INT_ARRAY));
    ...
    public ItemPattern getValue(MessageRunner runner) {
        ...
        return new ItemPattern().setItemName(name.getLeft(), name.getRight())
                .setDamageValues(runner.getIntArray("damage")).setNBT(runner.<NBTTagCompound>getValue("tag"));
    }
}
