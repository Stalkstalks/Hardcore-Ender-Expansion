...
public class DecimalValue extends Precondition<Double> {
    ...
    public boolean checkType(NBTBase tag) {
        return tag != null && (tag.getId() == NBT.TAG_INT || tag.getId() == NBT.TAG_BYTE
                || tag.getId() == NBT.TAG_SHORT
                || tag.getId() == NBT.TAG_LONG
                || tag.getId() == NBT.TAG_DOUBLE
                || tag.getId() == NBT.TAG_FLOAT);
    }
