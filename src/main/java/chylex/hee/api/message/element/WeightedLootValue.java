...
public class WeightedLootValue extends PreconditionComposite<LootItemStack> {
    ...
    private WeightedLootValue() {
        ...
        addCondition(
                "damage",
                Optional.of(
                        IntArrayValue.condition(IntValue.range(0, Short.MAX_VALUE), IntValue.range(1, 2)),
                        new int[] { 0 }));
        addCondition(
                "count",
                Optional.of(IntArrayValue.condition(IntValue.range(1, 64), IntValue.range(1, 2)), new int[] { 1 }));
    ...
    public LootItemStack getValue(MessageRunner runner) {
        ...
        if (item.getItem() == null) throw new IllegalStateException(
                "Failed constructing a LootItemStack (WeightedItem) from IMC, item is null.");
