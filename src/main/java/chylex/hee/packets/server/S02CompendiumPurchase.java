...
public class S02CompendiumPurchase extends AbstractServerPacket {
    ...
    protected void handle(EntityPlayerMP player) {
        ...
        if (isFragment) {
            ...
            if (fragment != null && compendiumData.getPoints() >= fragment.getPrice()
                    && compendiumData.tryUnlockFragment(fragment))
                points = fragment.getPrice();
        } else {
            KnowledgeObject<?> object = KnowledgeObject.getObjectById(id);
            if (object != null && compendiumData.getPoints() >= object.getUnlockPrice()
                    && compendiumData.tryDiscoverObject(object, false))
