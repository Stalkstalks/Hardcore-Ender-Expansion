...
public class S04TransportBeaconTravel extends AbstractServerPacket {
    ...
    protected void handle(EntityPlayerMP player) {
        ...
        if (tile instanceof TileEntityTransportBeacon
                && MathUtil.distance(tileX + 0.5D - player.posX, tileY + 0.5D - player.posY, tileZ + 0.5D - player.posZ)
                        <= 8D) {
            PlayerTransportBeacons data = PlayerTransportBeacons.getInstance(player);
            if (data.checkBeacon(targetX, targetZ, tileX, tileZ))
                ((TileEntityTransportBeacon) tile).teleportPlayer(player, targetX, targetZ, data);
        }
    }
}
