...
public class C11ParticleAltarOrb extends AbstractClientPacket {
    ...
    public C11ParticleAltarOrb(double startX, double startY, double startZ, double targetX, double targetY,
            double targetZ, byte essenceId, byte amount) {
        this.startX = startX;
        this.startY = startY;
        this.startZ = startZ;
        this.targetX = targetX;
    ...
    public C11ParticleAltarOrb(double startX, double startY, double startZ, double targetX, double targetY,
            double targetZ, byte essenceId, byte amount, float width, float height) {
    ...
    protected void handle(EntityClientPlayerMP player) {
        ...
        for (int a = 0; a < amount; a++) {
            HardcoreEnderExpansion.fx.altarOrb(
                    player.worldObj,
                    startX + width * (rand.nextFloat() - rand.nextFloat()),
                    startY + height * rand.nextFloat(),
                    startZ + width * (rand.nextFloat() - rand.nextFloat()),
                    targetX,
                    targetY,
                    targetZ,
                    essence);
