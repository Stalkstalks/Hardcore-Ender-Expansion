...
public class GuiAchievementsCustom extends GuiAchievements {
    ...
    public void initGui() {
        ...
        buttonList.add(
                new GuiButton(
                        ...
                        125,
                        20,
                        I18n.format("achievement.viewChallenges")));
    }

    private void loadPage() {
        String str = screenType == 0 ? AchievementManager.achievementScreenName
                : AchievementManager.challengeScreenName;
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 10 && isLoaded) {
            ...
            button.displayString = I18n
                    .format(screenType == 0 ? "achievement.viewChallenges" : "achievement.viewAchievements");
            loadPage();
    ...
    protected void mouseClicked(int mouseX, int mouseY, int buttonId) {
        if (buttonId == 0 && achievements != null && screenType == 0) {
            if (mouseX < width / 2 - 113 || mouseX > width / 2 + 113
                    || mouseY < height / 2 - 84
                    || mouseY > height / 2 + 68) {
            ...
            int offsetX = MathUtil.clamp(
                    MathUtil.floor(field_146567_u),
                    AchievementList.minDisplayColumn * 24 - 112,
                    AchievementList.maxDisplayColumn * 24 - 78); // OBFUSCATED viewportX
            int offsetY = MathUtil.clamp(
                    MathUtil.floor(field_146566_v),
                    AchievementList.minDisplayRow * 24 - 112,
                    AchievementList.maxDisplayRow * 24 - 78); // OBFUSCATED viewportY
            ...
            for (Achievement achievement : achievements.getAchievements()) {
                ...
                if (x >= -24 && y >= -24
                        && x <= 224F * field_146570_r
                        && y <= 155F * field_146570_r
                        && realMouseX >= x
                        && realMouseX <= x + 22
                        && realMouseY >= y
                        && realMouseY <= y + 22) {
                    KnowledgeObject<? extends IKnowledgeObjectInstance<?>> obj = ((HeeAchievement) achievement)
                            .getKnowledgeObj();
