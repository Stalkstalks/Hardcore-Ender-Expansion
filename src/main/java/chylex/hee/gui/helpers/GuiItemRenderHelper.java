...
public class GuiItemRenderHelper {
    ...
    public static void renderItemIntoGUI(TextureManager textureManager, ItemStack is, int x, int y) {
        ...
        if (is.getItemSpriteNumber() == 0 && block != null && RenderBlocks.renderItemIn3d(block.getRenderType())) {
            ...
            if (renderItem.renderWithColor)
                GL11.glColor4f((col >> 16 & 255) / 255F, (col >> 8 & 255) / 255F, (col & 255) / 255F, 1F);
    ...
    public static void drawTooltip(ITooltipRenderer gui, FontRenderer fontRendererObj) {
        ...
        int maxWidth = 0, xx = tooltipX + 12, yy = tooltipY - 12,
                height = strings.length > 1 ? 10 + (strings.length - 1) * 10 : 8;
