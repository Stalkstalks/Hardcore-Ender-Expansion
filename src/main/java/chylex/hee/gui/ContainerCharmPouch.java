...
public class ContainerCharmPouch extends Container {
    ...
    public ContainerCharmPouch(EntityPlayer player) {
        ...
        for (int a = 0; a < Math.min(charmInv.getSizeInventory(), charms.length); a++)
            charmInv.setInventorySlotContents(a, charms[a]);
        ...
        for (int a = 0; a < 3; a++) {
            for (int b = 0; b < 9; ++b)
                addSlotToContainer(new Slot(player.inventory, b + a * 9 + 9, 8 + b * 18, 99 + a * 18));
        }
    ...
    public void saveCharmPouch() {
        if (!player.worldObj.isRemote && isHoldingPouch()) ItemCharmPouch.setPouchCharms(
                player.getHeldItem(),
                new ItemStack[] { charmInv.getStackInSlot(0), charmInv.getStackInSlot(1), charmInv.getStackInSlot(2) });
    }
        ...
        if (inventory == runeInv) {
            ...
            if (runes.size() >= 3) {
                ...
                if (charm.getRight() != null)
                    runeResultInv.setInventorySlotContents(0, new ItemStack(ItemList.charm, 1, charm.getRight().id));
            }
        }
    }
    ...
    public void onContainerClosed(EntityPlayer player) {
        ...
        if (activePouch != null && activePouch.pouchID == pouchID)
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotId) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotId < 8) {
                ...
                if (!mergeItemStack(is2, 0, 3, false) && !mergeItemStack(is2, 9, inventorySlots.size(), true))
                    return null;
