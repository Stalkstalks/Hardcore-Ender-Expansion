...
public class GuiEndPowderEnhancements extends GuiContainer implements ITooltipRenderer {
    ...
    private static ResourceLocation guiResource = new ResourceLocation(
            "hardcoreenderexpansion:textures/gui/end_powder_enhancements.png");
    ...
    protected void mouseClicked(int mouseX, int mouseY, int button) {
        if (container.getSlot(0).getStack() != null) {
            for (int a = 0; a < container.enhancementSlotX.length; a++) {
                ...
                if (checkRect(mouseX - guiLeft, mouseY - guiTop, x, 37, 17, 17)
                        && !container.clientEnhancementBlocked[a]) {
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        String s = I18n.format(
                ModCommonProxy.hardcoreEnderbacon ? "container.endPowderEnhancements.bacon"
                        : "container.endPowderEnhancements");
        ...
        for (int a = 0, x, y; a < container.enhancementSlotX.length; a++) {
            ...
            if (checkRect(mouseX - guiLeft, mouseY - guiTop, x, y, 17, 17)) {
                ...
                GuiItemRenderHelper
                        .setupTooltip(mouseX - guiLeft, mouseY - guiTop, container.clientEnhancementTooltips[a]);
            }
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float renderPartialTicks, int mouseX, int mouseY) {
        ...
        if (container.containerInv.getStackInSlot(0) == null) {
            ...
            for (int a = 0; a < container.ingredientSlots.length; a++)
                container.ingredientSlots[a].xDisplayPosition = -3200;
        } else {
            int enhAmount = EnhancementHandler
                    .getEnhancementsForItem(container.containerInv.getStackInSlot(0).getItem()).size();
            ...
            if (selectedEnhancementSlot != -1) {
                SlotList slots = EnhancementHandler
                        .getEnhancementSlotsForItem(container.containerInv.getStackInSlot(0).getItem());
                int index = 0, indexPowder = 0, indexIngredient = 0,
                        w = 18 * (slots.amountPowder + slots.amountIngredient), x;

                for (Iterator<SlotType> iter = slots.iterator(); iter.hasNext();) {
                    SlotType type = iter.next();
                    if (type != SlotType.POWDER && type != SlotType.INGREDIENT)
                    ...
                    (type == SlotType.POWDER ? container.powderSlots : container.ingredientSlots)[type
                            == SlotType.POWDER ? indexPowder++ : indexIngredient++].xDisplayPosition = x + 1;
                }
            }
        }
    }
