...
public abstract class GuiAbstractTable extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (stardustTextX != -1) {
            ...
            fontRendererObj.drawStringWithShadow(
                    (table.getHoldingStardust() < stardustReq ? EnumChatFormatting.YELLOW : EnumChatFormatting.WHITE)
                            + String.valueOf(stardustReq),
                    stardustTextX,
                    stardustTextY,
                    0x404040);
    ...
    protected void drawGuiContainerBackgroundLayer(float renderPartialTicks, int mouseX, int mouseY) {
        ...
        if (energyIconX != -1 && table.hasInsufficientEnergy())
            drawTexturedModalRect(guiX + energyIconX, guiY + energyIconY, 176, 18, 10, 11);

        if (energyStorageX != -1) {
            int energy = table.getScaledStoredEnergyClient(49) - 1;
            if (energy > -1) drawTexturedModalRect(
                    guiX + energyStorageX,
                    guiY + energyStorageY + 49 - energy,
                    176,
                    39 - energy % 10,
                    16,
                    energy);
