    ...
    exact = new IRandomAmount() {
        ...
        public int generate(Random rand, int minAmount, int maxAmount) {
            ...
            preferSmaller = new IRandomAmount() {
                ...
                public int generate(Random rand, int minAmount, int maxAmount) {
                    return minAmount
                            + MathUtil.floor(rand.nextDouble() * rand.nextDouble() * (1 + maxAmount - minAmount));
                }
            },
            ...
            aroundCenter = new IRandomAmount() {
                ...
                public int generate(Random rand, int minAmount, int maxAmount) {
                    return MathUtil.clamp(
                            (int) Math.round(
                                    minAmount + (maxAmount - minAmount) * 0.5D
                                            + (rand.nextDouble() - 0.5D) * rand.nextDouble()
                                                    * (1 + maxAmount - minAmount)),
                            minAmount,
                            maxAmount);
                }
            },
            ...
            gaussian = new IRandomAmount() {
                ...
                @Override
                public int generate(Random rand, int minAmount, int maxAmount) {
                    return minAmount + (int) Math
                            .round(MathUtil.clamp(rand.nextGaussian() * 0.5D, 0D, 1D) * (maxAmount - minAmount));
                }
            };
