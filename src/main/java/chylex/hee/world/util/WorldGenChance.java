...
public final class WorldGenChance {
    ...
    linear2Incr = new WorldGenChance2() {
        ...
        public double calculate(double dist, double startDist, double endDist) {
            ...
            cubic2Incr = new WorldGenChance2() {
                ...
                @Override
                public double calculate(double dist, double startDist, double endDist) {
                    return dist < startDist ? 0D
                            : dist > endDist ? 1D : Math.pow((dist - startDist) / (endDist - startDist), 3);
                }
            },
            ...
            cubic2Decr = new WorldGenChance2() {
                ...
                @Override
                public double calculate(double dist, double startDist, double endDist) {
                    return dist < startDist || dist > endDist ? 0D
                            : 1D - Math.pow((dist - startDist) / (endDist - startDist), 3);
                }
            };
    ...
    linear3IncrDecr = new WorldGenChance3() {
        ...
        public double calculate(double dist, double startDist, double middleDist, double endDist) {
            return dist < startDist || dist > endDist ? 0D
                    : dist < middleDist ? linear2Incr.calculate(dist, startDist, middleDist)
                            : linear2Decr.calculate(dist, middleDist, endDist);
        }
    };
