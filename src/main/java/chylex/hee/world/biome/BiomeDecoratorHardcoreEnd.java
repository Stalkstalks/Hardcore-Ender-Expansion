...
/*
 * WorldGen layer specifications ============================= - single = happens once in a range - constant = same
 * chance for all ranges - linear+/- = chance increases/decreases linearly in a range - cubic+/- = chance
 * increases/decreases using cubic easing function in a range Chance ranges ============= a- => constant - generation
 * starts happening at a point a-b => single - generation happens once in that range => linear & cubic - generation
 * chance moves inside the range - the chance always equals 0 on the left side, and then moves from 0-1 (increasing) or
 * 1-0 (decreasing) a/b-c => linear & cubic - generation chance moves inside the b-c range, but the element starts
 * spawning sooner a-b-c => linear & cubic - generation chance moves from 0-1 and then from 1-0 - chance can be also
 * modified to stop worldgen elements from disappearing completely Element list ============ Dragon Lair | single |
 * 0-120 | End Powder Ore | constant | 0- | Endstone Blob | constant | 100- | some types spawn a bit further Energy
 * Cluster | cubic+ | 320-6400 | x != 0 => x*0.45+0.12 Energy Cluster (2) | linear+ | 320-6400 | x != 0 => x*0.8+0.1 ...
 * chance to continue spawning Dungeon Tower | cubic- | 350/900-3800 | x != 0 => x*0.75+0.25 Endium Ore | cubic+ |
 * 500-12000 | x != 0 => x*0.6+0.34 Endium Ore (2) | linear+ | 1500-21000 | max amount of blocks spawned Meteoroid |
 * linear+- | 1280-2700-15000 | Biome Island | linear+ | 1600-4000 | x != 0 => x*0.35+0.65 Insidious Sanctuary |
 * linear+- | 3200-8500-36000 | x != 0 => x*0.8+0.2 ... starts at 4200, but the bound is lower for higher chance
 */
...
public class BiomeDecoratorHardcoreEnd extends BiomeEndDecorator {
    ...
    protected void genDecorations(BiomeGenBase biome) {
        ...
        if (distFromCenter > 320D
                && checkChance(0.12D + 0.45D * WorldGenChance.cubic2Incr.calculate(distFromCenter, 320D, 6400D))
                && randomGenerator.nextDouble() * randomGenerator.nextDouble() > 0.65D) {
            ...
            for (int a = 0; a < randomGenerator.nextInt(4); a++) {
                ...
                if (!checkChance(0.1D + 0.8D * WorldGenChance.linear2Incr.calculate(distFromCenter, 320D, 6400D)))
                    break;
            }
        ...
        if (distFromCenter > 500D
                && checkChance(0.34D + 0.6D * WorldGenChance.cubic2Incr.calculate(distFromCenter, 500D, 12000D))) {
            Stopwatch.timeAverage("WorldGenEndiumOre", 64);

            for (int attempt = 0, max = 1 + randomGenerator.nextInt(
                    1 + randomGenerator.nextInt(
                            2 + MathUtil.ceil(
                                    9D * WorldGenChance.linear2Incr.calculate(distFromCenter, 1500D, 21000D)))); attempt
                                            < 440; attempt++) {
        ...
        if (distFromCenter > 1280D
                && checkChance(WorldGenChance.linear3IncrDecr.calculate(distFromCenter, 1280D, 2700D, 15000D))
                && randomGenerator.nextFloat() * randomGenerator.nextFloat() > 0.666F) {
            Stopwatch.timeAverage("WorldGenMeteoroid", 64);
            for (int attempt = 0; attempt < randomGenerator.nextInt(3); attempt++)
                tryGenerate(meteoroidGen, randX(), 8 + randomGenerator.nextInt(112), randZ());
    ...
    private boolean tryGenerate(WorldGenerator generator, int x, int y, int z) {
        try {
            ...
            Log.warn(
                    "Failed generating " + generator.getClass().getSimpleName()
                            + " at "
                            + (chunk_X + x)
                            + ","
                            + (chunk_Z + z)
                            + ", there might be an empty chunk.");
            return false;
        }
    }
}
