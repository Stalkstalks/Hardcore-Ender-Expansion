...
public class WorldGenMeteoroid extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        float fillFactor = 0.2F + rand.nextFloat() * 0.35F,
                stardustChance = 0.12F + rand.nextFloat() * rand.nextFloat() * 0.15F;

        for (int attempt = 0, maxAttempts = 20 + (int) (rad + rad * rad * 5), block,
                lineBlocks = MathUtil.ceil(rad / 0.9D); attempt < maxAttempts; attempt++) {
            dx = x + 0.5D;
            dy = y + 0.5D;
            dz = z + 0.5D;
            ...
            for (block = 0; block < lineBlocks; block++) {
                ...
                if (canPlaceAt(world, xx, yy, zz) && MathUtil.distance(xx - x, yy - y, zz - z) <= rad) {
                    BlockPosM.tmp(xx, yy, zz)
                            .setBlock(world, BlockList.sphalerite, rand.nextFloat() < stardustChance ? 1 : 0, 2);
                }
            }
        }
