...
public class WorldGenEndiumOre extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int check = 0; check < 25; check++) {
            if (pos.getBlock(world) == Blocks.end_stone
                    && (tmpPos.set(x - 1, y, z).isAir(world) || tmpPos.set(x + 1, y, z).isAir(world)
                            || tmpPos.set(x, y, z - 1).isAir(world)
                            || tmpPos.set(x, y, z + 1).isAir(world)
                            || (check > 15 && rand.nextInt(3) == 0))) {
