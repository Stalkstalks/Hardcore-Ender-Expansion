        ...
        if (tmpPos.set(x, y, z).isAir(world) && tmpPos.moveDown().getBlock(world) == Blocks.end_stone) {
            ...
            for (int xx = x - radius; xx <= x + radius; ++xx) {
                for (int zz = z - radius; zz <= z + radius; ++zz) {
                    if (MathUtil.square(xx - x) + MathUtil.square(zz - z) <= radius * radius + 1
                            && tmpPos.set(xx, y - 1, zz).getBlock(world) != Blocks.end_stone) {
                        return false;
                    }
                }
            }
            ...
            crystal.setCrystalType(
                    bars ? EntityBlockEnderCrystal.BARS
                            : (rand.nextInt(4) != 0 && height * radius * radius * Math.PI < 320D)
                                    ? EntityBlockEnderCrystal.BLAST
                                    : EntityBlockEnderCrystal.TNT);

            world.spawnEntityInWorld(crystal);
            tmpPos.set(x, y + height, z).setBlock(world, Blocks.bedrock, 0, 2);

            crystal.setCrystalKey(
                    WorldDataHandler.<DragonSavefile>get(DragonSavefile.class).addCrystal(x, y + height, z));
