...
public final class DecoratorFeatureGenerator {
    ...
    public void generate(World world, Random rand, int centerX, int centerY, int centerZ) {
        ...
        try {
            for (GeneratedBlock block : blocks.valueCollection()) {
                if (!block.block.canBlockStay(world, centerX + block.x, centerY + block.y, centerZ + block.z))
                    delayed.add(block);
                else world.setBlock(
                        centerX + block.x,
                        centerY + block.y,
            ...
            if (!delayed.isEmpty()) {
                for (GeneratedBlock block : delayed) world.setBlock(
                        centerX + block.x,
                        centerY + block.y,
                        centerZ + block.z,
                        block.block,
            ...
            if (!tileEntities.isEmpty()) {
                for (Entry<BlockPosM, ITileEntityGenerator> entry : tileEntities.entrySet()) {
                    ...
                    entry.getValue().onTileEntityRequested(
                            "",
                            world.getTileEntity(centerX + loc.x, centerY + loc.y, centerZ + loc.z),
                            rand);
                }
            }
