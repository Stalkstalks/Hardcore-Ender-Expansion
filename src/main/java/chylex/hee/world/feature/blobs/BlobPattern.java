...
public final class BlobPattern implements IWeightProvider {
    ...
    public BlobPattern setPopulatorAmountProvider(IRandomAmount populatorAmountGen, int minPopulatorAmount,
            int maxPopulatorAmount) {
    ...
    public Pair<BlobGenerator, List<BlobPopulator>> generatePattern(Random rand) {
        ...
        if (populatorAmountGen != null && !populators.isEmpty()) {
            ...
            for (int a = 0,
                    amount = populatorAmountGen.generate(rand, minPopulatorAmount, maxPopulatorAmount); a < amount
                            && !blobPopulators.isEmpty(); a++) {
