...
public class BlobGeneratorRecursive extends BlobGenerator {
    ...
    private IRandomAmount baseAmountGen = IRandomAmount.exact, totalAmountGen = IRandomAmount.exact,
            recursionAmountGen = IRandomAmount.exact;
    private byte minAmount, maxAmount, minTotalAmountLimit, maxTotalAmountLimit, minRecursionAmount, maxRecursionAmount,
            maxRecursion;
    private double minRad, maxRad, minRecursionChance, maxRecursionChance, recursionChanceMp, minBlobDistMp,
            maxBlobDistMp;
    ...
    public BlobGeneratorRecursive totalAmount(IRandomAmount totalAmountGen, int minTotalAmountLimit,
            int maxTotalAmountLimit) {
    ...
    public BlobGeneratorRecursive recursionAmount(IRandomAmount recursionAmountGen, int minRecursionAmount,
            int maxRecursionAmount) {
    ...
    public BlobGeneratorRecursive recursionChance(double minRecursionChance, double maxRecursionChance,
            double recursionChanceMp, int maxRecursion) {
    ...
    private void genNewBlob(DecoratorFeatureGenerator gen, Random rand, double x, double y, double z, double rad,
            int recursionLevel) {
        ...
        if (rand.nextDouble() < tmpRecursionChance * (Math.pow(recursionChanceMp, recursionLevel))) {
            for (int a = 0, amount = recursionAmountGen.generate(rand, minRecursionAmount, maxRecursionAmount); a
                    < amount; a++) {
                ...
                genNewBlob(
                        ...
                        y + dir.yCoord * dist,
                        z + dir.zCoord * dist,
                        minRad + rand.nextDouble() * (maxRad - minRad),
                        recursionLevel + 1);
            }
        }

        if (!recursionChanceCached)
            tmpRecursionChance = minRecursionChance + rand.nextDouble() * (maxRecursionChance - minRecursionChance);
    }
}
