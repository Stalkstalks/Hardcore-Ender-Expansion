...
public class BlobPopulatorLiquidFall extends BlobPopulator {
    ...
    public void generate(DecoratorFeatureGenerator gen, Random rand) {
        ...
        for (int attempts = minAttempts + rand.nextInt(maxAttempts - minAttempts + 1), xx, yy,
                zz; attempts > 0 && amount > 0 && !blocks.isEmpty(); attempts--) {
            BlockPosM loc = blocks.remove(rand.nextInt(blocks.size()));
            if (gen.getBlock(loc.x, loc.y, loc.z) != Blocks.end_stone
                    || gen.getBlock(loc.x, loc.y - 1, loc.z) != Blocks.end_stone)
                continue;
