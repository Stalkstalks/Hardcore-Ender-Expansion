...
public class WorldLoot implements IVillageTradeHandler {
    ...
    public static void registerWorldLoot() {
        ...
        getInfo(MINESHAFT_CORRIDOR)
                .addItem(new WeightedRandomChestContent(new ItemStack(ItemList.temple_caller), 0, 1, 1));

        item = new WeightedRandomChestContent(new ItemStack(ItemList.temple_caller), 0, 1, 1);
        for (String s : new String[] { DUNGEON_CHEST, PYRAMID_DESERT_CHEST, PYRAMID_JUNGLE_CHEST, VILLAGE_BLACKSMITH,
                STRONGHOLD_CORRIDOR, STRONGHOLD_CROSSING })
            getInfo(s).addItem(item);

        getInfo(STRONGHOLD_LIBRARY)
                .addItem(new WeightedRandomChestContent(new ItemStack(ItemList.temple_caller), 1, 1, 1));
        ...
        for (String s : new String[] { STRONGHOLD_CORRIDOR, STRONGHOLD_CROSSING, STRONGHOLD_LIBRARY })
            getInfo(s).addItem(item);
    ...
    public void manipulateTradesForVillager(EntityVillager villager, MerchantRecipeList recipeList, Random rand) {
        if (villager.getProfession() == 1) {
            if (rand.nextFloat() < 0.65F) recipeList.add(
                    new MerchantRecipe(
                            new ItemStack(Items.emerald, 4 + rand.nextInt(4)),
                            ItemKnowledgeNote.setRandomNote(new ItemStack(ItemList.knowledge_note), rand, 2)));
            else if (rand.nextFloat() < 0.6F) recipeList
                    .add(new MerchantRecipe(new ItemStack(Items.emerald, 3 + rand.nextInt(3), 0), lorePage.copy()));
            else if (rand.nextFloat() < 0.3F) recipeList.add(
                    new MerchantRecipe(
                            new ItemStack(Items.emerald, 11 + rand.nextInt(5), 0),
