    ...
    protected final boolean canSpawnStructureAtCoords(int x, int z) {
        ...
        if (origChunkX == x2 && origChunkZ == z2) {
            double dist = Math.sqrt(
                    MathUtil.square(x2 * 16L + (featureSize >> 1)) + MathUtil.square(z2 * 16L + (featureSize >> 1)));
        ...
        public void run(String... args) {
            ...
            for (int x = -48; x <= 48; x++) {
                for (int z = -48; z <= 48; z++) {
                    ...
                    if (origChunkX == x2 && origChunkZ == z2 && rand.nextInt(3) == 0)
                        world.setBlock(px + x, py - 1, pz + z, Blocks.brick_block);
                }
            }
        }
    };
