...
public class MapGenSanctuary extends MapGenScatteredFeatureCustom {
    ...
    protected boolean canStructureSpawn(int x, int z, double dist, Random rand) {
        return rand.nextInt(5) <= 1 && WorldGenChance
                .checkChance(0.2D + 0.8D * WorldGenChance.linear3IncrDecr.calculate(dist, 3200D, 8500D, 36000D), rand);
    }
    ...
    public static final HeeTest $debugTest = new HeeTest() {
        ...
        public void run(String... args) {
            ComponentSanctuary sanctuary = new ComponentSanctuary(
                    world,
                    world.rand,
                    (int) player.posX,
                    (int) player.posZ);
            sanctuary.addComponentParts(
                    world,
                    world.rand,
                    new StructureBoundingBox(-9999999, 0, -9999999, 9999999, 256, 9999999));
