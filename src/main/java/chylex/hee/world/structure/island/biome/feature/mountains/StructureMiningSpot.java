...
public class StructureMiningSpot extends AbstractIslandStructure {
    ...
    protected boolean generate(Random rand) {
        ...
        int x = getRandomXZ(rand, 32), z = getRandomXZ(rand, 32),
                y = 15 - rand.nextInt(2) * rand.nextInt(14) + rand.nextInt(15 + rand.nextInt(35));
    ...
    private void generateBlob(Random rand, int x, int y, int z, double rad, int recursionLevel) {
        if (x <= rad || z <= rad || x >= ComponentIsland.size - rad || z >= ComponentIsland.size - rad || y <= rad)
            return;
        if (iterationsLeft == 0 || --iterationsLeft == 0
                || recursionLevel > 35
                || (recursionLevel > 12 && rand.nextInt(60 - Math.min(recursionLevel * 2, 50)) == 0))
            return;
        ...
        for (xx = MathUtil.floor(x - rad) - 1; xx <= x + rad + 1; xx++) {
            for (yy = MathUtil.floor(y - rad) - 1; yy <= y + rad + 1; yy++) {
                ...
                for (zz = MathUtil.floor(z - rad) - 1; zz <= z + rad + 1; zz++) {
                    ...
                    if (world.getBlock(xx, yy, zz) == Blocks.end_stone && rand.nextInt(7) <= 1
                            && dist <= rad - rand.nextDouble() * 0.3D) {
        ...
        if (rand.nextInt(6 + recursionLevel * 3) == 0) generateBlob(
                rand,
    ...
    private void placeBlock(Random rand, int x, int y, int z, double distPercent) {
        if (rand.nextBoolean() && rand.nextBoolean()
                && rand.nextDouble() > distPercent - 0.2D - rand.nextDouble() * 0.25D)
            world.setBlock(x, y, z, oreWeights.getRandomItem(rand).getObject());
