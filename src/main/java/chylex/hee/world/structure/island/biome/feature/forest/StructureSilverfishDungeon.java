...
public class StructureSilverfishDungeon extends AbstractIslandStructure implements ITileEntityGenerator {
    ...
    public static WeightedLootList lootDungeon = new WeightedLootList(
            new LootItemStack[] { new LootItemStack(Items.paper).setAmount(1, 11).setWeight(36),
                    ...
                    new LootItemStack(Items.sugar).setAmount(1, 2).setWeight(3), new LootItemStack(ItemList.music_disk)
                            .setDamage(0, ItemMusicDisk.getRecordCount() - 1).setWeight(2) })
                                    .addItemPostProcessor(new IItemPostProcessor() {
                                        ...
                                        public ItemStack processItem(ItemStack is, Random rand) {
                                            if (is.getItemUseAction() == EnumAction.eat) {
                                                ...
                                                if (rand.nextInt(4) != 0) EnchantmentHelper
                                                        .addRandomEnchantment(rand, is, 16 + rand.nextInt(5));
    ...
    protected boolean generate(Random rand) {
        for (int x = getRandomXZ(rand, 8), z = getRandomXZ(rand, 8), y, attempt = 0; attempt < 300; attempt++) {
            ...
            if (world.isAir(x - 2, y, z - 2) || world.isAir(x - 2, y, z + 2)
                    || world.isAir(x + 2, y, z - 2)
                    || world.isAir(x + 2, y, z + 2))
                continue;

            // Cave generation

            double rx = 3.5D + rand.nextDouble() * 1.5D, ry = 2.4D + rand.nextDouble() * 0.6D,
                    rz = 3.5D + rand.nextDouble() * 1.5D;
            int lowestY = 999;

            for (int ix = (int) (x - rx - 1); ix <= x + rx + 1; ix++) {
                for (int iy = (int) (y - ry - 1); iy <= y + ry + 1; iy++) {
                    for (int iz = (int) (z - rz - 1); iz <= z + rz + 1; iz++) {
                        if (MathUtil.square(x - ix) / (rx * rx) + MathUtil.square(y - iy) / (ry * ry)
                                + MathUtil.square(z - iz) / (rz * rz) <= 0.9D + rand.nextDouble() * 0.2D) { // add in
                                                                                                            // ellipsoid
                                                                                                            // stuff
            ...
            for (int a = 0, attempts = 70 + (int) (rand.nextDouble() * rand.nextDouble() * 75D); a < attempts; a++) {
                int ix = x + rand.nextInt(11) - 5, iy = y + 1 - rand.nextInt(5), iz = z + rand.nextInt(11) - 5;
            ...
            if (rand.nextInt(5) <= 1) {
                for (int nextSpawnerAttempt = 0, spawnerX, spawnerY, spawnerZ; nextSpawnerAttempt
                        < 10; nextSpawnerAttempt++) {
                    spawnerX = x + rand.nextInt(11) - 5;
                    spawnerY = y + rand.nextInt(7) - 5;
                    spawnerZ = z + rand.nextInt(11) - 5;

                    if (world.isAir(spawnerX, spawnerY, spawnerZ) && world.isAir(spawnerX, spawnerY + 1, spawnerZ)
                            && !world.isAir(spawnerX, spawnerY - 1, spawnerZ)) {
            ...
            for (int chestAttempt = 0; chestAttempt < 40; chestAttempt++) {
                ...
                if (world.isAir(chestX, chestY, chestZ) && world.isAir(chestX, chestY + 1, chestZ)
                        && !world.isAir(chestX, chestY - 1, chestZ)) {
                    placedChest = true;
                    break;
                }
            }
