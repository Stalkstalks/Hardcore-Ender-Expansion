...
public class BiomeDecoratorEnchantedIsland extends IslandBiomeDecorator {
    ...
    public void genHomeland() {
        ...
        for (int attempt = 0, placed = 0, amount = rand.nextInt(3) + 5; attempt < (swamp ? 450 : 170)
                && placed < (swamp ? amount * 3 : amount); attempt++) {
        ...
        for (int cx = 0; cx < world.getChunkAmountX(); cx++) {
            for (int cz = 0; cz < world.getChunkAmountZ(); cz++) {
                if (rand.nextInt(5) <= 2) {
                    int height = rand.nextInt(14)
                            + (data.hasDeviation(IslandBiomeEnchantedIsland.TALL_PILES) ? 6 + rand.nextInt(12) : 4);
                    int radius = rand.nextInt(2) + 1;

                    int ox = cx * 16 + rand.nextInt(16), oz = cz * 16 + rand.nextInt(16),
                            ...
                            for (int xx = ox - radius; xx <= ox + radius; ++xx) {
                                for (int zz = oz - radius; zz <= oz + radius; ++zz) {
                                    for (int yy = world.getHighestY(xx, zz) + 1; yy < oy + height && yy < 128; ++yy) {
                                        if (MathUtil.square(xx - ox) + MathUtil.square(zz - oz)
                                                <= radius * radius + 0.5D + rand.nextGaussian() * 0.7D) {
        ...
        // OBSIDIAN ROADS
        for (int attempt = 0, placed = 0, placedMax = 8 + rand.nextInt(5); attempt < 36
                && placed < placedMax; attempt++) {
            if (generateStructure(genRoads)) ++placed;
        }

        // SHADOW ORCHID
        for (int attempt = data.hasDeviation(IslandBiomeEnchantedIsland.MORE_SHADOW_ORCHIDS) ? 750 : 450; attempt
                > 0; attempt--) {
            if (generateStructure(genShadowOrchid)) ++attempt;
        }

        // HOMELAND CACHE
        for (int attempt = 280 + rand.nextInt(42), placed = 14 + rand.nextInt(10), xx, yy, zz; attempt > 0
                && placed > 0; attempt--) {
        ...
        for (int spawnAttempt = 0, spawnedTotal = 52 + rand.nextInt(28) + rand.nextInt(16); spawnAttempt
                < spawnedTotal; spawnAttempt++) {
        ...
        for (int groupLeaders = rand.nextInt(3 + rand.nextInt(3) * rand.nextInt(2)); groupLeaders > 0
                && size > 0; groupLeaders--) {
            long groupId = endermanList.remove(rand.nextInt(size--)).setNewGroupLeader();
            if (groupId == -1) continue;

            for (int state = rand.nextBoolean() ? rand.nextInt(1 + rand.nextInt(4 + rand.nextInt(8))) : 0; state > 0
                    && size > 0; state--) {
                endermanList.remove(rand.nextInt(size--))
                        .setGroupMember(groupId, OvertakeGroupRole.getRandomMember(rand));
            }
        }
    }
    ...
    public void genLaboratory() {
        ...
        // VIOLET MOSS
        for (int amount = 30 + rand.nextInt(6) + rand.nextInt(8), attempt, yAttempt, xx, yy, zz, tmpY, clumps,
                placeAttempt, px, py, pz, placed = 0; amount > 0; amount--) {
            for (attempt = 0; attempt < 6; attempt++) {
                xx = rand.nextInt(ComponentIsland.size - 30) + 15;
                zz = rand.nextInt(ComponentIsland.size - 30) + 15;
                if ((yy = world.getHighestY(xx, zz)) == 0 || world.getBlock(xx, yy, zz) != BlockList.end_terrain)
                    continue;

                for (yAttempt = 0; yAttempt < 7; yAttempt++) {
                    tmpY = yy - rand.nextInt(20);

                    if (tmpY > 0 && world.getBlock(xx, tmpY, zz) == BlockList.end_terrain
                            && world.isAir(xx, tmpY + 1, zz)) {
                        yy = tmpY;
                        break;
                    }
                }
                        ...
                        for (yAttempt = 0; yAttempt < 5; yAttempt++) {
                            if (world.getBlock(px, py - 1, pz) == BlockList.end_terrain && world.isAir(px, py, pz)) {
                                world.setBlock(
                                        ...
                                        rand.nextInt(4) == 0 ? BlockCrossedDecoration.dataVioletMossTall
                                                : rand.nextInt(3) == 0 ? BlockCrossedDecoration.dataVioletMossModerate
                                                        : BlockCrossedDecoration.dataVioletMossShort);
                                ++placed;
                                break;
                            } else if (--py <= 0) break;
                        }
        ...
        // SHADOW ORCHID
        for (int attempt = data.hasDeviation(IslandBiomeEnchantedIsland.MORE_SHADOW_ORCHIDS) ? 550 : 350; attempt
                > 0; attempt--) {
            if (generateStructure(genShadowOrchid)) ++attempt;
        }
    }
