...
public class BiomeInteractionEnchantedIsland {
    ...
    public static class InteractionOvertake extends AbstractBiomeInteraction {
        ...
        @Override
        public void init() {
            List<EntityMobHomelandEnderman> endermen = world
                    .getEntitiesWithinAABB(EntityMobHomelandEnderman.class, getIslandBoundingBox());
            ...
            for (int attempt = 0; attempt < 3 && !endermen.isEmpty(); attempt++) {
                ...
                if (subject.getGroupId() != -1L) {
                    ...
                    if (roles.isEmpty() && rand.nextInt(666) < MathUtil.square(sameGroup.size())
                            && rand.nextInt(3) == 0) {
    ...
    public static class InteractionCellarSounds extends AbstractBiomeInteraction {
        ...
        public void init() {
            ...
            List<EntityTechnicalBiomeInteraction> interactions = world.getEntitiesWithinAABB(
                    EntityTechnicalBiomeInteraction.class,
                    entity.boundingBox.expand(1D, 1D, 1D));
            ...
            for (int attempt = 0; attempt <= 10; attempt++) {
                ...
                for (EntityTechnicalBiomeInteraction interaction : interactions) {
                    if (interaction != entity && interaction.getInteractionType() == InteractionCellarSounds.class
                            && ((InteractionCellarSounds) interaction.getInteraction()).target == target) {
                        target = null;
                        break;
                    }
                }
        ...
        public void update() {
            ...
            if (++cellarCheckTimer > 10) {
                ...
                int tx = (int) target.posX, tz = (int) target.posZ, minPersegritY = (int) target.posY,
                        maxPersegritY = minPersegritY + 1;
            ...
            if (--timer < 0) {
                switch (procedure) {
                    case FOOTSTEPS:
                        ...
                        play(
                                C08PlaySound.PERSEGRIT_FOOTSTEPS,
                                BlockList.persegrit.stepSound.getVolume() * 0.15F,
                                BlockList.persegrit.stepSound.getPitch() * 1F);
                        break;

                    case BLOCK_BREAKING:
                        timer = 7 + rand.nextInt(8 + rand.nextInt(15));
                        if (lastSoundId == 0 || rand.nextInt(8) == 0)
                            lastSoundId = (byte) (1 + rand.nextInt(BreakEffects.values.length));

                        play(
                                (byte) (C08PlaySound.GRASS_BREAK - 1 + lastSoundId),
                                lastSoundId == BreakEffects.WOOD.ordinal() + 1 ? 0.5F : 0.25F,
                                BlockList.persegrit.stepSound.getPitch() * 0.8F);
                        break;
                    ...
                    case MOB_KILLING:
                        ...
                        play(
                                (byte) (C08PlaySound.PIG_HURT + 2 * (lastSoundId - 1) + (rand.nextInt(6) == 0 ? 1 : 0)),
                                lastSoundId == MobEffects.COW.ordinal() + 1 ? 0.4F : 1F,
                                1F + (rand.nextFloat() - rand.nextFloat()) * 0.2F);
                        break;
                }
        ...
        private void play(byte soundId, float volume, float pitch) {
            PacketPipeline.sendToPlayer(
                    ...
                    new C08PlaySound(
                            soundId,
                            target.posX + MathHelper.cos(soundAngle) * soundDist,
                            target.posY,
                            target.posZ + MathHelper.sin(soundAngle) * soundDist,
                            volume,
                            pitch));
        }
