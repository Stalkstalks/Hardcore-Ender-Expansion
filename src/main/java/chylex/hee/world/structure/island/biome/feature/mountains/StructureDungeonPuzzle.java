...
public class StructureDungeonPuzzle extends AbstractIslandStructure {
    ...
    private static final byte[] checkX = new byte[] { -1, -1, 1, 1 }, checkZ = new byte[] { -1, 1, -1, 1 };

    @Override
    protected boolean generate(Random rand) {
        int xSize = BlockDungeonPuzzle.minDungeonSize
                + 2 * rand.nextInt(1 + ((BlockDungeonPuzzle.maxDungeonSize - BlockDungeonPuzzle.minDungeonSize) >> 1)),
                zSize = BlockDungeonPuzzle.minDungeonSize + 2 * rand
                        .nextInt(1 + ((BlockDungeonPuzzle.maxDungeonSize - BlockDungeonPuzzle.minDungeonSize) >> 1));

        if (xSize == BlockDungeonPuzzle.minDungeonSize) xSize = BlockDungeonPuzzle.minDungeonSize
                + 2 * rand.nextInt(1 + ((BlockDungeonPuzzle.maxDungeonSize - BlockDungeonPuzzle.minDungeonSize) >> 1));
        if (zSize == BlockDungeonPuzzle.minDungeonSize) zSize = BlockDungeonPuzzle.minDungeonSize
                + 2 * rand.nextInt(1 + ((BlockDungeonPuzzle.maxDungeonSize - BlockDungeonPuzzle.minDungeonSize) >> 1));

        int checkMpX = 1 + (xSize >> 1), checkMpZ = 1 + (zSize >> 1), distX = xSize >> 1, distZ = zSize >> 1;
        ...
        for (int attempt = 0, xx, yy, zz; attempt < 500; attempt++) {
            ...
            for (int a = 0; a < 4; a++) {
                if (world.isAir(xx + checkX[a] * checkMpX, yy - 1, zz + checkZ[a] * checkMpZ)
                        || world.isAir(xx + checkX[a] * checkMpX, yy + 6, zz + checkZ[a] * checkMpZ)) {
                    canGenerate = false;
                    break;
                }
            }
            ...
            for (int posY = yy; posY < yy + 5; posY++) {
                ...
                for (int posX = xx - distX; posX <= xx + distX; posX++) {
                    world.setBlock(
                            ...
                            posY,
                            zz - distZ,
                            BlockList.dungeon_puzzle,
                            genRock && rand.nextInt(24) == 0 ? BlockDungeonPuzzle.metaRock
                                    : BlockDungeonPuzzle.metaWall);
                    world.setBlock(
                            posX,
                            posY,
                            zz + distZ,
            ...
            for (int posX = xx - distX + 1, meta; posX <= xx + distX - 1; posX++) {
                for (int posZ = zz - distZ + 1; posZ <= zz + distZ - 1; posZ++) {
                    ...
                    if (posX == xx - distX + 1 || posX == xx + distX - 1
                            || posZ == zz - distZ + 1
                            || posZ == zz + distZ - 1)
                        meta = pickBlock(BlockDungeonPuzzle.metaChainedUnlit, rand);
            ...
            for (int a = 0, amt = 3 + rand.nextInt(5), posX, posZ; a < amt + rand.nextInt(4 + rand.nextInt(10)); a++) {
                ...
                world.setBlock(
                        ...
                        posZ,
                        BlockList.dungeon_puzzle,
                        pickBlock(BlockDungeonPuzzle.metaTriggerUnlit, rand));
            }

            boolean[] foundTriggerable = new boolean[4];

            for (int posX = xx - distX + 1; posX <= xx + distX - 1; posX++) {
                if (BlockDungeonPuzzle.getUnlit(world.getMetadata(posX, yy, zz - distZ + 1))
                        == BlockDungeonPuzzle.metaTriggerUnlit)
            ...
            for (int posZ = zz - distZ + 1; posZ <= zz + distZ - 1; posZ++) {
                if (BlockDungeonPuzzle.getUnlit(world.getMetadata(xx - distX + 1, yy, posZ))
                        == BlockDungeonPuzzle.metaTriggerUnlit)
            ...
            if (!foundTriggerable[0]) world.setBlock(
                    xx + rand.nextInt(distX * 2 + 1) - distX,
                    yy,
                    zz - distZ + 1,
                    BlockList.dungeon_puzzle,
                    pickBlock(BlockDungeonPuzzle.metaTriggerUnlit, rand));
            if (!foundTriggerable[1]) world.setBlock(
                    xx + rand.nextInt(distX * 2 + 1) - distX,
                    yy,
                    zz + distZ - 1,
                    BlockList.dungeon_puzzle,
                    pickBlock(BlockDungeonPuzzle.metaTriggerUnlit, rand));
            if (!foundTriggerable[2]) world.setBlock(
                    xx - distX + 1,
                    yy,
                    zz + rand.nextInt(distZ * 2 + 1) - distZ,
                    BlockList.dungeon_puzzle,
                    pickBlock(BlockDungeonPuzzle.metaTriggerUnlit, rand));
            if (!foundTriggerable[3]) world.setBlock(
                    xx + distX - 1,
                    yy,
                    zz + rand.nextInt(distZ * 2 + 1) - distZ,
                    BlockList.dungeon_puzzle,
            ...
            for (int amt = 3 + rand.nextInt(4 + rand.nextInt(xSize * zSize > 90 ? 5 : 3)), distrAttempt = amt * 3, posX,
                    posZ, meta, type; distrAttempt > 0 && amt > 0; distrAttempt--) {
                posX = xx + rand.nextInt(distX * 2 + 1) - distX;
                posZ = zz + rand.nextInt(distZ * 2 + 1) - distZ;
                meta = BlockDungeonPuzzle.getUnlit(world.getMetadata(posX, yy, posZ));
                type = rand.nextInt(2); // 0 = spread, 1 = square

                if (meta == BlockDungeonPuzzle.metaTriggerUnlit
                        || (meta == BlockDungeonPuzzle.metaChainedUnlit && rand.nextInt(5) > 2)) {
                    boolean stop = false;

                    for (int dir = 0; dir < 4; dir++) {
                        int adjMeta = BlockDungeonPuzzle.getUnlit(
                                world.getMetadata(posX + Direction.offsetX[dir], yy, posZ + Direction.offsetZ[dir]));

                        if (adjMeta == BlockDungeonPuzzle.metaDistributorSpreadUnlit
                                || adjMeta == BlockDungeonPuzzle.metaDistributorSquareUnlit) {
                            stop = true;
                            break;
                        }
                    }
                    ...
                    if (type == 0) {
                        ...
                        for (int px = xx - distX + 1; px <= xx + distX - 1; px++) {
                            if (px != posX && BlockDungeonPuzzle.getUnlit(world.getMetadata(px, yy, posZ))
                                    == BlockDungeonPuzzle.metaDistributorSpreadUnlit) {
                                nextX = px;
                                break;
                            }
                        }
                        ...
                        for (int pz = zz - distZ + 1; pz <= zz + distZ - 1; pz++) {
                            if (pz != posZ && BlockDungeonPuzzle.getUnlit(world.getMetadata(posX, yy, zz))
                                    == BlockDungeonPuzzle.metaDistributorSpreadUnlit) {
                                nextZ = pz;
                                break;
                            }
                        ...
                        if (nextX != Integer.MIN_VALUE && nextZ != Integer.MIN_VALUE
                                && BlockDungeonPuzzle.getUnlit(world.getMetadata(nextX, yy, nextZ))
                                        == BlockDungeonPuzzle.metaDistributorSpreadUnlit) {
                            continue;
                        }
                    }
                    ...
                    world.setBlock(
                            ...
                            BlockList.dungeon_puzzle,
                            pickBlock(
                                    type == 0 ? BlockDungeonPuzzle.metaDistributorSpreadUnlit
                                            : BlockDungeonPuzzle.metaDistributorSquareUnlit,
                                    rand));
                }
            }
            ...
            for (int amt = 1 + rand.nextInt(6 + (rand.nextInt(xSize * 2 + zSize * 2) >> 3))
                    + (MathUtil.square(xSize) >> 4)
                    + (MathUtil.square(zSize) >> 4), chainAttempt = amt * 3, posX,
                    posZ; chainAttempt > 0 && amt > 0; chainAttempt--) {
                posX = xx + rand.nextInt(distX * 2 + 1) - distX;
                posZ = zz + rand.nextInt(distZ * 2 + 1) - distZ;

                if (BlockDungeonPuzzle.getUnlit(world.getMetadata(posX, yy, posZ))
                        == BlockDungeonPuzzle.metaTriggerUnlit) {
                    boolean canSpawn = true;

                    for (int dir = 0, adjacentNonTrig = 0, adjMeta; dir < 4; dir++) {
                        adjMeta = BlockDungeonPuzzle.getUnlit(
                                world.getMetadata(posX + Direction.offsetX[dir], yy, posZ + Direction.offsetZ[dir]));

                        if ((adjMeta == BlockDungeonPuzzle.metaChainedUnlit && ++adjacentNonTrig > 1)
                                || adjMeta != BlockDungeonPuzzle.metaTriggerUnlit) {
                            canSpawn = false;
                            break;
                        }
                    }
            ...
            while (true) {
                ...
                for (int px = MathUtil.floor(airX - intrad); px <= airX + intrad; px++) {
                    for (int py = MathUtil.floor(airY - intrad); py <= airY + intrad; py++) {
                        for (int pz = MathUtil.floor(airZ - intrad); pz <= airZ + intrad; pz++) {
                            if (CaveGenerator.getDistance((int) (px - airX), (int) (py - airY), (int) (pz - airZ))
                                    < rad + rand.nextFloat() * 0.35F) {
                ...
                if (MathUtil.distance(xx - airX, yy - airY, zz - airZ) < 4.8D || brokenDungBlocks > 4
                        || ++iterLimiter > 30)
                    break;
            }
    ...
    private int pickBlock(int meta, Random rand) {
        return rand.nextInt(9) < 7 ? BlockDungeonPuzzle.getUnlit(meta)
                : BlockDungeonPuzzle.toggleState(BlockDungeonPuzzle.getUnlit(meta));
    }
}
