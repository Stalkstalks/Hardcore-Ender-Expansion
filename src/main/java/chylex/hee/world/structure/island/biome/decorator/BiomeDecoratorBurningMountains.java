...
public class BiomeDecoratorBurningMountains extends IslandBiomeDecorator {
    ...
    public void genScorching() {
        // CINDER
        for (int a = data.hasDeviation(IslandBiomeBurningMountains.EXCESSIVE_CINDER) ? 110 : 62; a > 0; a--)
            generateStructure(genCinderPatch);
        ...
        // SINGLE LAVA BLOCKS
        for (int a = data.hasDeviation(IslandBiomeBurningMountains.SINGLE_LAVA_ONLY) ? 8000 : 6500, xx, yy, zz; a
                > 0; a--) {
            ...
            if (world.getBlock(xx, yy, zz) == Blocks.end_stone
                    && (world.isAir(xx + 1, yy, zz) || world.isAir(xx - 1, yy, zz)
                            || world.isAir(xx, yy, zz + 1)
                            || world.isAir(xx, yy, zz - 1))) {
        ...
        if (!data.hasDeviation(IslandBiomeBurningMountains.SINGLE_LAVA_ONLY)) {
            for (int attempt = 0, placed = 0, placedMax = 4 + rand.nextInt(10); attempt < 550
            ...
            for (int cz = 0; cz < world.getChunkAmountZ(); cz++) {
                if (rand.nextInt(4) <= 1) {
                    for (int a = 0; a < 1 + rand.nextInt(6) + rand.nextInt(5); a++) {
                        int xx = cx * 16 + rand.nextInt(16), zz = cz * 16 + rand.nextInt(16),
        ...
        for (int attempt = 0, xx, yy, zz, lava; attempt < 420; attempt++) {
            ...
            for (int lavaCheck = 0; lavaCheck < 25 && lava < 5; lavaCheck++) {
                if (world.getBlock(xx + rand.nextInt(11) - 5, yy + rand.nextInt(4), zz + rand.nextInt(11) - 5)
                ...
                for (int yAttempt = 0; yAttempt < 8; yAttempt++) {
                    ...
                    if (world.isAir(px, py, pz) && world.getBlock(px, py - 1, pz) == BlockList.end_terrain) {
                        world.setBlock(
                                ...
                                pz,
                                BlockList.crossed_decoration,
                                rand.nextInt(3) == 0 ? BlockCrossedDecoration.dataFlameweed1
                                        : rand.nextBoolean() ? BlockCrossedDecoration.dataFlameweed2
                                                : BlockCrossedDecoration.dataFlameweed3);
                        break;
                    }
                }
            }
    ...
    public void genMine() {
        // RESOURCE PIT
        for (int attempt = 0, placed = 0, placedMax = 3 + rand.nextInt(3 + rand.nextInt(4)); attempt < 12
                && placed < placedMax; attempt++) {
            if (generateStructure(genResourcePit)) ++placed;
        }

        // FLAMEWEED
        int[] flameweed = new int[] { BlockCrossedDecoration.dataFlameweed1, BlockCrossedDecoration.dataFlameweed2,
                BlockCrossedDecoration.dataFlameweed3 };
