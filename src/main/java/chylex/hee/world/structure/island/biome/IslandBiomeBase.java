...
public abstract class IslandBiomeBase {
    ...
    public static final IslandBiomeBase infestedForest = new IslandBiomeInfestedForest(0),
            burningMountains = new IslandBiomeBurningMountains(1), enchantedIsland = new IslandBiomeEnchantedIsland(2);

    public static final List<IslandBiomeBase> biomeList = CollectionUtil
            .newList(infestedForest, burningMountains, enchantedIsland);
    ...
    public final IslandBiomeData generateData(Random rand) {
        ...
        int deviationAmt = available.isEmpty() ? 0
                : MathUtil.clamp(
                        MathUtil.floor(
                                (rand.nextGaussian() + 0.4D) * available.size() * (0.5D + 0.5D * rand.nextDouble())),
                        0,
                        available.size());
    ...
    public void updateCore(World world, int x, int y, int z, int meta) {
        ...
        if (world.rand.nextInt(3) == 0) {
            ...
            SpawnEntry entry = spawnEntries.containsKey((byte) meta)
                    ? spawnEntries.get((byte) meta).getRandomItem(world.rand)
                    : null;
            if (entry == null) return;

            if (world.difficultySetting == EnumDifficulty.PEACEFUL && entry.isMob) return;

            int currentAmount = world
                    .getEntitiesWithinAABB(
                            entry.getMobClass(),
                            AxisAlignedBB.getBoundingBox(x - halfsz, y, z - halfsz, x + halfsz, y + 100, z + halfsz))
                    .size();
            if (currentAmount >= entry.getMaxAmount()
                    || world.rand.nextFloat() * 1.1F < (float) currentAmount / entry.getMaxAmount())
                return;

            int playerAmount = world.playerEntities.size();

            for (int a = 0; a < Math.min(
                    entry.getMaxAmount() >> 2,
                    world.rand.nextInt(1 + entry.getMaxAmount() - currentAmount)); a++) {
                ...
                for (int attempt = 0; attempt < 20 + Math.min(10, playerAmount); attempt++) {
                    ...
                    double ang = world.rand.nextDouble() * 2D * Math.PI,
                            len = 19D + world.rand.nextInt(55) + Math.abs(world.rand.nextGaussian() * 12D);
                    ...
                    for (int yAttempt = 0; yAttempt < 28; yAttempt++) {
                        e.setLocationAndAngles(
                                posX,
                                MathUtil.floor(player.posY + (world.rand.nextDouble() - 0.65D) * (yAttempt + 4) * 3D)
                                        + 0.01D,
                                posZ,
                    ...
                    if (!hasBlockBelow || DragonUtil.getTopBlockY(world, BlockList.end_terrain, xx, zz, 120) <= 30)
                        continue;
        ...
        if (variation != null && interactions.containsKey(variation.id) && world.rand.nextInt(5) == 0) {
            for (BiomeInteraction interaction : interactions.get(variation.id)) {
                if (world.rand.nextInt(interaction.getRNG()) == 0) {
                    List<EntityTechnicalBiomeInteraction> list = world.getEntitiesWithinAABB(
                    ...
                    for (EntityTechnicalBiomeInteraction interactionEntity : list) {
                        if (interactionEntity.getInteraction().getIdentifier().equals(interaction.getIdentifier()))
                            ++instances;
                    }

                    if (instances < interaction.getMaxInstances()) {
                        AbstractBiomeInteraction interactionInstance = interaction.create();
                        if (interactionInstance != null) world.spawnEntityInWorld(
                                new EntityTechnicalBiomeInteraction(
                                        world,
                                        x + 0.5D,
                                        y + 0.5D,
                                        z + 0.5D,
        ...
        for (EntityPlayer player : (List<EntityPlayer>) world.playerEntities) {
            if (isPlayerMoving(player) && player instanceof EntityPlayerMP) {
                ...
                if (world.getBlock(ix, iy, iz) == getTopBlock()
                        && world.getBlockMetadata(ix, iy, iz) == getTopBlockMeta()) {
                    CausatumUtils.increase(player, CausatumMeters.END_EXPLORATION, 0.6F);

                    if (!((EntityPlayerMP) player).func_147099_x().hasAchievementUnlocked(getAchievement())) { // OBFUSCATED
                                                                                                               // getStatisticsFile
    ...
    private static final boolean hasEntitySpace(World world, Entity entity) {
        return world.checkNoEntityCollision(entity.boundingBox)
                && world.getCollidingBoundingBoxes(entity, entity.boundingBox).isEmpty()
                && !world.isAnyLiquid(entity.boundingBox);
    }

    private static final boolean isPlayerMoving(EntityPlayer player) {
        return Math.abs(player.lastTickPosX - player.posX) > 0.0001D
