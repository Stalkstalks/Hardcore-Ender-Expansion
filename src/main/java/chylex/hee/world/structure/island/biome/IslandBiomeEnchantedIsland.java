...
public class IslandBiomeEnchantedIsland extends IslandBiomeBase {
    ...
    public static final BiomeRandomDeviation MORE_SHADOW_ORCHIDS = new BiomeRandomDeviation(
            "ShadowOrchids",
            HOMELAND,
            LABORATORY);
    ...
    protected IslandBiomeEnchantedIsland(int biomeID) {
        ...
        getSpawnEntries(HOMELAND).addAll(
                new SpawnEntry[] { new SpawnEntry(EntityMobEnderman.class, 22, 38),
                        new SpawnEntry(EntityMobBabyEnderman.class, 14, 20) });

        getSpawnEntries(LABORATORY).addAll(
                new SpawnEntry[] { new SpawnEntry(EntityMobEndermage.class, 7, 15),
                        new SpawnEntry(EntityMobEnderman.class, 12, 10),
                        new SpawnEntry(EntityMobEnderGuardian.class, 20, 7) });

        getInteractions(HOMELAND).addAll(
                new BiomeInteraction[] { new BiomeInteraction("EI_Homeland_Overtake", InteractionOvertake.class, 50, 1),
                        new BiomeInteraction("EI_Homeland_CellarSounds", InteractionCellarSounds.class, 10, 20) });

        getInteractions(LABORATORY).addAll(
                new BiomeInteraction[] {
    ...
    public void updateCore(World world, int x, int y, int z, int meta) {
        ...
        if (meta == HOMELAND.id && world.rand.nextInt(40) == 0 && world.difficultySetting != EnumDifficulty.PEACEFUL) {
            AxisAlignedBB aabb = AxisAlignedBB.getBoundingBox(
                    x - ComponentIsland.halfSize,
                    10,
                    z - ComponentIsland.halfSize,
                    x + ComponentIsland.halfSize,
                    128,
                    z + ComponentIsland.halfSize);
            ...
            for (EntityMobHomelandEnderman enderman : all) {
                if (enderman.getHomelandRole() == HomelandRole.ISLAND_LEADERS && enderman.attackedRecentlyTimer == 0) {
                    ...
                    if (!babies.isEmpty()) {
                        ...
                        if (chosenOne.isCarrying()) world.spawnEntityInWorld(
                                new EntityItem(
                                        world,
                                        chosenOne.posX,
                                        chosenOne.posY,
                                        chosenOne.posZ,
                                        chosenOne.getCarrying()));
                        ...
                        PacketPipeline.sendToAllAround(
                                grown,
                                64D,
                                new C21EffectEntity(FXType.Entity.BABY_ENDERMAN_GROW, grown));
                    }
