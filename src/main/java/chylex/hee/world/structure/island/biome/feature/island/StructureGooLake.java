...
public class StructureGooLake extends AbstractIslandStructure {
    ...
    protected boolean generate(Random rand) {
        int x = getRandomXZ(rand, 16), z = getRandomXZ(rand, 16), y = world.getHighestY(x, z) + 1, yOff = 0,
                minY = y + 3;
        ...
        double rad = (isSwamp ? 3D * (rand.nextDouble() * 2D + 0.8D) : 2D) + rand.nextDouble() * 3D
                + rand.nextDouble() * 3D, hrad = rad * 0.5D;
