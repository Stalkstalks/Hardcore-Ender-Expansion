...
public final class BiomeDecoratorInfestedForest extends IslandBiomeDecorator {
    ...
    public void genDeep() {
        ...
        for (int a = 0; a < 169; a++) {
            ...
            for (int attempt = 0; attempt < 100; attempt++) {
                int xx = getRandomXZ(rand, 0), zz = getRandomXZ(rand, 0),
                        yy = attempt > 70 ? 10 + rand.nextInt(50) : world.getHighestY(xx, zz);
                if (world.getBlock(xx, yy, zz) == topBlock && world.isAir(xx, yy + 1, zz)) {
                    world.setBlock(
                            xx,
                            yy + 1,
                            ...
                            BlockCrossedDecoration.dataInfestedGrass);
                }
            }
            ...
            for (int attempt = 0; attempt < 85; attempt++) {
                int xx = getRandomXZ(rand, 0), zz = getRandomXZ(rand, 0),
                        yy = attempt > 60 ? 10 + rand.nextInt(50) : world.getHighestY(xx, zz);
                if (world.getBlock(xx, yy, zz) == topBlock && world.isAir(xx, yy + 1, zz)) {
                    world.setBlock(
                            xx,
                            yy + 1,
                            zz,
                            BlockList.crossed_decoration,
                            BlockCrossedDecoration.dataInfestedFern);
                }
            }
            ...
            for (int attempt = 0; attempt < 80; attempt++) {
                int xx = getRandomXZ(rand, 0), zz = getRandomXZ(rand, 0),
                        yy = attempt > 50 ? 10 + rand.nextInt(50) : world.getHighestY(xx, zz);
                if (world.getBlock(xx, yy, zz) == topBlock && world.isAir(xx, yy + 1, zz)) {
                    world.setBlock(
                            xx,
                            yy + 1,
                            zz,
                            BlockList.crossed_decoration,
                            BlockCrossedDecoration.dataInfestedTallgrass);
                }
            }
        }
    }
    ...
    public void genRavaged() {
        ...
        for (int attempt = 0, placed = 0; attempt < 1600 && placed < 420; attempt++) {
            genTree.setTreeType(
                    rand.nextInt(4) == 0 ? TreeType.SIMPLE_PYRAMID
                            : rand.nextBoolean() ? TreeType.SIMPLE_BULGING : TreeType.SIMPLE_SPHERICAL);
            if (generateStructure(genTree)) ++placed;
        }

        // PATCHES OF INFESTED GRASS, TALL GRASS AND FERNS
        for (int pick = 0, pickAmount = rand.nextInt(50) + 240; pick < pickAmount; pick++) {
            int xx = getRandomXZ(rand, 48), zz = getRandomXZ(rand, 48), yy = 8 + rand.nextInt(30),
                    type = rand.nextInt(3);
            ...
            for (int attempt = 0, attemptAmount = 70 + rand.nextInt(80), px, py, pz, meta; attempt
                ...
                if (world.getBlock(px, py, pz) == topBlock && world.isAir(px, py + 1, pz)) {
                    meta = (type == 0 ? BlockCrossedDecoration.dataInfestedTallgrass
                            : type == 1 ? BlockCrossedDecoration.dataInfestedGrass
                                    : BlockCrossedDecoration.dataInfestedFern);

                    if (rand.nextInt(8) == 0) {
                        int newType = rand.nextInt(3);
                        meta = (newType == 0 ? BlockCrossedDecoration.dataInfestedTallgrass
                                : newType == 1 ? BlockCrossedDecoration.dataInfestedGrass
                                        : BlockCrossedDecoration.dataInfestedFern);
                    }
                ...
                for (int attempt = 0, type; attempt < 120; attempt++) {
                    ...
                    if (world.getBlock(xx, yy - 1, zz) == topBlock) {
                        ...
                        world.setBlock(
                                ...
                                zz,
                                BlockList.crossed_decoration,
                                type == 0 ? BlockCrossedDecoration.dataInfestedTallgrass
                                        : type == 1 ? BlockCrossedDecoration.dataInfestedGrass
                                                : BlockCrossedDecoration.dataInfestedFern);
                    }
                }
    ...
    public void genRuins() {
        ...
        for (int cx = 0; cx < world.getChunkAmountX(); cx++) {
            for (int cz = 0; cz < world.getChunkAmountZ(); cz++) {
                for (int attempt = 0, amount = 14 + rand.nextInt(7); attempt < amount; attempt++)
                    generateStructure(genRuinPillar.setIsDeep(rand.nextInt(20) == 0));
            }
        }
        ...
        for (int bulkAttempt = 0, bulk = 0; bulkAttempt < 18 && bulk < 4 + rand.nextInt(3); bulkAttempt++) {
            ...
            for (int attempt = 0; attempt < totalAttempts; attempt++) {
                px = xx + (int) ((rand.nextInt(20) - rand.nextInt(20)) * rand.nextGaussian()
                        * ((2D * attempt) / totalAttempts));
                pz = zz + (int) ((rand.nextInt(20) - rand.nextInt(20)) * rand.nextGaussian()
                        * ((2D * attempt) / totalAttempts));
