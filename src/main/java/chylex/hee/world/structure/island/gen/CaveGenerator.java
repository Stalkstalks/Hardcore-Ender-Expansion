...
public class CaveGenerator {
    ...
    public void setup(Random rand, IslandBiomeBase biome) {
        for (int a = 0, x, y, z,
                caveAmount = (int) (biome.getCaveAmountMultiplier() * (8F + rand.nextFloat() * 5.5F)); a
                        < caveAmount; a++) {
            ...
            generateNode(
                    ...
                    y,
                    z,
                    rand.nextFloat() * 0.45F + 2.65F,
                    biome.getCaveBranchingChance(),
                    Vec3.createVectorHelper(
                            rand.nextBoolean() ? (centerX - x) : (x - centerX),
                            (centerY - y) * rand.nextDouble() * 4D,
                            rand.nextBoolean() ? (centerZ - z) : (z - centerZ)).normalize(),
                    0);
        }
    }
    ...
    private void generateNode(Random rand, float x, float y, float z, float rad, float branchingChance, Vec3 vec,
            int iteration) {
        ...
        while (++cycle < 150) {
            ...
            if (rand.nextFloat() < branchingChance) {
                generateNode(
                        ...
                        y,
                        z,
                        rad - rand.nextFloat() * 0.15F,
                        branchingChance * 0.75F,
                        Vec3.createVectorHelper(
                                vec.xCoord + 0.8F * (rand.nextDouble() - 0.5D),
                                vec.yCoord + 0.4F * (rand.nextDouble() - 0.5D),
                                vec.zCoord + 0.8F * (rand.nextDouble() - 0.5D)),
                        iteration + 1);
            }
        }
    }
    ...
    private boolean createAirBlob(Random rand, int x, int y, int z, float rad) {
        ...
        for (int xx = x - intrad; xx <= x + intrad; xx++) {
            for (int yy = y - intrad; yy <= y + intrad; yy++) {
                for (int zz = z - intrad; zz <= z + intrad; zz++) {
                    if (xx <= centerX - radX || xx >= centerX + radX
                            || yy <= centerY - radY
                            || yy >= centerY + radY
                            || zz <= centerZ - radZ
                            || zz >= centerZ + radZ)
        ...
        if (rand.nextInt(120) == 0) {
            ...
            if ((getDistance(xx - x, yy - y, zz - z) < rad * 0.5F || rand.nextInt(8) == 0))
                clusterList.add(new BlockPosM(xx, yy, zz));
        }
