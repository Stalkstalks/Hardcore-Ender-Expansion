...
public class LaboratoryElementPlacer {
    ...
    public static void generateHallStairs(LargeStructureWorld world, Random rand, int x, int y, int z, int xAdd,
            int yAdd, int zAdd) {
        int x1 = x, x2 = x, z1 = z, z2 = z;
    ...
    public static void generateSmallRoom(LargeStructureWorld world, Random rand, int x, int y, int z) {
            ...
            for (int a = 0; a < 5; a++) {
                for (int b = 0; b < 2; b++) {
                    world.setBlock(
                            ...
                            y + py,
                            z - 2 + a,
                            py == 2 && a >= 1 && a <= 3 ? BlockList.laboratory_glass : blockLab);
                    world.setBlock(
                            x - 2 + a,
                            y + py,
                            z - 4 + b * 8,
        ...
        for (int px = 0; px < 9; px++) {
            for (int pz = 0; pz < 9; pz++) {
                for (int py = -1; py > -5; py--) {
                    if (world.isAir(x - 4 + px, y + py, z - 4 + pz)
                            && isValidFloor(world.getBlock(x - 4 + px, y + py + 1, z - 4 + pz))) {
        ...
        for (int py = 1; py < 4; py++) {
            for (int px = 0; px < 7; px++) {
                for (int pz = 0; pz < 7; pz++) {
                    if (world.getBlock(x - 3 + px, y + py, z - 3 + pz) != blockLab)
    ...
    public static void generateLargeRoom(LargeStructureWorld world, Random rand, int x, int y, int z) {
        ...
        for (int px = 0; px < 11; px++) {
            for (int pz = 0; pz < 11; pz++) {
                for (int py = -1; py > -5; py--) {
                    if (world.isAir(x - 5 + px, y + py, z - 5 + pz)
                            && isValidFloor(world.getBlock(x - 5 + px, y + py + 1, z - 5 + pz))) {
            ...
            for (int a = 0; a < 5; a++) {
                for (int b = 0; b < 2; b++) {
                    if (world.getBlock(x - 2 + a, y + py, z - 4 + b * 8) != blockLab)
                        world.setBlock(x - 2 + a, y + py, z - 4 + b * 8, Blocks.air);
                    if (world.getBlock(x - 4 + b * 8, y + py, z - 2 + a) != blockLab)
                        world.setBlock(x - 4 + b * 8, y + py, z - 2 + a, Blocks.air);
                }
            }
        }
    }
