...
public class StructureIgneousRockOre extends AbstractIslandStructure {
    ...
    protected boolean generate(Random rand) {
        int minX = 20, maxX = ComponentIsland.size - 20, minY = 20, maxY = 60, minZ = 20,
                maxZ = ComponentIsland.size - 20, amount, px, py, pz;

        for (int a = 0; a < attempts; a++) {
            amount = 3 + rand.nextInt(4);
            int x = rand.nextInt(maxX - minX + 1) + minX, y = rand.nextInt(maxY - minY + 1) + minY,
