...
public class OreGenerator {
    ...
    private void generateOreCluster(List<OreLocationList> locList, Random rand, int amount) {
        int xx = rand.nextInt(maxX - minX + 1) + minX, yy = rand.nextInt(maxY - minY + 1) + minY,
                zz = rand.nextInt(maxZ - minZ + 1) + minZ;
        ...
        for (int a = 0, n = amount * attemptMp; a < n; ++a) {
            list.blockList.add(
                    new BlockPosM(
                            xx + MathHelper.cos((float) (rand.nextDouble() * 2D * Math.PI)) * sqrtAmount
                                    * rand.nextDouble(),
                            yy + MathHelper.cos((float) (rand.nextDouble() * 2D * Math.PI)) * sqrtAmount
                                    * rand.nextDouble(),
                            zz + MathHelper.cos((float) (rand.nextDouble() * 2D * Math.PI)) * sqrtAmount
                                    * rand.nextDouble()));
        }
