...
public class StructureSpookyTree extends AbstractIslandStructure {
    ...
    protected boolean generate(Random rand) {
        ...
        if (world.getBlock(x, y++, z) == surface()) {
            ...
            int treeHeight = rand.nextInt(4 + rand.nextInt(4))
                    + (tallTrees ? 5 + rand.nextInt(1 + rand.nextInt(7)) : 4);
            ...
            for (int treeY = 0, meta = 0; treeY
                    < treeHeight; treeY++, meta = canGenerateFace && rand.nextInt(12) == 0 ? rand.nextInt(5) : 0) {
            ...
            if (treeType == TreeType.SIMPLE_PYRAMID) {
                ...
                for (int dir = 0; dir < 4; dir++) world.setBlock(
                        x + Direction.offsetX[dir],
                        topY - 1,
                        z + Direction.offsetZ[dir],
                        BlockList.spooky_leaves);

                for (int pyramidBottomY = topY - 2; pyramidBottomY
                        >= topY - 3 - pyramidBottomHeight; pyramidBottomY--) {
                ...
                for (int py = y + treeHeight + 2; py >= y + treeHeight; py--)
                    world.setBlock(x, py, z, BlockList.spooky_leaves);

                for (int middleY = 0; middleY <= 2 + bulgeHeight; middleY++) {
                    for (int dir = 0; dir < 4; dir++) world.setBlock(
                            x + Direction.offsetX[dir],
                            y + treeHeight + 1 - middleY,
                            z + Direction.offsetZ[dir],
                            BlockList.spooky_leaves);
                }
                ...
                for (int bulgeY = 0; bulgeY <= bulgeHeight; bulgeY++) {
                    ...
                    if (treeType == TreeType.SIMPLE_SPHERICAL) {
                        for (int dir = 0; dir < 4; dir++) world.setBlock(
                                x + Direction.offsetX[dir] * 2,
                                y + treeHeight - bulgeY,
                                z + Direction.offsetZ[dir] * 2,
                                BlockList.spooky_leaves);
                    }
                }
            }
