...
public class StructureCinderPatch extends AbstractIslandStructure {
    ...
    protected boolean generate(Random rand) {
        ...
        double rad = 3.5D + rand.nextDouble() * rand.nextDouble()
                * 7D
                * (biomeData.hasDeviation(IslandBiomeBurningMountains.EXCESSIVE_CINDER) ? 1.5D : 1D);
        generateBlob(rand, x, y, z, rad);

        for (int attempt = 0, attemptAmount = rand.nextInt(4) * rand.nextInt(4); attempt < attemptAmount; attempt++) {
            generateBlob(
                    rand,
                    x + (rand.nextDouble() - 0.5D) * 1.2D * rad,
                    y + (rand.nextDouble() - 0.5D) * 1.2D * rad,
                    z + (rand.nextDouble() - 0.5D) * 1.2D * rad,
                    3D + rand.nextDouble() * (rad - 2D));
        }
            ...
            for (yy = MathUtil.floor(y - rad) - 1; yy <= y + rad + 1; yy++) {
                ...
                for (zz = MathUtil.floor(z - rad) - 1; zz <= z + rad + 1; zz++) {
                    if (MathUtil.distance(xx - x, yy - y, zz - z) <= rad - rand.nextDouble()) {
                        ...
                        if (block == Blocks.end_stone || block == BlockList.end_terrain)
                            world.setBlock(xx, yy, zz, BlockList.cinder);
                    }
                }
            }
        }
