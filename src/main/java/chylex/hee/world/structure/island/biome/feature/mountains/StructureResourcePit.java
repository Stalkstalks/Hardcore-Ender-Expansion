...
public class StructureResourcePit extends AbstractIslandStructure {
    ...
    protected boolean generate(Random rand) {
        ...
        byte height = (byte) (rand.nextInt(10) + 12
                + (biomeData.hasDeviation(IslandBiomeBurningMountains.DEEP_RESOURCE_PITS) ? 6 + rand.nextInt(10) : 0)),
        ...
        for (int attempt = 0, x, z; attempt < 60; attempt++) {
            ...
            for (int py = maxy; py > maxy - height - rand.nextInt(4); py--) {
                for (int px = -irad; px <= irad; px++) {
                    for (int pz = -irad; pz <= irad; pz++) {
                        if (holes[px + irad][pz + irad]) world.setBlock(
                                x + px,
                                py,
                                z + pz,
                                py < maxy - height + 3 ? Blocks.flowing_lava : Blocks.air,
                                0,
                                py < maxy - height + 3);
                    }
                }
            ...
            for (int featureAttempt = 0, placed = 0, max = 25 + rand.nextInt(18), px, py,
                    pz; featureAttempt < 580 + height * 5 + irad * irad * 4
                            && placed < max + rand.nextInt(8); featureAttempt++) {
                ...
                if (world.getBlock(px, py, pz) == Blocks.end_stone
                        && (world.isAir(px - 1, py, pz) || world.isAir(px + 1, py, pz)
                                || world.isAir(px, py, pz - 1)
                                || world.isAir(px, py, pz + 1))) {
