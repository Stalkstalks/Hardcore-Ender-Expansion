...
public final class IslandSpawnChecker {
    ...
    public static final byte getIslandBiomeAt(int chunkX, int chunkZ, long seed1, int seed2) {
        ...
        double dist = Math
                .sqrt(MathUtil.square(x2 * 16L + (featureSize >> 1)) + MathUtil.square(z2 * 16L + (featureSize >> 1)));

        if (dist >= minDistanceFromCenter && coordCheckRand.nextInt(7) <= 4
                && WorldGenChance.checkChance(
                        0.65D + 0.35D * WorldGenChance.linear2Incr.calculate(dist, 1600D, 4000D),
                        coordCheckRand)) {
