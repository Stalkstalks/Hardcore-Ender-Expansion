...
public final class LaboratoryPlan {
    ...
    public boolean generate(LargeStructureWorld world, Random rand) {
        ...
        while (--attempts >= 0) {
            ...
            int xx = room.x + room.type.halfSizeX * Direction.offsetX[dir],
                    zz = room.z + room.type.halfSizeZ * Direction.offsetZ[dir], startY = room.y, prevY = room.y,
                    dist = 0;

            while (++dist < 25) {
                LaboratoryElement hall = trySpawnElement(
                        world,
                        dir == 0 || dir == 2 ? LaboratoryElementType.HALL_Z : LaboratoryElementType.HALL_X,
                        xx + Direction.offsetX[dir] * dist,
                        zz + Direction.offsetZ[dir] * dist,
                        dir);
                if (hall == null || Math.abs(hall.y - prevY) > 1
                        || Math.abs(hall.y - startY) > 3
                        || !hasSpaceFor(hall, room))
                    break;
            }
            ...
            for (int placeAttempt = 0, tmpDist; placeAttempt < dist + 5; placeAttempt++) {
                ...
                LaboratoryElementType type = rand.nextBoolean() ? LaboratoryElementType.LARGE_ROOM
                        : LaboratoryElementType.SMALL_ROOM;
                LaboratoryElement newRoom = trySpawnElement(
                        world,
                        type,
                        xx + tmpDist * Direction.offsetX[dir],
                        zz + tmpDist * Direction.offsetZ[dir],
                        dir);
                ...
                if (newRoom != null && Math.abs(newRoom.y - startY) <= 4 && hasSpaceFor(newRoom, null)) {
                    ...
                    hasGenerated = true;
                    tmpDist -= Math.abs(
                            Direction.offsetX[dir] * (newRoom.type.halfSizeX + 1)
                                    + Direction.offsetZ[dir] * (newRoom.type.halfSizeZ + 1));

                    LaboratoryElementType hallType = dir == 0 || dir == 2 ? LaboratoryElementType.HALL_Z
                            : LaboratoryElementType.HALL_X;
    ...
    private boolean hasSpaceFor(LaboratoryElement testElement, LaboratoryElement exception) {
        for (LaboratoryElement element : elements) {
            ...
            if (Math.abs(element.x - testElement.x) <= element.type.sizeX
                    && Math.abs(element.z - testElement.z) <= element.type.sizeZ)
                return false;
        }
    ...
    private static LaboratoryElement trySpawnElement(LargeStructureWorld world, LaboratoryElementType type, int x,
            int z, int dir) {
