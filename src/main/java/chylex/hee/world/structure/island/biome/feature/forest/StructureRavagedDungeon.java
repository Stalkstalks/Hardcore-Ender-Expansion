...
public class StructureRavagedDungeon extends AbstractIslandStructure {
    ...
    private static final byte dungW = 26, dungH = 26, dungHalfW = dungW >> 1, dungHalfH = dungH >> 1, scale = 5,
            scaleHalf = 2, hallHeight = 4, maxEntranceHeight = 10;
    ...
    protected boolean generate(Random rand) {
        ...
        for (int a = 0; a < gen.layers.length; a++) {
            ...
            if (a < gen.layers.length - 1)
                generateDescendRoom(rand, elements, elements.getAll(DungeonElementType.DESCEND).get(0), y);
    ...
    private int generateEntrance(Random rand, DungeonElementList elements, DungeonElement entrance, int y) {
        placer.generateEntrance(
                world,
                rand,
                getElementX(entrance),
                y,
                getElementZ(entrance),
                maxEntranceHeight,
                entrance);
    ...
    private void generateHallwayWithoutConnections(Random rand, DungeonElementList elements, DungeonElement hallway,
            int y) {
    ...
    private void generateEnd(Random rand, DungeonElementList elements, List<DungeonElement> end, int y) {
        ...
        for (DungeonElement endElement : end) generateConnections(elements, endElement, y); // generate connections
                                                                                            // first only in end room
