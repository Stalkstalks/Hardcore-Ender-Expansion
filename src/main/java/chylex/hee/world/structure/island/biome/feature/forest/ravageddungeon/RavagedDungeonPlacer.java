...
public final class RavagedDungeonPlacer implements ITileEntityGenerator {
    ...
    public void generateEntrance(LargeStructureWorld world, Random rand, int x, int y, int z, int maxEntranceHeight,
            DungeonElement entrance) {
        ...
        for (int yy = surfaceY + 5; yy >= y - maxEntranceHeight; yy--) {
            for (int xx = x - radEntrance; xx <= x + radEntrance; xx++) {
                for (int zz = z - radEntrance; zz <= z + radEntrance; zz++) {
                    if (yy > surfaceY || (Math.abs(xx - x) <= radEntrance - 1 && Math.abs(zz - z) <= radEntrance - 1
    ...
    public void generateDescend(LargeStructureWorld world, Random rand, int x, int y, int z, DungeonElement descend) {
        ...
        for (int yy = y; yy >= y - hallHeight - 2; yy--) {
            for (int xx = x - radEntrance; xx <= x + radEntrance; xx++) {
                for (int zz = z - radEntrance; zz <= z + radEntrance; zz++) {
                    if (Math.abs(xx - x) <= 1 && Math.abs(zz - z) <= radEntrance - 1 && yy != y - hallHeight - 2)
                        world.setBlock(xx, yy, zz, yy == y ? coverBlock : Blocks.air);
                    else world.setBlock(xx, yy, zz, BlockList.ravaged_brick, getBrickMeta(rand));
                }
            }
        }
                        ...
                        for (int b = 0; b < 2; b++) {
                            for (int py = 0; py < hallHeight - 1; py++) {
                                world.setBlock(
                                        ...
                                        y + 1 + py,
                                        z - 4 + 8 * b,
                                        py == 2 ? BlockList.ravaged_brick_glow : BlockList.ravaged_brick);
                            }
                        }
                    }
    ...
    public void generateHallway(LargeStructureWorld world, Random rand, int x, int y, int z, DungeonElement hallway) {
        ...
        int connections = ((hallway.checkConnection(DungeonDir.UP) ? 1 : 0)
                + (hallway.checkConnection(DungeonDir.DOWN) ? 1 : 0)
                + (hallway.checkConnection(DungeonDir.LEFT) ? 1 : 0)
                + (hallway.checkConnection(DungeonDir.RIGHT) ? 1 : 0));

        boolean isStraight = (hallway.checkConnection(DungeonDir.UP) && hallway.checkConnection(DungeonDir.DOWN)
                && !hallway.checkConnection(DungeonDir.LEFT)
                && !hallway.checkConnection(DungeonDir.RIGHT))
                || (hallway.checkConnection(DungeonDir.LEFT) && hallway.checkConnection(DungeonDir.RIGHT)
                        && !hallway.checkConnection(DungeonDir.UP)
                        && !hallway.checkConnection(DungeonDir.DOWN));
        boolean isFullyOpen = hallway.checkConnection(DungeonDir.UP) && hallway.checkConnection(DungeonDir.LEFT)
                && hallway.checkConnection(DungeonDir.DOWN)
                && hallway.checkConnection(DungeonDir.RIGHT);
        ...
        switch (design) {
            case DESTROYED_WALLS:
                for (int attempt = 0, attempts = 80 + rand.nextInt(40 + rand.nextInt(30)), xx, yy, zz; attempt
                        < attempts; attempt++) {
                    ...
                    if (world.getBlock(xx, yy, zz) == BlockList.ravaged_brick) {
                        world.setBlock(
                                ...
                                yy,
                                zz,
                                BlockList.ravaged_brick,
                                BlockRavagedBrick.metaDamaged1 + rand
                                        .nextInt(1 + BlockRavagedBrick.metaDamaged4 - BlockRavagedBrick.metaDamaged1));
                    }
                }
            ...
            case STAIR_PATTERN:
                ...
                int metaStairsLeft = (isLR ? offFacing.getRotatedRight() : offFacing.getRotatedLeft()).getStairs()
                        + (rand.nextBoolean() ? 0 : 4),
                        metaStairsRight = (isLR ? offFacing.getRotatedLeft() : offFacing.getRotatedRight()).getStairs()
                                + (rand.nextBoolean() ? 0 : 4);
                        ...
                        for (int ax2 = 2; ax2 <= 3; ax2++) {
                            if (ax2 == 2 && (ax1 == -1 || ax1 == 1)) {
                                world.setBlock(
                                        ...
                                        yy,
                                        z + rotZ(off, ax1, 2),
                                        BlockList.ravaged_brick_stairs,
                                        ax1 == -1 ? metaStairsLeft : metaStairsRight);
                                continue;
                            }

                            if (ax2 == 3 && !canReplaceBlock(
                                    world.getBlock(x + rotX(off, ax1, ax2), yy, z + rotZ(off, ax1, ax2))))
                                continue;
                            world.setBlock(
                                    x + rotX(off, ax1, ax2),
                                    yy,
                                    z + rotZ(off, ax1, ax2),
                                    ax2 == 2 ? Blocks.air : BlockList.ravaged_brick,
                                    ax2 == 2 ? 0 : getBrickMeta(rand));
                        }
                    }
                }
            ...
            case EMBEDDED_CHEST:
                ...
                off = hallway.checkConnection(DungeonDir.UP) || hallway.checkConnection(DungeonDir.DOWN)
                        ? DungeonDir.LEFT
                        : DungeonDir.UP;
                ...
                for (int yy = y + 1; yy <= y + hallHeight && canGenerate; yy++) {
                    for (int ax1 = -sz; ax1 <= sz && canGenerate; ax1++) {
                        for (int ax2 = 2; ax2 <= 3 && canGenerate; ax2++) {
                            if (ax2 == 3 && !canReplaceBlock(
                                    world.getBlock(x + rotX(off, ax1, ax2), yy, z + rotZ(off, ax1, ax2)))) {
                                if (world.getBlock(x + rotX(off, ax1, ax2), yy, z + rotZ(off, ax1, ax2))
                                        == Blocks.chest) {
                                    canGenerate = false;
                                }
                ...
                if (sz == 2) {
                    world.setBlock(
                            ...
                            z + rotZ(off, -2, 2),
                            BlockList.ravaged_brick_stairs,
                            4 + (isLR ? offFacing.getRotatedRight() : offFacing.getRotatedLeft()).getStairs());
                    world.setBlock(
                            x + rotX(off, 2, 2),
                            y + 1,
                            z + rotZ(off, 2, 2),
                            BlockList.ravaged_brick_stairs,
                            4 + (isLR ? offFacing.getRotatedLeft() : offFacing.getRotatedRight()).getStairs());
                }

                for (int a = 0; a < 2; a++) world.setBlock(
                        x + rotX(off, -1 + 2 * a, 2),
                        y + 1,
                        z + rotZ(off, -1 + 2 * a, 2),
                        BlockList.ravaged_brick_stairs,
                ...
                world.setBlock(
                        ...
                        z + rotZ(off, 0, 2),
                        Blocks.chest,
                        offFacing.getReversed().get6Directional());
                world.setTileEntityGenerator(
                        x + rotX(off, 0, 2),
                        y + 2,
                        z + rotZ(off, 0, 2),
                        "hallwayEmbeddedChest",
                        this);
                break;
            ...
            case COBWEBS:
                for (int attempt = 0, attempts = 2 + rand.nextInt(6 * (1 + rand.nextInt(2))), xx, yy, zz; attempt
                        < attempts; attempt++) {
                    xx = x + rand.nextInt(radHallway + 1) - rand.nextInt(radHallway + 1);
                    zz = z + rand.nextInt(radHallway + 1) - rand.nextInt(radHallway + 1);
                    yy = y + 1 + rand.nextInt(hallHeight);
            ...
            case WALL_MOUNTED_SPAWNERS:
                ...
                for (int attempt = 0, placed = 0; attempt < 1 + rand.nextInt(3)
                        && !availableDirs.isEmpty(); attempt++) {
            ...
            case SPAWNERS_IN_WALLS:
                ...
                for (int attempt = 0, placed = 0, maxPlaced = 2 + level + rand.nextInt(3 + level), xx, yy,
                        zz; attempt < 22 && placed < maxPlaced; attempt++) {
                    xx = x + rand.nextInt(radHallway + 1) - rand.nextInt(radHallway + 1);
                    zz = z + rand.nextInt(radHallway + 1) - rand.nextInt(radHallway + 1);
                    yy = rand.nextBoolean() ? y + 1 : y + hallHeight;
                ...
                for (DungeonDir dir : DungeonDir.values) {
                    if (!hallway.checkConnection(dir)) {
                        for (int a = -1; a <= 1; a++) {
                            for (int yy = y + 1; yy <= y + hallHeight; yy++) {
                                if (canReplaceBlock(world.getBlock(x + rotX(dir, a, 3), yy, z + rotZ(dir, a, 3))))
                                    world.setBlock(
                                            x + rotX(dir, a, 3),
                                            yy,
                                            z + rotZ(dir, a, 3),
            ...
            case FLOWER_POT:
                ...
                if (type == 0) { // embedded in wall
                    ...
                    for (int ax1 = -1; ax1 <= 1; ax1++) {
                        if (canReplaceBlock(world.getBlock(x + rotX(off, ax1, 3), y + 2, z + rotZ(off, ax1, 3))))
                            world.setBlock(
                                    x + rotX(off, ax1, 3),
                                    y + 2,
                                    z + rotZ(off, ax1, 3),
        ...
        if (connections >= 3 && rand.nextInt(10) == 0)
            world.setBlock(x, y + hallHeight, z, BlockList.ravaged_brick_glow);
    }
    ...
    public void generateRoom(LargeStructureWorld world, Random rand, int x, int y, int z, DungeonElement room) {
        ...
        boolean isStraight = (room.checkConnection(DungeonDir.UP) && room.checkConnection(DungeonDir.DOWN)
                && !room.checkConnection(DungeonDir.LEFT)
                && !room.checkConnection(DungeonDir.RIGHT))
                || (room.checkConnection(DungeonDir.LEFT) && room.checkConnection(DungeonDir.RIGHT)
                        && !room.checkConnection(DungeonDir.UP)
                        && !room.checkConnection(DungeonDir.DOWN));
        ...
        while (!hasGenerated) {
            ...
            switch (design) {
                ...
                case SCATTERED_SPAWNERS_WITH_COAL:
                    for (int attempt = 0, attemptAmount = rand.nextInt(3) + 4 + level * 2, xx, zz,
                            spawnerMeta = rand.nextBoolean() ? 2 : 3; attempt < attemptAmount; attempt++) {
                ...
                case ENCASED_CUBICLE:
                    ...
                    for (int a = 0; a < 2; a++) {
                        for (int b = 0; b < 2; b++) {
                            world.setBlock(
                                    ...
                                    y + hallHeight,
                                    z - 6 + 12 * b,
                                    BlockList.ravaged_brick_glow);
                            world.setBlock(
                                    x - 6 + 12 * a,
                                    y + hallHeight - 1,
                                    z - 6 + 12 * b,
                                    BlockList.ravaged_brick_slab,
                                    8);
                        }
                    }
                    ...
                    int[] spawnerX = new int[] { -1, -1, -1, 0, 0, 1, 1, 1 },
                            spawnerZ = new int[] { -1, 0, 1, -1, 1, -1, 0, 1 };
                            ...
                            for (int plant = 0, xx, zz; plant < plants; plant++) {
                                ...
                                if (world.getBlock(xx, y + 1, zz) == Blocks.dirt) {
                                    ...
                                    switch (rand.nextInt(11)) {
                                        ...
                                        case 5:
                                            plantBlock = rand.nextBoolean() ? Blocks.brown_mushroom
                                                    : Blocks.red_mushroom; // mushrooms are a little more rare
                                            break;
                ...
                case RUINS:
                    for (int attempt = 0, attemptAmount = 28 + rand.nextInt(12), placedChests = 0, xx, zz; attempt
                            < attemptAmount; attempt++) {
                ...
                case FOUR_SPAWNERS:
                    ...
                    for (int a = 0; a < zValues.length; a++) {
                        world.setBlock(
                                x + rotX(dir, 6, zValues[a]),
                                y + 2,
                                z + rotZ(dir, 6, zValues[a]),
                                BlockList.ravaged_brick_fence);
                        world.setBlock(
                                x + rotX(dir, 6, zValues[a]),
                                y + 3,
                                z + rotZ(dir, 6, zValues[a]),
                ...
                case GLOWING_ROOM:
                    int[] pillarX = new int[] { -6, -2, 2, 6, -6, 6, -6, 6, -6, -2, 2, 6 },
                            pillarZ = new int[] { -6, -6, -6, -6, -2, -2, 2, 2, 6, 6, 6, 6 };
                        ...
                        for (int yy = y + 1; yy <= y + hallHeight; yy++) {
                            ...
                            for (DungeonDir checkDir : DungeonDir.values) {
                                if (room.checkConnection(checkDir)) {
                                    for (int ax1 = -1; ax1 <= 1; ax1++) {
                                        world.setBlock(
                                                x + rotX(checkDir, ax1, -6),
                                                yy,
                                                z + rotZ(checkDir, ax1, -6),
                                                Blocks.air);
                                    }
                                }
                            }
                            ...
                            switch (bowlFill) {
                                ...
                                case 2:
                                    world.setBlock(
                                            ...
                                            y + 1,
                                            z - 4 + 7 * b,
                                            bowlFill == 0 ? BlockList.ender_goo
                                                    : bowlFill == 1 ? BlockList.ravaged_brick_slab : Blocks.obsidian);
                                    world.setBlock(
                                            x - 3 + 7 * a,
                                            y + 1,
                                            z - 4 + 7 * b,
                                ...
                                case 3:
                                    world.setBlock(
                                            ...
                                            y + 1,
                                            z - 4 + 7 * b,
                                            (a ^ b) == 1 ? BlockList.custom_spawner : Blocks.coal_block,
                                            (a ^ b) == 1 ? 3 : 0);
                                    world.setBlock(
                                            x - 3 + 7 * a,
                                            y + 1,
                                            z - 4 + 7 * b,
                                            (a ^ b) == 0 ? BlockList.custom_spawner : Blocks.coal_block,
                                            (a ^ b) == 0 ? 3 : 0);
                                    world.setBlock(
                                            x - 3 + 7 * a,
                                            y + 1,
    ...
    public void generateEndLayout(LargeStructureWorld world, Random rand, int[] x, int y, int[] z,
            List<DungeonElement> end) {
    ...
    public void generateEndContent(LargeStructureWorld world, Random rand, int[] x, int y, int[] z,
            List<DungeonElement> end) {
        ...
        for (int a = 0; a < gooX.length; a++) {
            for (int px = 0; px < 2; px++) {
                for (int pz = 0; pz < 2; pz++) {
                    world.setBlock(
                            topLeftAirX + gooX[a] + 1 + px,
                            y + hallHeight,
                            topLeftAirZ + gooZ[a] + 1 + pz,
                            BlockList.ender_goo);
                    world.setBlock(
                            topLeftAirX + gooX[a] + 1 + px,
                            y + hallHeight + 1,
            ...
            for (int b = 0; b < 2; b++) {
                for (int py = 0; py < 4; py++) {
                    world.setBlock(
                            ...
                            y + 1 + py,
                            topLeftAirZ + 12 + 3 * b,
                            py == 0 || py == 3 ? BlockList.ravaged_brick_glow : BlockList.ravaged_brick);
        ...
        int[] chestX = new int[] { 13, 14, 13, 14, 12, 12, 15, 15 },
                chestZ = new int[] { 12, 12, 15, 15, 13, 14, 13, 14 };
    ...
    public void onTileEntityRequested(String key, TileEntity tile, Random rand) {
        if (key.equals("hallwayEmbeddedChest")) {
            ...
            for (int attempt = 0, attemptAmount = 4 + rand.nextInt(5) + rand.nextInt(4 + level); attempt
                    < attemptAmount; attempt++) {
                chest.setInventorySlotContents(
                        rand.nextInt(chest.getSizeInventory()),
                        (rand.nextInt(3) == 0 ? RavagedDungeonLoot.lootUncommon : RavagedDungeonLoot.lootGeneral)
                                .generateIS(rand));
            }
            ...
            for (int attempt = 0, attemptAmount = 7 + rand.nextInt(8 + level * 2); attempt < attemptAmount; attempt++) {
                chest.setInventorySlotContents(
                        rand.nextInt(chest.getSizeInventory()),
                        (rand.nextInt(3) != 0 ? RavagedDungeonLoot.lootRare : RavagedDungeonLoot.lootGeneral)
                                .generateIS(rand));
            }
            ...
            for (int attempt = 0, attemptAmount = 5 + rand.nextInt(5 + level * 2); attempt < attemptAmount; attempt++) {
                chest.setInventorySlotContents(
                        rand.nextInt(chest.getSizeInventory()),
                        RavagedDungeonLoot.lootGeneral.generateIS(rand));
            }
        } else if (key.equals("encasedCubicleChest")) {
            TileEntityChest chest = (TileEntityChest) tile;

            for (int attempt = 0, attemptAmount = 10 + level + rand.nextInt(10 + level); attempt
                    < attemptAmount; attempt++) {
                chest.setInventorySlotContents(
                        rand.nextInt(chest.getSizeInventory()),
                        RavagedDungeonLoot.lootRare.generateIS(rand));
            }
        } else if (key.equals("endRoomChest")) {
            TileEntityChest chest = (TileEntityChest) tile;

            for (int attempt = 0, attemptAmount = 3 + rand.nextInt(2 + rand.nextInt(2)); attempt
                    < attemptAmount; attempt++) {
                chest.setInventorySlotContents(
                        rand.nextInt(chest.getSizeInventory()),
                        RavagedDungeonLoot.lootEnd.generateIS(rand));
            }
        } else if (key.equals("louseSpawner")) {
            LouseRavagedSpawnerLogic logic = (LouseRavagedSpawnerLogic) ((TileEntityCustomSpawner) tile)
                    .getSpawnerLogic();
            logic.setLouseSpawnData(new LouseSpawnData(level, rand));
        } else if (key.equals("flowerPot")) {
            ItemStack is = RavagedDungeonLoot.flowerPotItems[rand.nextInt(RavagedDungeonLoot.flowerPotItems.length)]
                    .copy();

            if (is.getItem() == Item.getItemFromBlock(BlockList.death_flower)) {
                tile.getWorldObj().setBlock(
                        tile.xCoord,
                        tile.yCoord,
                        tile.zCoord,
                ...
                if (!flowerPot.getWorldObj().setBlockMetadataWithNotify(
                        flowerPot.xCoord,
                        flowerPot.yCoord,
                        flowerPot.zCoord,
                        is.getItemDamage(),
    ...
    private static int getBrickMeta(Random rand) {
        return rand.nextInt(8) != 0 ? 0
                : rand.nextInt(12) == 0 ? BlockRavagedBrick.metaCracked
                        : BlockRavagedBrick.metaDamaged1
                                + rand.nextInt(1 + BlockRavagedBrick.metaDamaged4 - BlockRavagedBrick.metaDamaged1);
    }
