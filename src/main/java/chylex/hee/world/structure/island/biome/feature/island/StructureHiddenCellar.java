...
public class StructureHiddenCellar extends AbstractIslandStructure implements ITileEntityGenerator {
    ...
    private static final Block[] endermanSolidBlocks = new Block[] { Blocks.grass, Blocks.dirt, Blocks.sand,
            Blocks.gravel, Blocks.tnt, Blocks.clay, Blocks.pumpkin, Blocks.melon_block, Blocks.mycelium };

    private static final WeightedLootList normalChest = new WeightedLootList(
            new LootItemStack[] { new LootItemStack(Items.ender_pearl).setAmount(2, 6).setWeight(20),
    ...
    public static final WeightedLootList[] normalChestVariation = new WeightedLootList[] {
            normalChest.copy().addAll(
                    new LootItemStack[] { new LootItemStack(ItemList.arcane_shard).setAmount(1, 2).setWeight(8) }),

            normalChest.copy().addAll(
                    new LootItemStack[] { new LootItemStack(ItemList.obsidian_fragment).setAmount(1, 3).setWeight(9),
                            new LootItemStack(ItemList.auricion).setAmount(1, 2).setWeight(6) }) };

    private static final WeightedLootList rareChest = new WeightedLootList(
            new LootItemStack[] { new LootItemStack(ItemList.enhanced_ender_pearl).setAmount(3, 9).setWeight(20),
                            ...
                            .addItemPostProcessor(new IItemPostProcessor() {
                                ...
                                public ItemStack processItem(ItemStack is, Random rand) {
                                    if (is.getItem() == ItemList.knowledge_note)
                                        ...
                                        for (int a = 0; a < 1 + Math
                                                .abs(Math.round(rand.nextDouble() * rand.nextGaussian() * 3.2D)); a++) {
    ...
    public static final WeightedLootList[] rareChestVariation = new WeightedLootList[] {
            ...
            rareChest.copy().addAll(new LootItemStack[] { new LootItemStack(ItemList.auricion).setWeight(5) }) };
    ...
    protected boolean generate(Random rand) {
        ...
        for (int attempt = 0, x, y, z; attempt < 50; attempt++) {
            ...
            for (int roomAttemptsLeft = 70 + rand.nextInt(110), roomsLeft = 5 + rand.nextInt(16), side, dist, hWidth,
                    offX, offZ; roomAttemptsLeft > 0 && roomsLeft > 0; roomAttemptsLeft--) {
                ...
                genHall(
                        room.x + (room.halfWidth - 1) * offX - offZ * 2,
                        room.z + (room.halfWidth - 1) * offZ - offX * 2,
                        newRoom.x - (hWidth - 1) * offX + offZ * 2,
                        newRoom.z - (hWidth - 1) * offZ + offX * 2,
                        y,
                        height,
                        rand);
            }
    ...
    private RoomInfo genRoom(int x, int z, int halfWidth, int bottomY, int height, Random rand) {
        ...
        for (int horCheck = 0; horCheck < 8; horCheck++) {
            for (int yCheck = -1; yCheck <= 1; yCheck++) {
                if (world.getBlock(
                        x + horCheckX[horCheck] * halfWidth,
                        bottomY + halfHeight + yCheck * (halfHeight + 1),
                        z + horCheckZ[horCheck] * halfWidth) != Blocks.end_stone)
                    return null;
            }
        }
    ...
    private void genRoomContent(RoomInfo room, int bottomY, int height, Random rand) {
        ...
        switch (type) {
            case CONNECTING_LINES:
                ...
                for (int lines = 3 + ((halfWidth * (height - 2)) >> 1) + rand.nextInt(6 + halfWidth * 2 + height),
                        width = halfWidth * 2 - 2, dir, xx, yy, zz, addX = 0, addY = 0, addZ = 0, a,
                        b; lines > 0; lines--) {
                    dir = rand.nextInt(4);
                    addX = addY = addZ = 0;
                    ...
                    for (a = 0; a < 10; a++) {
                        ...
                        if (world.getBlock(xx - 1, yy, zz) == Blocks.bedrock
                                || world.getBlock(xx + 1, yy, zz) == Blocks.bedrock
                                || world.getBlock(xx, yy - 1, zz) == Blocks.bedrock
                                || world.getBlock(xx, yy + 1, zz) == Blocks.bedrock
                                || world.getBlock(xx, yy, zz - 1) == Blocks.bedrock
                                || world.getBlock(xx, yy, zz + 1) == Blocks.bedrock
                                || !world.isAir(xx, yy, zz))
                            continue;
            ...
            case SPIKES:
                for (int spikes = 2 + halfWidth + rand.nextInt(4 + 3 * halfWidth), xx, yy, zz,
                        width = halfWidth * 2 - 2, addY, spikeHeight; spikes > 0; spikes--) {
                    xx = x + rand.nextInt(width) - (width >> 1);
                    ...
                    if (world.isAir(xx, yy, zz)) {
                        ...
                        for (spikeHeight = 1 + rand.nextInt(height - 3); spikeHeight > 0; spikeHeight--) {
                            if ((xx == x - (halfWidth - 1) || world.isAir(xx - 1, yy + addY, zz))
                                    && (xx == x - (halfWidth + 1) || world.isAir(xx + 1, yy + addY, zz))
                                    && (zz == z - (halfWidth - 1) || world.isAir(xx, yy + addY, zz - 1))
                                    && (zz == z - (halfWidth - 1) || world.isAir(xx, yy + addY, zz + 1))
            ...
            case LOTS_OF_CHESTS:
                for (int chests = 10 + rand.nextInt(8 + 3 * halfWidth) + 4 * halfWidth, xx, yy, zz,
                        width = halfWidth * 2 - 2, attempt, yTest, dir, normal, trapped; chests > 0; chests--) {
                    for (attempt = 0; attempt < 4; attempt++) {
                        ...
                        if (normal == 0 && trapped == 0) world.setBlock(
                                xx,
                                yy,
                                zz,
                                rand.nextBoolean() ? Blocks.chest : Blocks.trapped_chest,
                                rand.nextInt(4));
                        else if (normal == 1 && trapped == 0)
                            world.setBlock(xx, yy, zz, Blocks.trapped_chest, rand.nextInt(4));
                        else if (normal == 0 && trapped == 1) world.setBlock(xx, yy, zz, Blocks.chest, rand.nextInt(4));
                        else continue;

                        if (rand.nextInt(10) == 0) {
                            if (rand.nextInt(6) == 0) world
                                    .setTileEntityGenerator(xx, yy, zz, "CellarChestRare|" + variation.ordinal(), this);
                            else world.setTileEntityGenerator(
                                    xx,
                                    yy,
                                    zz,
                                    "CellarChestNormal|" + variation.ordinal(),
                                    this);
                        }
            ...
            case FLOATING_CUBES:
                for (int cubes = 5 + rand.nextInt(10), size, attempt, xx, yy, zz, px, py, pz, xzSpace, ySpace; cubes
                        > 0; cubes--) {
            ...
            case CHEST_PILLARS:
                for (int amount = 2 + room.halfWidth * (room.halfWidth - 2) + rand.nextInt(5 + room.halfWidth * 3), xx,
                        yy, zz; amount > 0; amount--) {
                    ...
                    if (world.isAir(xx - 1, bottomY + 1, zz) && world.isAir(xx + 1, bottomY + 1, zz)
                            && world.isAir(xx, bottomY + 1, zz - 1)
                            && world.isAir(xx, bottomY + 1, zz + 1)) {
                        world.setBlock(xx, yy, zz, Blocks.chest);
                        if (rand.nextInt(5) <= 1) world.setTileEntityGenerator(
                                xx,
                                yy,
                                zz,
                                (rand.nextInt(4) == 0 ? "CellarChestRare" : "CellarChestNormal") + "|"
                                        + variation.ordinal(),
                                this);
            ...
            case CHAOTIC_PERSEGRIT:
                for (int amount = MathUtil.ceil(
                        Math.sqrt(room.halfWidth * room.halfWidth * (height - 1)) * (0.85D + rand.nextDouble() * 0.5D)),
                        width = halfWidth * 2 - 2; amount > 0; amount--) {
                    world.setBlock(
                            x + rand.nextInt(width) - (width >> 1),
                            bottomY + 1 + rand.nextInt(height - 1),
                            z + rand.nextInt(width) - (width >> 1),
                            BlockList.persegrit);
                }
    ...
    private void genPatterns(Random rand) {
        ...
        for (BlockPosM loc : patternBlocks) {
            int x = loc.x, y = loc.y, z = loc.z, addX = 0, addY = 0, addZ = 0,
                    iterations = 6 + rand.nextInt(30 + rand.nextInt(60));

            if (!isWall(x, y, z)) {
                if (rand.nextBoolean()) addX = rand.nextInt(2) * 2 - 1;
                else addZ = rand.nextInt(2) * 2 - 1;
            } else if (world.getBlock(x - 1, y, z) != BlockList.persegrit
                    || world.getBlock(x + 1, y, z) != BlockList.persegrit) {
                        if (rand.nextInt(3) != 0) addY = rand.nextInt(2) * 2 - 1;
                        else addZ = rand.nextInt(2) * 2 - 1;
                    } else
                if (world.getBlock(x, y, z - 1) != BlockList.persegrit
                        || world.getBlock(x, y, z + 1) != BlockList.persegrit) {
                            if (rand.nextInt(3) != 0) addY = rand.nextInt(2) * 2 - 1;
        ...
        for (BlockPosM loc : connections) {
            world.setBlock(
                    ...
                    loc.z,
                    BlockList.persegrit,
                    BlockPersegrit.getConnectionMeta(world, rand, loc.x, loc.y, loc.z));
        }
    }
    ...
    private boolean isCorner(int x, int y, int z) {
        ...
        Block l = world.getBlock(x - 1, y, z), r = world.getBlock(x + 1, y, z), u = world.getBlock(x, y, z - 1),
                d = world.getBlock(x, y, z + 1), b = world.getBlock(x, y - 1, z), t = world.getBlock(x, y + 1, z);

        return (((l == pg && r == pg) || (u == pg && d == pg)) && ((b == pg && t != pg) || (b != pg && t == pg))) || (t
                == pg && b == pg
                && ((l == pg && r != pg) || (u == pg && d != pg) || (l != pg && r == pg) || (u != pg && d == pg)));
    }
