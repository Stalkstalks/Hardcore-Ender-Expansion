...
public final class LargeStructureWorld {
    ...
    public LargeStructureChunk getChunkFromChunkCoords(int chunkX, int chunkZ) {
        return chunkX < 0 || chunkZ < 0 || chunkX >= chunks.length || chunkZ >= chunks[0].length ? emptyFallbackChunk
                : chunks[chunkX][chunkZ];
    }

    private LargeStructureChunk getChunk(int blockX, int blockZ) {
        if (lastActiveChunk != null && (blockX >> 4) == lastActiveChunk.x && (blockZ >> 4) == lastActiveChunk.z)
            return lastActiveChunk;
        if (blockX < 0 || blockZ < 0 || blockX >= chunks.length * 16 || blockZ >= chunks[0].length * 16)
