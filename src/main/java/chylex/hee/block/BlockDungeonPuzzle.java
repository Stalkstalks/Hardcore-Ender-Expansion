...
public class BlockDungeonPuzzle extends Block implements IBlockSubtypes {
    ...
    public static final byte metaTriggerUnlit = 0, metaTriggerLit = 1, metaChainedUnlit = 2, metaChainedLit = 3,
            metaDistributorSpreadUnlit = 4, metaDistributorSpreadLit = 5, metaDistributorSquareUnlit = 6,
            metaDistributorSquareLit = 7, metaPortal = 11, metaDisabled = 12, metaWall = 13, metaRock = 14,
            metaCeiling = 15;

    public static final byte[] icons = new byte[] { 2, 3, 4, 5, 6, 7, 8, 9, 3, 3, 3, 10, 10, 0, 1, 3 };

    public static final String[] names = new String[] { "trigger.unlit", "trigger.lit", "chained.unlit", "chained.lit",
            "distr.spread.unlit", "distr.spread.lit", "distr.square.unlit", "distr.square.lit", null, null, null, null,
            "disabled", "wall", "rock", "ceiling" };
    ...
    public static final int toggleState(int meta) {
        if (meta == metaWall || meta == metaRock || meta == metaCeiling || meta == metaDisabled || meta == metaPortal)
            return meta;
        ...
        if (meta != toggled) {
            ...
            if (unlit == metaDistributorSpreadUnlit) {
                for (int dir = 0, distrMeta, tx, tz; dir < 4; dir++) {
                    ...
                    if (toggleState(
                            distrMeta = world.getBlockMetadata(
                                    x + (tx = Direction.offsetX[dir]),
                                    y,
                                    z + (tz = Direction.offsetZ[dir])))
                            != distrMeta && world.getBlock(x + tx, y, z + tz) == BlockList.dungeon_puzzle) {
                ...
                for (int xx = -1, zz, distrMeta, distrToggled; xx <= 1; xx++) {
                    for (zz = -1; zz <= 1; zz++) {
                        if ((distrToggled = toggleState(distrMeta = world.getBlockMetadata(x + xx, y, z + zz)))
                                != distrMeta && !(xx == 0 && zz == 0)
                                && world.getBlock(x + xx, y, z + zz) == BlockList.dungeon_puzzle) {
                            PacketPipeline.sendToAllAround(
                                    ...
                                    64D,
                                    new C20Effect(
                                            FXType.Basic.DUNGEON_PUZZLE_BURN,
                                            x + xx + 0.5D,
                                            y + 0.5D,
                                            z + zz + 0.5D));
            ...
            PacketPipeline.sendToAllAround(
                    ...
                    z + 0.5D,
                    64D,
                    new C20Effect(FXType.Basic.DUNGEON_PUZZLE_BURN, x + 0.5D, y + 0.5D, z + 0.5D));
        }
    ...
    public void checkWinConditions(World world, int x, int y, int z) {
        if (world.getEntitiesWithinAABB(
                EntityTechnicalPuzzleChain.class,
                AxisAlignedBB.getBoundingBox(
                        x + 0.5D - maxDungeonSize,
                        y,
                        z + 0.5D - maxDungeonSize,
                        x + 0.5D + maxDungeonSize,
                        y + 1D,
                        z + 0.5D + maxDungeonSize))
                .size() == 1) {
            ...
            if (isFinished && cnt > (maxX - minX + 1) * (maxZ - minZ + 1) * 0.9D) world.spawnEntityInWorld(
                    new EntityTechnicalPuzzleSolved(
                            world,
                            minX + ((maxX - minX + 1) >> 1),
                            y,
                            minZ + ((maxZ - minZ + 1) >> 1),
    ...
    public boolean onBlockEventReceived(World world, int x, int y, int z, int eventID, int eventData) {
        if (eventID == 69) {
            for (int a = 0; a < (eventData == 0 ? 3 : 25); a++) world.spawnParticle(
                    "flame",
                    x + world.rand.nextDouble(),
                    y + (eventData == 0 ? 1.15D : 1D + world.rand.nextDouble() * 2D),
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (BlockPosM.tmp(x, y, z).getMetadata(world) == metaPortal) {
            for (int a = 0; a < 18; a++) HardcoreEnderExpansion.fx.portalColor(
                    world,
                    x + 0.5D + (rand.nextDouble() - 0.5D) * 0.3D,
                    y + 1D + rand.nextDouble() * 2D,
                    z + 0.5D + (rand.nextDouble() - 0.5D) * 0.3D,
                    (rand.nextDouble() - 0.5D) * 0.8D,
    ...
    public void getSubBlocks(Item item, CreativeTabs tab, List list) {
        for (byte meta : new byte[] { metaWall, metaRock, metaCeiling, metaDisabled, metaTriggerUnlit, metaTriggerLit,
                metaChainedUnlit, metaChainedLit, metaDistributorSpreadUnlit, metaDistributorSpreadLit,
                metaDistributorSquareUnlit, metaDistributorSquareLit, })
