...
public class BlockCrossedDecoration extends BlockFlower implements IShearable, IBlockSubtypes {
    ...
    private static final String[] decorTypes = new String[] { "decor_bullrush_bottom", "decor_bullrush_top",
    ...
    public boolean canBlockStay(World world, int x, int y, int z) {
        return (world.getFullBlockLightValue(x, y, z) >= 8 || world.canBlockSeeTheSky(x, y, z)
                ...
                && (BlockPosM.tmp(x, y - 1, z).getBlock(world)
    ...
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (meta == dataLilyFire)
            return AxisAlignedBB.getBoundingBox(x + 0.3F, y, z + 0.3F, x + 0.7F, y + 0.8F, z + 0.7F);
        else if (meta == dataShadowOrchid)
            return AxisAlignedBB.getBoundingBox(x + 0.25F, y, z + 0.25F, x + 0.75F, y + 0.95F, z + 0.75F);
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (!world.isRemote && BlockPosM.tmp(x, y, z).getMetadata(world) == dataThornBush) {
            ...
            if (world.rand.nextInt(80) == 0 && entity instanceof EntityLivingBase
                    && !((EntityLivingBase) entity).isPotionActive(Potion.poison)) {
                ((EntityLivingBase) entity)
                        .addPotionEffect(new PotionEffect(Potion.poison.id, 30 + world.rand.nextInt(40), 1, true));
            }
        }
    }
