...
public class BlockEnhancedTNT extends BlockContainer {
    ...
    public void onBlockExploded(World world, int x, int y, int z, Explosion explosion) {
        if (!world.isRemote) {
            ...
            if (tile != null) {
                EntityBlockEnhancedTNTPrimed tnt = new EntityBlockEnhancedTNTPrimed(
                        world,
                        x + 0.5F,
                        y + 0.5F,
                        z + 0.5F,
                        explosion.getExplosivePlacedBy(),
                        tile.getEnhancements());
                tnt.fuse = tile.getEnhancements().contains(TNTEnhancements.NO_FUSE) ? 1
                        : world.rand.nextInt(tnt.fuse / 4) + tnt.fuse / 8;
    ...
    public boolean removedByPlayer(World world, EntityPlayer player, int x, int y, int z, boolean willHarvest) {
        ...
        if (!exploded) {
            ...
            if (tile instanceof IEnhanceableTile)
                dropBlockAsItem(world, x, y, z, ((IEnhanceableTile) tile).createEnhancedItemStack());
        }
        ...
        if (!world.isRemote && entity instanceof EntityArrow) {
            ...
            if (arrow.isBurning()) {
                tryIgniteTNT(
                        ...
                        z,
                        true,
                        arrow.shootingEntity instanceof EntityLivingBase ? (EntityLivingBase) arrow.shootingEntity
                                : null);
                BlockPosM.tmp(x, y, z).setAir(world);
            }
        }
