...
public class BlockObsidianSpecial extends Block implements IBlockSubtypes {
    ...
    public IIcon getIcon(int side, int meta) {
        if (meta == metaChiseled || meta == metaChiseledParticlesU) {
            ...
            return (meta == metaPillarV && side <= 1) || (meta == metaPillarNS && side >= 4)
                    || (meta == metaPillarEW && (side == 2 || side == 3)) ? iconPillarTop : iconPillar;
    ...
    public int onBlockPlaced(World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int meta) {
        return (meta == 2)
                ? (side == 0 || side == 1 ? 2 : side == 2 || side == 3 ? 4 : side == 4 || side == 5 ? 3 : meta)
                : meta;
    }

    @Override
    public int damageDropped(int meta) {
        return meta == metaChiseledParticlesU ? 1
                : meta == metaSmoothParticlesD ? 0 : meta == metaPillarNS || meta == metaPillarEW ? 2 : meta;
    }
    ...
    public String getUnlocalizedName(ItemStack is) {
        switch (is.getItemDamage()) {
            case metaChiseled:
                return isGlowing ? "tile.obsidianSpecialGlowing.chiseled" : "tile.obsidianSpecial.chiseled";
            case metaPillarV:
                return isGlowing ? "tile.obsidianSpecialGlowing.pillar" : "tile.obsidianSpecial.pillar";
            default:
                return isGlowing ? "tile.obsidianSpecialGlowing.smooth" : "tile.obsidianSpecial.smooth";
        }
    }
