...
public class BlockEnergyCluster extends BlockContainer {
    ...
    public boolean addHitEffects(World world, MovingObjectPosition target, EffectRenderer effectRenderer) {
        for (int a = 0; a < 4; a++) effectRenderer.addEffect(
    ...
    public boolean addDestroyEffects(World world, int x, int y, int z, int meta, EffectRenderer effectRenderer) {
        for (int a = 0; a < 4; a++) effectRenderer
                .addEffect(new EntityEnergyClusterFX(world, x + 0.5D, y + 0.5D, z + 0.5D, 0D, 0D, 0D, 0D, 0D, 0D));
        return true;
    }
    ...
    public static void destroyCluster(TileEntityEnergyCluster tile) {
        ...
        WorldDataHandler.<EnergySavefile>get(EnergySavefile.class).getFromBlockCoords(world, x, z, true)
                .addEnergy(tile.data.getEnergyLevel() * 0.2F);
        BlockPosM tmpPos = BlockPosM.tmp();

        for (int xx = x - idist; xx <= x + idist; xx++) {
            for (int zz = z - idist; zz <= z + idist; zz++) {
                for (int yy = y - idist; yy <= y + idist; yy++) {
                    if (MathUtil.distance(xx - x, yy - y, zz - z) <= dist && world.isAirBlock(xx, yy, zz))
                        tmpPos.set(xx, yy, zz).setBlock(world, BlockList.corrupted_energy_high, energyMeta);
                }
            }
        }

        for (EntityPlayer player : (List<EntityPlayer>) world.getEntitiesWithinAABB(
                EntityPlayer.class,
                AxisAlignedBB.getBoundingBox(x + 0.5D, y + 0.5D, z + 0.5D, x + 0.5D, y + 0.5D, z + 0.5D)
                        .expand(6D, 6D, 6D))) {
            if (player.getDistance(x + 0.5D, y + 0.5D, z + 0.5D) <= 6D)
                CausatumUtils.increase(player, CausatumMeters.END_ENERGY, 20F);
        }
