...
public class BlockRavagedBrick extends Block implements IBlockSubtypes {
    ...
    public static byte metaNormal = 0, metaCracked = 1, metaDamaged1 = 2, metaDamaged2 = 3, metaDamaged3 = 4,
            metaDamaged4 = 5, metaAmount = 6;
    ...
    public float getBlockHardness(World world, int x, int y, int z) {
        ...
        for (TileEntity tile : list) {
            if (tile.blockType == BlockList.custom_spawner && MathUtil.distance(x - tile.xCoord, z - tile.zCoord) < 260)
                ++spawnerCount;
        }

        if (spawnerCount > 24) world.spawnParticle(
                "reddust",
                x - 0.2D + world.rand.nextDouble() * 1.4D,
                y - 0.2D + world.rand.nextDouble() * 1.4D,
                z - 0.2D + world.rand.nextDouble() * 1.4D,
                1D,
                0.2D,
                0.2D);

        return spawnerCount <= 24 ? blockHardness
                : blockHardness + (3F + (float) Math.pow(spawnerCount - 24, 0.8D) * 1.5F);
    }
    ...
    public IIcon getIcon(int side, int meta) {
        return meta >= metaDamaged1 && meta <= metaDamaged4 && (side == 0 || side == 1) ? iconArray[0]
                : iconArray[meta < metaAmount ? meta : 0];
    }
