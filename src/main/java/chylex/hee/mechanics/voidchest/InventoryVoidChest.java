...
public class InventoryVoidChest extends InventoryBasic {
    ...
    private static boolean isSameItem(ItemStack dis, ItemStack other) {
        if (dis == null && other != null || dis != null && other == null) return false;
        return dis == other || dis.getItem() == other.getItem() && dis.getItemDamage() == other.getItemDamage()
                && ItemStack.areItemStackTagsEqual(dis, other);
    }
    ...
    public void putItem(ItemStack is) {
        ...
        if (is.isStackable()) {
            ...
            for (int a = 0; a < size; a++) {
                ...
                if (!isSameItem(slotIS, is)) continue;
