...
public class PotionTypes {
    ...
    static {
        ...
        mapItemToIndex(Items.fish, (short) FishType.PUFFERFISH.func_150976_a(), new int[] { 12 }); // OBFUSCATED get
                                                                                                   // fish damage
        mapItemToIndex(Items.fermented_spider_eye, 2, 4, 6, 8/* ,16 */);
        /*
         * mapItemToIndex(Item.rottenFlesh,13); mapItemToIndex(Item.flint,14); mapItemToIndex(Item.feather,15);
         */
    }
    ...
    public static boolean canBeApplied(ItemStack ingredient, ItemStack is, boolean enhanced) {
        ...
        if (indexes.length == 0) {
            ...
            if (is.getItemDamage() <= 16) {
                ...
                else if (ingredientItem == Items.gunpowder && is.getItem() instanceof ItemAbstractPotion)
    ...
    public static ItemStack applyIngredientUnsafe(ItemStack ingredient, ItemStack is) {
        ...
        if (isSpecialIngredient(ingredientItem)
                || (ingredientItem == Items.gunpowder && is.getItem() instanceof ItemAbstractPotion)) {
        ...
        if (indexes.length == 0) {
            ...
            if (ingredientItem == Items.glowstone_dust) {
                ...
                newEffect = new PotionEffect(
                        eff.getPotionID(),
                        Math.min(eff.getDuration() + data.getDurationStep(), data.maxDuration),
                        eff.getAmplifier(),
                        eff.getIsAmbient());
            }
        ...
        for (Byte b : indexes) {
            ...
            if (data != null && data.requiredDamageValue == (is.getItemDamage() & ~16384)) {
                ...
                if (prevEffect != null) {
                    ...
                    if (curEffect != null) setCustomPotionEffect(
                            is,
                            new PotionEffect(
                                    curEffect.getPotionID(),
                                    prevEffect.getDuration(),
                                    prevEffect.getAmplifier(),
                                    prevEffect.getIsAmbient()));
                }
