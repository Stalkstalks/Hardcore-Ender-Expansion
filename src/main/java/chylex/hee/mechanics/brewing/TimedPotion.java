...
public class TimedPotion extends AbstractPotionData {
    ...
    public TimedPotion(Potion potion, int requiredDamageValue, int damageValue, int maxLevel, int maxLevelEnhanced,
            int startDuration, int maxDuration) {
        this(potion, requiredDamageValue, damageValue, maxLevel, maxLevelEnhanced, startDuration, maxDuration, 75);
    }

    public TimedPotion(Potion potion, int requiredDamageValue, int damageValue, int maxLevel, int maxLevelEnhanced,
            int startDuration, int maxDuration, int durationStep) {
    ...
    public void onFirstBrewingFinished(ItemStack is) {
        ...
        if (eff != null) PotionTypes.setCustomPotionEffect(
                is,
                new PotionEffect(eff.getPotionID(), startDuration, eff.getAmplifier(), eff.getIsAmbient()));
    }
