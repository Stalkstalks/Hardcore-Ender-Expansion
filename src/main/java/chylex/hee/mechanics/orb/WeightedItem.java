...
public class WeightedItem implements IWeightProvider {
    ...
    /**
     * Runs a blacklist pattern on the item. If it only blacklists certain damage values, those will be removed. If no
     * damage values are left or the pattern blacklists the item itself, it will return true to signal it should be
     * removed. It will also return the amount of removed damage values.
     */
    ...
    public String toString() {
        return new StringBuilder().append("WeightedItemID: ITEM ").append(item.getUnlocalizedName()).append(", DMG ")
                .append(Arrays.toString(possibleDamageValues)).append(", WEIGHT ").append(weight).toString();
    }
}
