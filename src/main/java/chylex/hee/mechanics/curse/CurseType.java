...
public enum CurseType {
                    ...
                    if (!hasTeleported) entity.setPosition(prevX, prevY, prevZ);
                        ...
                        if (!entity.worldObj.isRemote) {
                            ...
                            PacketPipeline.sendToAllAround(
                                    ...
                                    256D,
                                    new C21EffectEntity(
                                            FXType.Entity.SIMPLE_TELEPORT_NOSOUND,
                                            prevX,
                                            prevY,
                                            prevZ,
                                            entity.width,
    ...
    CONFUSION(1, new ICurseHandler() {
        ...
        private EntityLiving getMob(EntityLiving me, double dist) {
            ...
            for (EntityLiving entity : (List<EntityLiving>) me.worldObj
                    .getEntitiesWithinAABB(EntityLiving.class, me.boundingBox.expand(dist, dist / 2, dist))) {
    ...
    TRANQUILITY(2, new ICurseHandler() {
        ...
        public boolean tickEntity(EntityLivingBase entity, ICurseCaller caller) {
            ...
            if (entity.getEntityAttribute(SharedMonsterAttributes.followRange).getModifier(noNavigation.getID())
                    == null) {
            ...
            if (timer == 0) timer = (byte) (21 + rand.nextInt(20));
                ...
                if (entity instanceof EntityPlayer) {
                    ...
                    while (!indexes.isEmpty()) {
                        ...
                        ItemStack is = index < 0 ? player.inventory.armorInventory[4 + index]
                                : player.inventory.mainInventory[index];
                    ...
                    while (!indexes.isEmpty()) {
                        ...
                        if (equipment[index] == null) continue;
                            ...
                            equipment[index]
                                    .setItemDamage(equipment[index].getItemDamageForDisplay() + 1 + rand.nextInt(3));
    ...
    VAMPIRE(8, new ICurseHandler() {
        ...
        public boolean tickEntity(EntityLivingBase entity, ICurseCaller caller) {
            if (entity.ticksExisted % 20 == 0) {
                if (entity.worldObj.isDaytime() && entity.getBrightness(1F) > 0.5F
                        && entity.worldObj.canBlockSeeTheSky(
                                MathUtil.floor(entity.posX),
                                MathUtil.floor(entity.posY),
                                MathUtil.floor(entity.posZ))) {
                    ...
                    while (!indexes.isEmpty()) {
                        ...
                        if (equipmentDropChances[index] <= 1F && is.isItemStackDamageable()) {
                            int maxDmg = Math.max(is.getMaxDamage() - 25, 1),
                                    dmg = is.getMaxDamage() - rand.nextInt(rand.nextInt(maxDmg) + 1);
    ...
    static {
        TELEPORTATION.setRecipe(Items.ender_pearl, Items.ender_pearl, Items.ender_pearl, Items.ender_pearl)
                .setUses(EnumCurseUse.BLOCK, 22, 34).setUses(EnumCurseUse.ENTITY, 7, 12)
                .setUses(EnumCurseUse.PLAYER, 3, 6).setColor1h(290).setColor2h(270);

        CONFUSION.setRecipe(Items.flint, new ItemStack(Items.dye, 1, 0), ItemList.instability_orb, Items.blaze_powder)
                .setUses(EnumCurseUse.BLOCK, 11, 15).setUses(EnumCurseUse.ENTITY, 6, 10).setColor1h(180).setColor2h(0);

        TRANQUILITY.setRecipe(Items.flint, new ItemStack(Items.dye, 1, 0), Items.string, Items.leather)
                .setUses(EnumCurseUse.BLOCK, 68 >> 1, 96 >> 1).setUses(EnumCurseUse.ENTITY, 150 >> 1, 210 >> 1)
                .setColor1h(180).setColor2h(210);

        SLOWNESS.setRecipe(Items.sugar, Items.sugar, Items.fermented_spider_eye, Items.fermented_spider_eye)
                .setUses(EnumCurseUse.BLOCK, 35, 42).setUses(EnumCurseUse.ENTITY, 16, 21)
                .setUses(EnumCurseUse.PLAYER, 5, 8).setColor1g(50).setColor2h(35);

        WEAKNESS.setRecipe(
                Items.blaze_powder,
                Items.blaze_powder,
                Items.fermented_spider_eye,
                Items.fermented_spider_eye).setUses(EnumCurseUse.BLOCK, 35, 42).setUses(EnumCurseUse.ENTITY, 16, 21)
                .setUses(EnumCurseUse.PLAYER, 5, 8).setColor1g(50).setColor2h(0);
        ...
        BLINDNESS
                .setRecipe(
                        ...
                        new ItemStack(Items.dye, 1, 0))
                .setUses(EnumCurseUse.BLOCK, 30, 39).setUses(EnumCurseUse.ENTITY, 12, 16)
                .setUses(EnumCurseUse.PLAYER, 3, 5).setColor1g(50).setColor2h(150);

        DEATH.setRecipe(Items.blaze_rod, ItemList.ectoplasm, ItemList.silverfish_blood, Items.bone)
                .setUses(EnumCurseUse.BLOCK, 28, 36).setUses(EnumCurseUse.ENTITY, 12, 16)
                .setUses(EnumCurseUse.PLAYER, 6, 9).setColor1g(40).setColor2g(57);

        DECAY.setRecipe(ItemList.stardust, ItemList.stardust, Items.iron_ingot, ItemList.instability_orb)
                .setUses(EnumCurseUse.BLOCK, 18, 25).setUses(EnumCurseUse.ENTITY, 14, 19)
                .setUses(EnumCurseUse.PLAYER, 9, 14).setColor1h(46).setColor2g(52);

        VAMPIRE.setRecipe(Items.glowstone_dust, Items.quartz, ItemList.igneous_rock, ItemList.silverfish_blood)
                .setUses(EnumCurseUse.BLOCK, 15, 22).setUses(EnumCurseUse.ENTITY, 8, 13)
                .setUses(EnumCurseUse.PLAYER, 6, 9).setColor1h(55).setColor2h(9);

        REBOUND.setRecipe(Items.ender_eye, Items.gold_nugget, Items.flint, Items.gold_nugget)
                .setUses(EnumCurseUse.BLOCK, 20, 26).setUses(EnumCurseUse.ENTITY, 15, 22)
                .setUses(EnumCurseUse.PLAYER, 12, 16).setColor1h(0).setColor2h(200);

        LOSS.setRecipe(ItemList.instability_orb, Items.book, Items.redstone, Items.emerald)
                .setUses(EnumCurseUse.BLOCK, 14, 19).setUses(EnumCurseUse.ENTITY, 10, 12)
    ...
    private CurseType setRecipe(Object o1, Object o2, Object o3, Object o4) {
        ...
        for (int a = 0; a < 4; a++) {
            if (array[a] instanceof Block) items[a] = new ItemStack((Block) array[a]);
                    ...
                    "Invalid recipe object, accepting only Block, Item and ItemStack, got " + array[a].getClass());
        }
    ...
    private CurseType setColor1(int hue, int saturation, int value) {
        ...
        this.color1 = (MathUtil.floor(rgb[0] * 255) << 16) | (MathUtil.floor(rgb[1] * 255) << 8)
                | MathUtil.floor(rgb[2] * 255);
        return this;
    }
    ...
    private CurseType setColor2(int hue, int saturation, int value) {
        float[] rgb = ColorUtil.hsvToRgb(hue / 360F, saturation / 100F, value / 100F);
        this.color2 = (MathUtil.floor(rgb[0] * 255) << 16) | (MathUtil.floor(rgb[1] * 255) << 8)
                | MathUtil.floor(rgb[2] * 255);
        return this;
    }
    ...
    public int getUses(EnumCurseUse useType, Random rand) {
        switch (useType) {
            case BLOCK:
                return usesBlockMin + rand.nextInt(usesBlockMax - usesBlockMin + 1);
            case ENTITY:
                return usesEntityMin + rand.nextInt(usesEntityMax - usesEntityMin + 1);
            case PLAYER:
                return usesPlayerMin + rand.nextInt(usesPlayerMax - usesPlayerMin + 1);
            default:
                return 0;
        }
    }
