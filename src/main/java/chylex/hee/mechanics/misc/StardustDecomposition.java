...
public final class StardustDecomposition {
            ...
            if (itemId[1].equals("*")) { // BLACKLIST ALL BLOCKS AND ITEMS FROM MOD
                ...
                for (String key : (Set<String>) GameData.getBlockRegistry().getKeys()) {
                    if (key.startsWith(identifier)) {
                        ...
                        if (block == null) {
                            if (itemId[0].equals("minecraft") || Loader.isModLoaded(itemId[0])) Log.warn(
                                    "Stumbled upon invalid entry in block registry while parsing Decomposition Blacklist, object not found: $0",
                                    key);
                            continue;
                        }
                ...
                for (String key : (Set<String>) GameData.getItemRegistry().getKeys()) {
                    if (key.startsWith(identifier)) {
                        ...
                        if (item == null) {
                            if (itemId[0].equals("minecraft") || Loader.isModLoaded(itemId[0])) Log.warn(
                                    "Stumbled upon invalid entry in item registry while parsing Decomposition Blacklist, object not found: $0",
                                    key);
                            continue;
                        }
    ...
    private static boolean isBlacklisted(ItemStack is) {
        ...
        return (blacklistedDamages != null && blacklistedDamages.length > 0
                && (blacklistedDamages[0] == -1
                        || ArrayUtils.contains(blacklistedDamages, (short) is.getItemDamage())));
    }
    ...
    public static List<ItemStack> getRandomRecipeIngredientsFor(ItemStack is, Random rand) {
        ...
        for (IRecipe recipe : (List<IRecipe>) CraftingManager.getInstance().getRecipeList()) {
            ...
            if (output == null || output.getItem() != item
                    || output.stackSize != is.stackSize
                    || (!is.isItemStackDamageable() && output.getItemDamage() != is.getItemDamage()))
                continue;
            ...
            if (recipe instanceof ShapedRecipes) {
                ...
                Object[] objs = recipe instanceof ShapedOreRecipe
                        ? DragonUtil.getNonNullValues(((ShapedOreRecipe) recipe).getInput())
                        : recipe instanceof ShapelessOreRecipe ? ((ShapelessOreRecipe) recipe).getInput().toArray()
                                : null;
        ...
        while (!ingredients.isEmpty()) {
            List<ItemStack> randRecipeIngredients = CollectionUtil
                    .newList(ingredients.remove(rand.nextInt(ingredients.size())));
