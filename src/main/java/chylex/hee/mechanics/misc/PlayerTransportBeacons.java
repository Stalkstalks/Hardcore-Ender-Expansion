...
public class PlayerTransportBeacons implements IExtendedEntityProperties {
    ...
    public static void register() {
        PlayerDataHandler
                .registerProperty(playerPropertyIdentifier, new IExtendedPropertyInitializer<PlayerTransportBeacons>() {
    ...
    public void loadNBTData(NBTTagCompound nbt) {
        ...
        for (int a = 0; a < list.tagCount(); a++) {
            ...
            if (loc.length == 2) locations.add(new LocationXZ(loc[0], loc[1]));
                    ...
                    "Corrupted data found when loading transport beacon, expected an int array of size 2, got $0",
                    loc.length);
        }
    }
