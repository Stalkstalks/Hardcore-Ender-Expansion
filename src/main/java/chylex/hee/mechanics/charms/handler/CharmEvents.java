...
public final class CharmEvents {
    ...
    private final AttributeModifier attrSpeed = new AttributeModifier(
    ...
    public void onPlayerTick(PlayerTickEvent e) {
        ...
        if (e.phase == Phase.START) {
            ...
            if (MathUtil.floatEquals(prevSpd, playerSpeed.getNoEntryValue()) || !MathUtil.floatEquals(prevSpd, spd)) {
                IAttributeInstance attribute = e.player.getAttributeMap()
                        .getAttributeInstance(SharedMonsterAttributes.movementSpeed);
                ...
                if (attribute != null) {
                    ...
                    attribute.applyModifier(
                            new AttributeModifier(
                                    attrSpeed.getID(),
                                    attrSpeed.getName() + spd,
                                    attrSpeed.getAmount() * spd,
                                    attrSpeed.getOperation()));
                }
            ...
            if (playerLastResortCooldown.containsKey(playerID)) {
                if (playerLastResortCooldown.adjustOrPutValue(playerID, (byte) -1, (byte) -100) <= -100)
                    playerLastResortCooldown.remove(playerID);
            }
        }
    }

    /**
     * BASIC_POWER, BASIC_DEFENSE, EQUALITY, BLOCKING, BLOCKING_REFLECTION, BLOCKING_REPULSION, CRITICAL_STRIKE,
     * FALLING_PROTECTION, WITCHERY_HARM, DAMAGE_REDIRECTION, MAGIC_PENETRATION, LIFE_STEAL, LAST_RESORT It is not
     * called on client side, check not needed.
     */
    @SubscribeEvent(priority = EventPriority.LOWEST)
        ...
        if (e.source.getEntity() == null) {
            ...
            if (isSourcePlayer) {
                ...
                if (crit.length > 0) {
                    ...
                    if (val > 0F) {
                        ...
                        PacketPipeline.sendToAllAround(
                                e.entity,
                                64,
                                new C21EffectEntity(FXType.Entity.CHARM_CRITICAL, e.entity));
                    }
                }
                ...
                if (badEff.length > 0) {
                    ...
                    List<Potion> potionEffects = CollectionUtil
                            .newList(Potion.weakness, Potion.moveSlowdown, Potion.blindness, Potion.poison, null // null
                                                                                                                 // =
                                                                                                                 // fire
                            );
                    ...
                    for (int a = 0; a < badEff.length && !potionEffects.isEmpty(); a++) {
                        if (e.entity.worldObj.rand.nextFloat() < badEff[a]) {
                            ...
                            else e.entityLiving.addPotionEffect(
                                    new PotionEffect(type.id, 20 * (int) badEffTime[a], (int) badEffLvl[a] - 1));

                            causedEffect = true;
                        }
                    }

                    if (causedEffect) PacketPipeline
                            .sendToAllAround(e.entity, 64, new C21EffectEntity(FXType.Entity.CHARM_WITCH, e.entity));
                }
            ...
            if (isTargetPlayer) {
                ...
                if (targetPlayer.isBlocking()) {
                    ...
                    if (repulseAmt > 0.001F) {
                        ...
                        double[] vec = DragonUtil
                                .getNormalizedVector(source.posX - targetPlayer.posX, source.posZ - targetPlayer.posZ);
                        vec[0] *= mp;
                        vec[1] *= mp;
                        ...
                        if (source instanceof EntityPlayer) {
                            PacketPipeline.sendToPlayer(
                                    (EntityPlayer) source,
                                    new C07AddPlayerVelocity(vec[0], 0.25D, vec[1]));
                    ...
                    if (showBlockingEffect) PacketPipeline.sendToAllAround(
                            e.entity,
                            64,
                            new C21EffectEntity(FXType.Entity.CHARM_BLOCK_EFFECT, e.entity));
                }
                ...
                if (redirMobs.length > 0) {
                    ...
                    List<EntityLivingBase> nearbyEntities = e.entity.worldObj
                            .getEntitiesWithinAABB(EntityLivingBase.class, targetPlayer.boundingBox.expand(6D, 3D, 6D));
                        ...
                        for (int mob = 0; mob < Math.round(redirMobs[a]); mob++) {
                            while (iter.hasNext()) {
                                ...
                                entity.attackEntityFrom(
                                        DamageSource.causePlayerDamage(targetPlayer),
                                        redirAmt[a] * e.ammount);
                                PacketPipeline.sendToAllAround(
                                        targetPlayer,
                                        64D,
                                        new C22EffectLine(FXType.Line.CHARM_DAMAGE_REDIRECTION, entity, targetPlayer));
            ...
            if (isSourcePlayer) {
                ...
                if (stealHealth.length > 0) {
                    ...
                    if (playerStealDealtDamage
                            .adjustOrPutValue(sourcePlayer.getGameProfile().getId(), e.ammount, e.ammount)
                            >= stealDealt[randIndex]) {
    ...
    /**
     * SLAUGHTER_IMPACT, LAST_RESORT It is not called on client side, check not needed.
     */
    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onLivingDeath(LivingDeathEvent e) {
        if (e.entity instanceof EntityPlayer) {
            ...
            if (lastResortCooldown.length > 0
                    && !playerLastResortCooldown.containsKey(targetPlayer.getGameProfile().getId())) {
                ...
                for (int attempt = 0; attempt < 128; attempt++) {
                    ...
                    for (int yAttempt = 0, origY = tmpPos.y; yAttempt <= 6; yAttempt++) {
                        if (!tmpPos.setY(origY - 1).isAir(targetPlayer.worldObj)
                                && tmpPos.setY(origY).isAir(targetPlayer.worldObj)
                                && tmpPos.setY(origY + 1).isAir(targetPlayer.worldObj)) {
                            PacketPipeline.sendToAllAround(
                                    targetPlayer,
                                    64D,
                                    new C21EffectEntity(FXType.Entity.CHARM_LAST_RESORT, targetPlayer));
                ...
                playerLastResortCooldown
                        .put(targetPlayer.getGameProfile().getId(), (byte) (-100 + lastResortCooldown[randIndex] * 20));
        ...
        if (e.source.getEntity() instanceof EntityPlayer) {
            ...
            if (impactRad.length > 0) {
                ...
                float lastDamage = ReflectionUtils.getFieldValue(e.entityLiving, "lastDamage");

                for (int a = 0; a < impactRad.length; a++) {
                    List<EntityLivingBase> entities = e.entity.worldObj.getEntitiesWithinAABB(
                            EntityLivingBase.class,
                            e.entity.boundingBox.expand(impactRad[a], impactRad[a], impactRad[a]));

                    for (EntityLivingBase entity : entities) {
                        if (entity == sourcePlayer || entity == e.entity
                                || (entity instanceof EntityPlayer
                                        && !sourcePlayer.canAttackPlayer((EntityPlayer) entity)))
                            continue;
                        if (entity.getDistanceToEntity(e.entity) <= impactRad[a]) {
                            ...
                            PacketPipeline.sendToAllAround(
                                    e.entity,
                                    64D,
                                    new C22EffectLine(FXType.Line.CHARM_SLAUGHTER_IMPACT, entity, e.entity));
                        }
                    }
                }
            }
    ...
    /**
     * BASIC_MAGIC, EQUALITY It is not called on client side, check not needed.
     */
    @SubscribeEvent(priority = EventPriority.LOWEST)
    public void onLivingDrops(LivingDropsEvent e) {
        if (e.recentlyHit && e.source.getEntity() instanceof EntityPlayer
                && e.entityLiving instanceof EntityLiving
                && !e.entityLiving.isChild()
                && e.entity.worldObj.getGameRules().getGameRuleBooleanValue("doMobLoot")) {
            // BASIC_MAGIC / EQUALITY
            int xp = (int) ReflectionPublicizer.invoke(
                    ReflectionPublicizer.entityLivingBaseGetExperiencePoints,
                    e.entityLiving,
                    (EntityPlayer) e.source.getEntity());
    ...
    public void onBlockBreak(BreakEvent e) {
        ...
        // BASIC_MAGIC / EQUALITY
        e.setExpToDrop(
                e.getExpToDrop() + MathUtil.ceil(getPropPercentIncrease(e.getPlayer(), "exp", e.getExpToDrop())));
    }
    ...
    public void onItemDestroyed(PlayerDestroyItemEvent e) {
        if (e.entity == null || e.entity.worldObj == null || e.entity.worldObj.isRemote) return; // NPE checks for some
                                                                                                 // Thaumcraft bullshit
        ...
        if (e.original.isItemStackDamageable() && e.original.getItem().isRepairable()) {
            ...
            if (repair.length > 0) {
                ...
                newIS.stackSize = 1;
                newIS.setItemDamage(
                        newIS.getMaxDamage() - MathUtil.floor(newIS.getMaxDamage() * Math.min(1F, toRepair)));

                EntityItem newItem = new EntityItem(
                        e.entity.worldObj,
                        e.entity.posX,
                        e.entity.posY + e.entityPlayer.getEyeHeight() - 0.3D,
                        e.entity.posZ,
                        newIS);
                newItem.delayBeforeCanPickup = 40;

                float power = 0.3F, yawRadians = (float) Math.toRadians(e.entityPlayer.rotationYaw),
                        randomAngle = e.entity.worldObj.rand.nextFloat() * (float) Math.PI * 2F;
