...
public final class EnergyClusterData {
    ...
    public void generate(World world, int blockX, int blockZ) {
        if (world.provider.dimensionId == 1) {
            ...
            for (int a = 0; a < 4; a++) average += file.getFromChunkCoords(
                    world,
                    chunkX + Direction.offsetX[a] * EnergySavefile.sectionSize,
                    chunkZ + Direction.offsetZ[a] * EnergySavefile.sectionSize,
                    false).getEnergyLevel();
    ...
    public void update(TileEntityEnergyCluster cluster) {
        ...
        if (energyLevel > 0.1F && energyLevel / maxEnergyLevel > 0.85F + rand.nextFloat() * 2.5F
                && rand.nextInt(75) == 0) {
            ...
            for (int attempt = 0, placed = 0; attempt < 8 && placed < 4; attempt++) {
                tmpPos.set(
                        cluster.xCoord + rand.nextInt(7) - 3,
                        cluster.yCoord + rand.nextInt(7) - 3,
                        cluster.zCoord + rand.nextInt(7) - 3);
        ...
        float regenTimMp = energyLevel < maxEnergyLevel * 0.5F
                ? 1F + (1F - (energyLevel / (maxEnergyLevel * 0.5F))) * 2F
                : 1F; // slow down regen if below half max energy

        if (healthStatus.regenTimer != -1
                && ++regenTimer > Math.min((int) (healthStatus.regenTimer * regenTimMp), 125)) {
            energyLevel += Math.min(
                    (EnergyChunkData.energyDrainUnit * 0.08F + (float) Math.sqrt(maxEnergyLevel) * 0.005F)
                            * healthStatus.regenMultiplier,
                    maxEnergyLevel - energyLevel);
            cluster.synchronize();
            regenTimer = 0;
        }

        if (world.provider.dimensionId == 1 && rand.nextInt(healthStatus.ordinal() + 1) == 0
                && ++drainTimer > 10 + rand.nextInt(70)) {
            drainTimer = 0;

            EnergyChunkData environment = WorldDataHandler.<EnergySavefile>get(EnergySavefile.class)
                    .getFromBlockCoords(world, cluster.xCoord, cluster.zCoord, true);
    ...
    public void weakenCluster() {
        if (healthStatus.ordinal() < EnergyClusterHealth.values.length)
            healthStatus = EnergyClusterHealth.values[healthStatus.ordinal() + 1];
    }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        healthStatus = status >= 0 && status < EnergyClusterHealth.values.length ? EnergyClusterHealth.values[status]
                : EnergyClusterHealth.HEALTHY;
