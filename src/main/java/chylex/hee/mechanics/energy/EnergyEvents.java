...
public final class EnergyEvents {
    ...
    public void onWorldTick(WorldTickEvent e) {
        if (e.phase != Phase.START || e.world.provider.dimensionId != 1 || e.world.isRemote || ++updateTimer <= 4)
            return;
        ...
        for (Chunk chunk : chunks) {
            ...
            for (int a = 0; a < 4; a++) {
                data.onAdjacentInteract(
                        e.world.rand,
                        file.getFromChunkCoords(
                                e.world,
                                chunk.xPosition + Direction.offsetX[a] * EnergySavefile.sectionSize,
                                chunk.zPosition + Direction.offsetZ[a] * EnergySavefile.sectionSize,
                                true));
            }
    ...
    public void onLivingDeath(LivingDeathEvent e) {
        ...
        WorldDataHandler.<EnergySavefile>get(EnergySavefile.class)
                .getFromBlockCoords(
                        e.entity.worldObj,
                        MathUtil.floor(e.entity.posX),
                        MathUtil.floor(e.entity.posZ),
                        true)
                .addEnergy(energy);
    }
