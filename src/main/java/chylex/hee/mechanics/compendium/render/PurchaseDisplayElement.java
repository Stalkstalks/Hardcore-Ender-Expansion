...
public class PurchaseDisplayElement {
    ...
    public void render(GuiScreen gui, int mouseX, int mouseY, int pageCenterX) {
        ...
        GuiEnderCompendium.renderItem.renderItemIntoGUI(
                gui.mc.fontRenderer,
                gui.mc.getTextureManager(),
                GuiEnderCompendium.knowledgeFragmentIS,
                pageCenterX - 22,
                y - 10);
        RenderHelper.disableStandardItemLighting();
        ...
        int color = status == FragmentPurchaseStatus.CAN_PURCHASE ? 0x404040
                : (status == FragmentPurchaseStatus.REQUIREMENTS_UNFULFILLED
                        || status == FragmentPurchaseStatus.NOT_BUYABLE) ? 0x888888
                                : status == FragmentPurchaseStatus.NOT_ENOUGH_POINTS ? 0xdd2020 : 0;
        gui.mc.fontRenderer
                .drawString(price, pageCenterX - gui.mc.fontRenderer.getStringWidth(price) + 20, y - 5, color);
        ...
        if (object instanceof KnowledgeObject) {
            ...
            for (int a = 0, yy = y - 25 - (parsed.size() - 1) * gui.mc.fontRenderer.FONT_HEIGHT; a
                    < parsed.size(); a++) {
                gui.mc.fontRenderer.drawString(
                        parsed.get(a),
                        pageCenterX - (gui.mc.fontRenderer.getStringWidth(parsed.get(a)) >> 1),
                        yy,
                        0x404040);
            ...
            if (status == FragmentPurchaseStatus.NOT_BUYABLE) {
                ...
                if (fragmentHasRedirect) tooltip = new StringBuilder(tooltip).append('\n')
                        .append(EnumChatFormatting.DARK_PURPLE).append(I18n.format("compendium.viewObject")).append(' ')
                        .append(((KnowledgeFragment) object).getUnlockRedirect().getTooltip()).toString();
    ...
    public boolean isMouseOver(int mouseX, int mouseY, int pageCenterX) {
        return mouseX >= (pageCenterX + 3) - 27 && mouseY >= y - 14
                && mouseX <= (pageCenterX + 3) + 27
                && mouseY <= y + 12;
    }
