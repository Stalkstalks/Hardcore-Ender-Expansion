...
public final class CompendiumEventsClient {
    ...
    public static boolean canOpenCompendium() {
        if (instance.data == null) {
            Minecraft.getMinecraft().thePlayer
                    .addChatMessage(new ChatComponentText(I18n.format("compendium.dataError")));
            return false;
        } else return true;
    }
    ...
    private CompendiumEventsClient() {
        keyOpenCompendium = new KeyBinding(
                ModCommonProxy.hardcoreEnderbacon ? "key.openCompendium.bacon" : "key.openCompendium",
                25,
    ...
    public void onPlayerLogin(PlayerLoggedInEvent e) {
        ...
        for (KeyBinding kb : Minecraft.getMinecraft().gameSettings.keyBindings) {
            if (isKeybindingValid() && kb != instance.keyOpenCompendium
                    && kb.getKeyCode() == instance.keyOpenCompendium.getKeyCode()) {
                HardcoreEnderExpansion.notifications.report(
                        I18n.format("key.openCompendium.conflict").replace("$", I18n.format(kb.getKeyDescription())));
                break;
            }
        }
    ...
    public void onClientTick(ClientTickEvent e) {
        ...
        if (achievementTimer > Byte.MIN_VALUE && --achievementTimer == Byte.MIN_VALUE)
            Minecraft.getMinecraft().guiAchievement.func_146257_b();

        if (isKeybindingValid()
                && (keyOpenCompendium.isPressed()
                        || Keyboard.getEventKeyState() && Keyboard.getEventKey() == keyOpenCompendium.getKeyCode())
                && (mc.inGameHasFocus || mc.currentScreen instanceof GuiContainer)) {
            if (canOpenCompendium()) {
                ...
                if (mc.inGameHasFocus) {
                    ...
                    int mouseX = Mouse.getX() * res.getScaledWidth() / mc.displayWidth,
                            mouseY = res.getScaledHeight() - Mouse.getY() * res.getScaledHeight() / mc.displayHeight
                                    - 1;
                    ...
                    for (Slot slot : slots) {
                        if (slot.getHasStack() && slot.func_111238_b()
                                && mouseX >= slot.xDisplayPosition - 1
                                && mouseX <= slot.xDisplayPosition + 16
                                && mouseY >= slot.yDisplayPosition - 1
                                && mouseY <= slot.yDisplayPosition + 16) {
