...
public final class CompendiumEvents implements IExtendedPropertyInitializer<PlayerCompendiumData> {
    ...
    public static KnowledgeObservation getObservation(EntityPlayer player) {
        ...
        Vec3 posVec = Vec3.createVectorHelper(
                player.posX,
                (player.worldObj.isRemote ? 1.5D : 0D) + player.boundingBox.minY
                        + player.getEyeHeight()
                        - (player.isSneaking() ? 0.08D : 0D),
                player.posZ);
        Vec3 lookVec = player.getLookVec();

        MovingObjectPosition mopBlock = player.worldObj.rayTraceBlocks(
                posVec.addVector(0D, 0D, 0D),
                posVec.addVector(lookVec.xCoord * 10D, lookVec.yCoord * 10D, lookVec.zCoord * 10D),
                true);
        double distBlock = mopBlock != null && mopBlock.typeOfHit == MovingObjectType.BLOCK ? MathUtil.distance(
                mopBlock.blockX + 0.5D - posVec.xCoord,
                mopBlock.blockY + 0.5D - posVec.yCoord,
                mopBlock.blockZ + 0.5D - posVec.zCoord) : Double.MAX_VALUE;

        double bbX = posVec.xCoord + lookVec.xCoord * 5D, bbY = posVec.yCoord + lookVec.yCoord * 5D,
                bbZ = posVec.zCoord + lookVec.zCoord * 5D;
        List<Entity> list = player.worldObj.getEntitiesWithinAABB(
                Entity.class,
                AxisAlignedBB.getBoundingBox(bbX - 6D, bbY - 6D, bbZ - 6D, bbX + 6D, bbY + 6D, bbZ + 6D));
        ...
        for (Entity entity : list) {
            ...
            MovingObjectPosition mop = entity.boundingBox.expand(size, size, size)
                    .calculateIntercept(posVec, interceptVec);
        ...
        if (distBlock < distEntity && mopBlock != null) {
            ...
            BlockMetaWrapper wrapper = new BlockMetaWrapper(
                    tmpPos.getBlock(player.worldObj),
                    tmpPos.getMetadata(player.worldObj));
            ...
            if (tracedEntity instanceof EntityLiving)
                observationReuse.setMob(KnowledgeObject.<ObjectMob>getObject((EntityLiving) tracedEntity));
