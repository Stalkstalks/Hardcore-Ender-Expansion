...
public class KnowledgeFragmentEnhancement extends KnowledgeFragment {
    ...
    public boolean onClick(GuiEnderCompendium gui, int x, int y, int mouseX, int mouseY, int buttonId,
        ...
        if (isUnlocked && buttonId == 0 && mouseX >= x && mouseX <= x + 17 && mouseY >= y && mouseY <= y + 17) {
            KnowledgeObject<? extends IKnowledgeObjectInstance<?>> obj = KnowledgeUtils
                    .tryGetFromItemStack(enhancement.getItemSelector().getRepresentativeItem());
    ...
    public void onRender(GuiEnderCompendium gui, int x, int y, int mouseX, int mouseY, boolean isUnlocked) {
        ...
        ItemStack is = isUnlocked ? enhancement.getItemSelector().getRepresentativeItem()
                : KnowledgeFragmentCrafting.lockedItem;
