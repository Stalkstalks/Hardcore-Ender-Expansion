...
public final class KnowledgeObservation {
    ...
    public void discover(EntityPlayer player) {
        ...
        switch (type) {
            case 1:
                result = CompendiumEvents.getPlayerData(player)
                        .tryDiscoverBlock((KnowledgeObject<ObjectBlock>) object, true);
                break;
            case 2:
                result = CompendiumEvents.getPlayerData(player)
                        .tryDiscoverItem((KnowledgeObject<ObjectItem>) object, true);
                break;
            case 3:
                result = CompendiumEvents.getPlayerData(player)
                        .tryDiscoverMob((KnowledgeObject<ObjectMob>) object, true);
                break;
            default:
                return;
        }
