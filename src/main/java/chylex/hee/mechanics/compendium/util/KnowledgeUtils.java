...
public final class KnowledgeUtils {
    ...
    public static KnowledgeObject<? extends IKnowledgeObjectInstance<?>> tryGetFromItemStack(ItemStack is) {
        ...
        if (uniqueId != null && uniqueId.modId.equalsIgnoreCase("hardcoreenderexpansion")) {
            if (is.getItem() == ItemList.spawn_eggs) {
                ...
                if (entity == null)
                    entity = (Class<? extends EntityLiving>) EntityList.IDtoClassMapping.get(is.getItemDamage());
    ...
    public static List<String> getCompendiumTooltip(ItemStack is, EntityPlayer player) {
        ...
        if (KnowledgeUtils.tryGetFromItemStack(is) != null)
            tooltip.add(EnumChatFormatting.DARK_PURPLE + I18n.format("compendium.viewObject"));
        return tooltip;
    }
            ...
            for (CharmRecipe recipe : type.recipes) {
                fragments.add(
                        new KnowledgeFragmentCharm(startID + (a++)).setRecipe(recipe).setPrice(2)
                                .setUnlockRequirements(requiredFragment));
            }
        }
    ...
    public static KnowledgeFragment[] createEnhancementFragments(Class<? extends Enum> enhancementCls, int startID,
            int points, int maxUsedIDs, int[] requiredFragments) {
        Enum[] enums = enhancementCls.getEnumConstants();
        if (enums.length > maxUsedIDs) throw new RuntimeException(
                "Enhancement " + enhancementCls.getName() + " exceeds fragment ID limit (" + maxUsedIDs + ").");

        KnowledgeFragment[] fragments = new KnowledgeFragment[enums.length];
        for (int a = 0; a < enums.length; a++) fragments[a] = new KnowledgeFragmentEnhancement(startID + a)
                .setEnhancement((IEnhancementEnum) enums[a]).setPrice(points).setUnlockRequirements(requiredFragments);
        return fragments;
    }
