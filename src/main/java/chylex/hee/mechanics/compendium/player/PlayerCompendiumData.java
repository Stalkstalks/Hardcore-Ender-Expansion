...
public class PlayerCompendiumData implements IExtendedEntityProperties {
    ...
    private final PlayerDiscoveryList<ObjectBlock, BlockMetaWrapper> discoveredBlocks = new PlayerDiscoveryList<>(
            new DiscoveryBlockSerializer());
    private final PlayerDiscoveryList<ObjectItem, Item> discoveredItems = new PlayerDiscoveryList<>(
            new DiscoveryItemSerializer());
    private final PlayerDiscoveryList<ObjectMob, Class<? extends EntityLiving>> discoveredMobs = new PlayerDiscoveryList<>(
            new DiscoveryMobSerializer());
    private final PlayerDiscoveryList<ObjectDummy, String> discoveredMisc = new PlayerDiscoveryList<>(
            new DiscoveryStringSerializer());
    ...
    public boolean tryDiscoverObject(KnowledgeObject<?> object, boolean addReward) {
        ...
        else if (obj instanceof ObjectDummy)
            return discoveredMisc.addObject(((KnowledgeObject<ObjectDummy>) object).getObject());
        else return false;
    }
    ...
    private static class DiscoveryBlockSerializer implements IObjectSerializer<BlockMetaWrapper> {
        ...
        public BlockMetaWrapper deserialize(String data) {
            ...
            else return new BlockMetaWrapper(
                    GameData.getBlockRegistry().getObject(data.substring(0, secondColonIndex)),
