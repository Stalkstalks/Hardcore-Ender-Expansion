...
public class DragonEssenceHandler extends AltarActionHandler {
    ...
    public static final List<AltarItemRecipe> recipes = CollectionUtil.newList(
            new AltarItemRecipe[] {
                    new AltarItemRecipe(
                    ...
                    new AltarItemRecipe(
                            new ItemStack(ItemList.ghost_amulet, 1, 0),
    ...
    public void onUpdate() {
            ...
            for (int xx = -range, id; xx <= range; xx++) {
                for (int zz = -range; zz <= range; zz++) {
                    id = Block.getIdFromBlock(
                            BlockPosM.tmp(altar.xCoord + xx, altar.yCoord, altar.zCoord + zz).getBlock(world));
                ...
                for (int xx = altar.xCoord - range; xx <= altar.xCoord + range; xx++) {
                    for (int zz = altar.zCoord - range; zz <= altar.zCoord + range; zz++) {
                        ...
                        if (tmpPos.set(xx, altar.yCoord + 1, zz).isAir(world)
                                && (tmpPos.set(xx - 1, altar.yCoord, zz).isAir(world)
                                        || !hasCollisionBox(altar, xx - 1, altar.yCoord, zz))
                                && (tmpPos.set(xx + 1, altar.yCoord, zz).isAir(world)
                                        || !hasCollisionBox(altar, xx + 1, altar.yCoord, zz))
                                && (tmpPos.set(xx, altar.yCoord, zz - 1).isAir(world)
                                        || !hasCollisionBox(altar, xx, altar.yCoord, zz - 1))
                                && (tmpPos.set(xx, altar.yCoord, zz + 1).isAir(world)
                                        || !hasCollisionBox(altar, xx, altar.yCoord, zz + 1))
                            ...
                            if (blockCounts.containsKey(block))
                                blockCounts.put(block, (byte) (blockCounts.get(block) + 1));
            ...
            for (BlockPosM loc : pedestals) {
                if (world.rand.nextInt(5) <= 1) {
                    PacketPipeline.sendToAllAround(
                            altar,
                            64D,
                            new C11ParticleAltarOrb(altar, loc.x + 0.5D, altar.yCoord + 0.5D, loc.z + 0.5D));
                }
            }
        }

        if (itemBoundingBox == null) {
            itemBoundingBox = AxisAlignedBB.getBoundingBox(
                    altar.xCoord + 0.5D - 4.5D,
                    altar.yCoord + 0.9D,
                    altar.zCoord + 0.5D - 4.5D,
                    altar.xCoord + 0.5 + 4.5D,
                    altar.yCoord + 1.6D,
                    altar.zCoord + 0.5D + 4.5D);
        }
        ...
        for (EntityItem item : thrownItems) {
            for (BlockPosM loc : pedestals) {
                ...
                if (Math.abs(item.posX - targX) > 0.001D || Math.abs(item.posY - targY) > 0.001D
                        || Math.abs(item.posZ - targZ) > 0.001D) {
                    if (world
                            .getEntitiesWithinAABB(
                                    EntityItemAltar.class,
                                    AxisAlignedBB.getBoundingBox(targX, targY, targZ, targX, targY, targZ))
                            .isEmpty()
                            && Math.sqrt(
                                    MathUtil.square(targX - item.posX) + MathUtil.square(targY - item.posY)
                                            + MathUtil.square(targZ - item.posZ))
                                    < 0.275D) {
                        world.spawnEntityInWorld(
                                new EntityItemAltar(world, targX, targY, targZ, item, EssenceType.DRAGON.id));
                    }
    ...
    public static boolean hasCollisionBox(TileEntityEssenceAltar altar, int x, int y, int z) {
        ...
        return block.getMaterial() == Material.air ? false
                : block.getCollisionBoundingBoxFromPool(altar.getWorldObj(), x, y, z) != null;
    }
        ...
        if (item.worldObj.rand.nextInt(3) != 0) {
            if (is.isItemStackDamageable() && is.getItemDamage() != 0 && is.getItem().isRepairable()) {
                for (int a = enhancements.contains(EssenceAltarEnhancements.SPEED) ? 2 : 1; a > 0; a--) {
                    ...
                    int amount = MathUtil
                            .clamp(MathUtil.floor(Math.sqrt(is.getMaxDamage()) * 0.65D), 1, is.getItemDamage());
            ...
            for (int b = enhancements.contains(EssenceAltarEnhancements.SPEED) ? 2 : 1; b > 0; b--) {
                ...
                for (int attempt = 0; attempt < 3; attempt++) {
                    ...
                    final ObjectWeightPair<?>[] eligibleEnchants = IntStream.range(0, enchants.tagCount())
                            ...
                            .filter(Objects::nonNull).map(e -> ObjectWeightPair.of(e, e.getWeight()))
         ...
         */
            ...
            for (AltarItemRecipe recipe : recipes) {
                if (recipe.isApplicable(is)) {
                    for (int a = enhancements.contains(EssenceAltarEnhancements.SPEED) ? 2 : 1; a > 0; a--) {
                        if (updateItemCounter(is, "HEE_transform", 1) <= Math.max(
                                MathUtil.ceil(
                                        recipe.cost
                                                * (enhancements.contains(EssenceAltarEnhancements.EFFICIENCY) ? 0.65F
                                                        : 1F)),
                                recipe.cost >> 1)) {
                            altar.drainEssence(1);
                            continue;
    ...
    private int getEnchantmentCost(Enchantment ench, int level) {
        return MathUtil.floor(
                Math.max(
                        1F,
                        1F + (2F * level * ((float) level / ench.getMaxLevel())) + (10 - ench.getWeight()) * 0.2F));
    }
