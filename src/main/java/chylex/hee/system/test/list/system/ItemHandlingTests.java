...
public class ItemHandlingTests {
    ...
    public void testItemPatternName() {
        ...
        Assert.equal(
                new ItemPattern().setItemName("HardcoreEnderExpansion", "*").retainMatching(patternTestList).size(),
                2,
                matchError);
        Assert.equal(
                new ItemPattern().setItemName("minecraft", "dirt").retainMatching(patternTestList).size(),
                3,
                matchError);
        Assert.equal(
                new ItemPattern().setItemName("minecraft", "sand").retainMatching(patternTestList).size(),
                0,
                matchError);
    }
    ...
    public void testItemPatternDamage() {
        Assert.equal(
                new ItemPattern().setDamageValues(new int[] { 1 }).retainMatching(patternTestList).size(),
                1,
                matchError);
        Assert.equal(
                new ItemPattern().setDamageValues(new int[] { 1, 2 }).retainMatching(patternTestList).size(),
                2,
                matchError);
    }
    ...
    public void testItemPatternNBT() {
        Assert.equal(
                new ItemPattern().setNBT((NBTTagCompound) patternTestNbt.copy()).retainMatching(patternTestList).size(),
                1,
                matchError);
    }
