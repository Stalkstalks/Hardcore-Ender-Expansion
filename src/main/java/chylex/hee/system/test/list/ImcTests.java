...
public class ImcTests {
    ...
    public void prepareImcs() {
        for (String msgs : new String[] {
                ...
                "HEE:Orb:ItemBlacklist { 'pattern': { 'id': '*' } }", "HEE:Orb:MobAdd { 'id': 'Villager' }",
    ...
    public void testImcsDragonEssence() {
        ...
        Assert.equal(
                DragonEssenceHandler.recipes.get(1).input.getItem(),
                Items.ender_eye,
                "Unexpected second entry, expected $2, got $1. Full list: " + DragonEssenceHandler.recipes);
    ...
    public void testImcsMobs() {
        Assert.state(
                GlobalMobData.isEnderGooTolerant(new EntityBlaze(null)),
                "Expected Blaze to be marked as a Goo tolerant mob.");
        Assert.equal(
                EnergyValues.getMobEnergy(new EntityMobVampiricBat(null)),
                EnergyChunkData.energyDrainUnit * 2.5F,
                "Unexpected mob Energy value, expected $2, got $1.");
    }

    @UnitTest(type = MethodType.TEST, runTime = RunTime.LOADCOMPLETE)
    public void testImcsOrb() {
        Assert.state(
                OrbSpawnableMobs.classList.contains(EntityVillager.class),
                "Expected Villager to have been added to mob list.");
        Assert.state(
                !OrbSpawnableMobs.classList.contains(EntityWither.class),
                "Expected Wither to NOT have been added to mob list.");
        Assert.state(
                !OrbSpawnableMobs.classList.contains(EntityCreeper.class),
                "Expected Creeper to have been removed from mob list.");
    }

    @UnitTest(type = MethodType.TEST, runTime = RunTime.LOADCOMPLETE)
    public void testImcTables() {
        Assert.isNull(
                StardustDecomposition.getRandomRecipeIngredientsFor(new ItemStack(Blocks.dispenser), new Random()),
                "Unexpected ingredient list, expected null, got $.");
        Assert.notNull(
                StardustDecomposition.getRandomRecipeIngredientsFor(new ItemStack(Items.bow), new Random()),
                "Unexpected ingredient list, got null.");

        Assert.equal(
                EnergyValues.getItemEnergy(new ItemStack(Items.coal, 1, 1)),
                EnergyChunkData.energyDrainUnit * 5.4F,
                "Unexpected item Energy value, expected $2, got $1.");
        Assert.equal(
                EnergyValues.getItemEnergy(new ItemStack(Items.coal, 1, 0)),
                0F,
                "Unexpected item Energy value, expected $2, got $1."); // make sure it only takes charcoal

        Assert.equal(
                TileEntityExperienceTable.getDirectExperience(new ItemStack(Items.coal, 1, 0)),
                12,
                "Unexpected item Experience value, expected $2, got $1.");
        Assert.equal(
                TileEntityExperienceTable.getDirectExperience(new ItemStack(Items.coal, 1, 1)),
                12,
                "Unexpected item Experience value, expected $2, got $1."); // make sure it takes all damage values
    }

    @UnitTest(type = MethodType.TEST, runTime = RunTime.LOADCOMPLETE)
    public void testImcWorld() {
        Assert.equal(
                ComponentTower.lootFuel.size(),
                2,
                "Unexpected list size, expected $2, got $1. List: " + ComponentTower.lootFuel);
    ...
    public void testImcSystem() {
        Assert.state(
                ModIntegrationManager.blacklistedMods.contains("NotEnoughItems"),
                "Missing entry in mod integration blacklist, expected NotEnoughItems to be present.");
    }
}
