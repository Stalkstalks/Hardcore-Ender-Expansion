...
public class BlockTests {
    ...
    public void prepBlockChunk() {
        ...
        for (pos.x = 0; pos.x < 16; pos.x++) {
            for (pos.z = 0; pos.z < 16; pos.z++) {
                for (pos.y = 4; pos.y < 128; pos.y++) {
                    if (pos.getBlock(world) == BlockList.energy_cluster)
                        ((TileEntityEnergyCluster) pos.getTileEntity(world)).shouldNotExplode = true;
                    pos.setAir(world);
                }
            }
        }
        ...
        for (int run = 0; run < 2; run++) {
            ...
            if (run == 1)
                getTile(TileEntityEssenceAltar.class).getEnhancements().add(EssenceAltarEnhancements.EFFICIENCY);
        ...
        for (Object[] t : potions) {
            ...
            getTile(TileEntityEnhancedBrewingStand.class)
                    .setInventorySlotContents(1, new ItemStack(Items.potionitem, 1, (Integer) t[0]));
            getTile(TileEntityEnhancedBrewingStand.class)
                    .setInventorySlotContents(3, t[1] instanceof Item ? new ItemStack((Item) t[1]) : (ItemStack) t[1]);
            if ((Integer) t[2] != 0) getTile(TileEntityEnhancedBrewingStand.class)
                    .setInventorySlotContents(4, new ItemStack(ItemList.end_powder, (Integer) t[2]));
            if (t.length > 3) getTile(TileEntityEnhancedBrewingStand.class).getEnhancements()
                    .add(EnhancedBrewingStandEnhancements.TIER);
            move();
        }
        ...
        ItemStack[] decompositionTable = new ItemStack[] { new ItemStack(Items.diamond_chestplate),
                new ItemStack(Items.diamond_chestplate, 1, 528), new ItemStack(Blocks.planks, 4),
                new ItemStack(ItemList.energy_wand), new ItemStack(BlockList.void_chest), new ItemStack(Blocks.beacon),
        ...
        for (int a = -3; a <= 3; a++) {
            ...
            getTile(TileEntityDecompositionTable.class)
                    .setInventorySlotContents(1, new ItemStack(ItemList.stardust, 64));
            storedLocs.put("DecompositionTable", pos.copy());
        }

        // fourth floor - energy - experience table

        ItemStack[] experienceTable = new ItemStack[] { new ItemStack(Blocks.diamond_ore),
                new ItemStack(Blocks.gold_block, 3), new ItemStack(Items.redstone, 64), new ItemStack(Items.dye, 1, 4),
        ...
        ItemStack[] accumulationTable = new ItemStack[] { new ItemStack(ItemList.temple_caller),
        ...
        ItemStack[][] extractionTable = new ItemStack[][] {
                new ItemStack[] { new ItemStack(Blocks.end_stone, 4), new ItemStack(BlockList.end_terrain, 2, 0),
                        new ItemStack(BlockList.end_terrain, 2, 1), new ItemStack(BlockList.end_terrain, 2, 2) },
                new ItemStack[] { new ItemStack(ItemList.energy_wand), new ItemStack(ItemList.energy_wand),
                        new ItemStack(ItemList.energy_wand) },
                new ItemStack[] { new ItemStack(BlockList.endium_block, 64) } };
        ...
        for (int a = -1; a <= 1; a++) {
            ...
            getTile(TileEntityExtractionTable.class)
                    .setInventorySlotContents(2, new ItemStack(ItemList.instability_orb, 16));
        ...
        if (MathUtil.distance(player.posX, player.posZ) > 64D)
    ...
    public void testFallingBlocks() {
        Assert.equal(
                storedLocs.get("FallingBlockObsidian").size(),
                2,
                "Unexpected amount of stored locs, expected $2, got $1.");

        for (BlockPosM testPos : storedLocs.get("FallingBlockObsidian")) {
            Assert.instanceOf(
                    testPos.move(0, -2, 0).getBlock(world),
                    BlockObsidianEnd.class,
                    "Unexpected block class, expected $2, got $1.");
        }

        Assert.equal(
                storedLocs.get("FallingBlockDragonEgg").size(),
                2,
                "Unexpected amount of stored locs, expected $2, got $1.");
    ...
    public void testDragonEssence() {
        ...
        Assert.equal(
                storedLocs.get("DragonEssenceAltar").size(),
                2,
                "Unexpected amount of stored locs, expected $2, got $1.");
        ...
        for (BlockPosM testPos : storedLocs.get("DragonEssenceAltar")) {
            ...
            Assert.equal(
                    getEntities(EntityItemAltar.class).get(0).getEntityItem().getItem(),
                    ItemList.enhanced_brewing_stand,
                    "Unexpected altar item, expected $2, got $1.");
            ...
            pos.set(testPos).move(3, 1, 0);
            Assert.equal(
                    getEntities(EntityItemAltar.class).get(0).getEntityItem().getItemDamage(),
                    1,
                    "Unexpected altar item damage, expected $2, got $1.");
        ...
        Assert.state(
                essence1 < essence2,
                "Unexpected Essence levels, expected " + essence1 + " to be lower than " + essence2 + ".");
    }

    @UnitTest(type = MethodType.TEST, runTime = RunTime.INGAME, trigger = testTrigger)
    public void testBrewing() {
        Object[][] data = new Object[][] { new Object[] { 0, new ItemStack(Items.potionitem, 1, 16) },
                new Object[] { 0, new ItemStack(Items.potionitem, 1, 8194) },
                new Object[] { 1,
                        PotionTypes.setCustomPotionEffect(
                                new ItemStack(Items.potionitem, 1, 8194),
                                new PotionEffect(Potion.moveSpeed.id, 1200, 2)) },
                new Object[] { 1,
                        PotionTypes.setCustomPotionEffect(
                                new ItemStack(Items.potionitem, 1, 8194),
                ...
                new Object[] { 1,
                        PotionTypes.setCustomPotionEffect(
                                new ItemStack(Items.potionitem, 1, 8194),
                                new PotionEffect(Potion.moveSpeed.id, 14400, 0)) },
                new Object[] { 0,
                        PotionTypes.setCustomPotionEffect(
                                new ItemStack(Items.potionitem, 1, 8194 | 16384),
                                new PotionEffect(Potion.moveSpeed.id, 1200, 0)) },
        ...
        for (int a = 0; a < data.length; a++) {
            ...
            if ((int) data[a][0] == 0) Assert.isNull(
                    getTile(TileEntityEnhancedBrewingStand.class).getStackInSlot(4),
                    "Unexpected item, expected powder slot to be empty.");
            else Assert.equal(
                    getTile(TileEntityEnhancedBrewingStand.class).getStackInSlot(4).stackSize,
                    data[a][0],
                    "Unexpected powder amount, expected $2, got $1.");

            Assert.equal(
                    getTile(TileEntityEnhancedBrewingStand.class).getStackInSlot(1),
                    data[a][1],
                    "Unexpected potion item, expected $2, got $1.");
        }
    }
    ...
    public void testDecompositionTable() {
        ...
        for (int a = 0; a < stardust.length; a++) {
            ...
            Assert.equal(
                    getTile(TileEntityDecompositionTable.class).getStackInSlot(1).stackSize,
                    stardust[a],
                    "Unexpected stardust amount, expected $2, got $1.");
        }
        ...
        Assert.state(
                get.apply(0).getStackInSlot(0).stackSize > 3,
                "Unexpected state, expected stack size to be greater than 4.");
        ...
        Assert.equal(
                get.apply(2).getStackInSlot(0).getItem(),
                Item.getItemFromBlock(Blocks.log),
                "Unexpected item, expected $2, got $1.");
