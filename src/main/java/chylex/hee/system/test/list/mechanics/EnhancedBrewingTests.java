...
public class EnhancedBrewingTests {
    ...
    public void prepPotions() {
        potionList = new Potion[] { Potion.heal, Potion.harm, Potion.moveSpeed, Potion.moveSlowdown, Potion.damageBoost,
                Potion.weakness, Potion.nightVision, Potion.invisibility, Potion.regeneration, Potion.poison,
                Potion.fireResistance, Potion.waterBreathing };

        specialIngredients = new Item[] { ItemList.instability_orb, ItemList.ectoplasm, ItemList.silverfish_blood };
        ...
        isCustomSpeed = PotionTypes
                .setCustomPotionEffect(isVanillaSpeed.copy(), new PotionEffect(Potion.moveSpeed.id, 1200, 0)); // 1:00
        isCustomSpeedT2 = PotionTypes
                .setCustomPotionEffect(isVanillaSpeed.copy(), new PotionEffect(Potion.moveSpeed.id, 1200, 1)); // 1:00
        isCustomSpeedL2 = PotionTypes
                .setCustomPotionEffect(isVanillaSpeed.copy(), new PotionEffect(Potion.moveSpeed.id, 2700, 0)); // 2:15
        isCustomSpeedL3 = PotionTypes
                .setCustomPotionEffect(isVanillaSpeed.copy(), new PotionEffect(Potion.moveSpeed.id, 4200, 0)); // 3:30
    }
    ...
    public void testValidation() {
        for (Item item : specialIngredients) {
            Assert.state(
                    PotionTypes.isSpecialIngredient(item),
                    "Unexpected state, invalid special ingredient: " + item.getUnlocalizedName());
        }

        Assert.state(
                PotionTypes.isPotionItem(Items.glass_bottle),
                "Unexpected state, invalid potion item: " + Items.glass_bottle.getUnlocalizedName());
        Assert.state(
                PotionTypes.isPotionItem(Items.potionitem),
                "Unexpected state, invalid potion item: " + Items.potionitem.getUnlocalizedName());
    }

    @UnitTest(type = MethodType.TEST, runTime = RunTime.LOADCOMPLETE)
    public void testPotionData() {
        Assert.equal(
                PotionTypes.getPotionData(isAwkward).getPotionType(),
                null,
                "Unexpected potion data, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getPotionData(isVanillaHeal).getPotionType(),
                Potion.heal,
                "Unexpected potion data, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getPotionData(isCustomHeal).getPotionType(),
                Potion.heal,
                "Unexpected potion data, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getPotionData(isVanillaSpeed).getPotionType(),
                Potion.moveSpeed,
                "Unexpected potion data, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getPotionData(isCustomSpeed).getPotionType(),
                Potion.moveSpeed,
                "Unexpected potion data, expected $2, got $1.");
    }
    ...
    public void testPotionEffects() {
        ...
        Assert.equal(
                PotionTypes.getEffectIfValid(isVanillaHeal),
                new PotionEffect(Potion.heal.id, 1, 0),
                "Unexpected potion effect, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getEffectIfValid(isCustomHeal),
                new PotionEffect(Potion.heal.id, 1, 0),
                "Unexpected potion effect, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getEffectIfValid(isCustomSpeed),
                new PotionEffect(Potion.moveSpeed.id, 1200, 0),
                "Unexpected potion effect, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getEffectIfValid(isCustomSpeedT2),
                new PotionEffect(Potion.moveSpeed.id, 1200, 1),
                "Unexpected potion effect, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getEffectIfValid(isCustomSpeedL2),
                new PotionEffect(Potion.moveSpeed.id, 2700, 0),
                "Unexpected potion effect, expected $2, got $1.");
    }
    ...
    public void testRequiredPowder() {
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.nether_wart, isWater),
                0,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(ItemList.instability_orb, isAwkward),
                8,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.glowstone_dust, isCustomHeal),
                2,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.glowstone_dust, isCustomSpeed),
                2,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.glowstone_dust, isCustomSpeedT2),
                4,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.redstone, isCustomSpeed),
                1,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.redstone, isCustomSpeedL2),
                1,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.redstone, isCustomSpeedL3),
                2,
                "Unexpected powder requirement, expected $2, got $1.");
        Assert.equal(
                PotionTypes.getRequiredPowder(Items.gunpowder, isCustomSpeed),
                3,
                "Unexpected powder requirement, expected $2, got $1.");
    }
    ...
    public void testBrewingSpeedLongOverFail() {
        ItemStack startIS = PotionTypes
                .setCustomPotionEffect(isCustomSpeed.copy(), new PotionEffect(Potion.moveSpeed.id, 11700, 0));
        stages.clear();
        stages.add(Pair.of(Items.redstone, new PotionEffect(Potion.moveSpeed.id, 13200, 0)));
        stages.add(Pair.of(Items.redstone, new PotionEffect(Potion.moveSpeed.id, 14400, 0))); // limit
        ...
        for (Pair<Item, PotionEffect> stage : stages) {
            ...
            if (!PotionTypes.canBeApplied(ingredient, is, enhanced)) {
                if (shouldFail) return;
                        ...
                        "Unexpected brewing state, cannot apply ingredient " + stage.getLeft().getUnlocalizedName()
                                + " to "
                                + is
                                + ".");
            }
            ...
            Assert.equal(
                    PotionTypes.getEffectIfValid(is),
                    stage.getRight(),
                    "Unexpected brewing state effect, expected $2, got $1.");
        }
