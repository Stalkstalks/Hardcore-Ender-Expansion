...
public final class Stopwatch {
    ...
    public static void timeThreshold(String identifier, long threshold) {
        ...
        if (timer == null || !(timer instanceof ThresholdTimer))
            runningStopwatches.put(identifier, timer = new ThresholdTimer());
    ...
    public static void timeAverage(String identifier, int count) {
        ...
        if (timer == null || !(timer instanceof AveragingTimer))
            runningStopwatches.put(identifier, timer = new AveragingTimer());
    ...
    private static class ThresholdTimer extends BasicTimer {
        ...
        public void onFinish(String identifier) {
            double time = (System.nanoTime() - startTime) * toMillis;
            if (time >= threshold) Log.debug(
                    "Stopwatch $0 finished above threshold in $2 ~ $1 ms.",
                    identifier,
                    Math.round(time),
    ...
    private static class AveragingTimer extends StopwatchHandler {
        ...
        public void onFinish(String identifier) {
            ...
            if (--currentCounter == 0 && count > 0) {
                ...
                Log.debug(
                        "Stopwatch $0 finished in averagely $2 ~ $1 ms.",
                        identifier,
                        Math.round(time),
                        numberFormat.format(time));
