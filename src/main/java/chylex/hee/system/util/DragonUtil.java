...
public final class DragonUtil {
    ...
    private static final Pattern regexChatFormatting = Pattern
            .compile("(?i)" + String.valueOf('\u00a7') + "[0-9A-FK-OR]");

    private static final double[] rayX = new double[] { -1D, -1D, 1D, 1D }, rayZ = new double[] { -1D, 1D, -1D, 1D };
    ...
    public static boolean canEntitySeePoint(EntityLivingBase entity, double x, double y, double z, double pointScale,
        ...
        double px = entity.posX, py = entity.posY, pz = entity.posZ, ry = -entity.rotationYaw,
        ...
        for (int a = 0; a < 4; a++) {
            for (int b = 0; b < 2; b++) {
                MovingObjectPosition mop = entity.worldObj.rayTraceBlocks(
                        Vec3.createVectorHelper(px, py + 0.12F, pz),
                        Vec3.createVectorHelper(
                                x + rayX[a] * pointScale,
                                y + (b == 0 ? -1D : 1D) * pointScale,
                                z + rayZ[a] * pointScale));

                if (mop != null && mop.typeOfHit == MovingObjectType.BLOCK) {
                    isBlockedArr[a * 2 + b] = MathUtil
                            .distance(mop.blockX + 0.5D - x, mop.blockY + 0.5D - y, mop.blockZ + 0.5D - z) > 0.1D
                            && MathUtil.distance(mop.blockX + 0.5D - px, mop.blockY + 0.62D - py, mop.blockZ - pz) > 2D;

                    if (isBlockedArr[a * 2 + b]) {
                        if (!entity.worldObj.getBlock(mop.blockX, mop.blockY, mop.blockZ).isOpaqueCube())
        ...
        boolean isBlocked = isBlockedArr[0] && isBlockedArr[1]
                && isBlockedArr[2]
                && isBlockedArr[3]
                && isBlockedArr[4]
                && isBlockedArr[5]
    ...
    public static float rotateSmoothly(float sourceAngle, float targetAngle, float amount) {
        ...
        sourceAngle += amount * ((targetAngle > d && targetAngle < sourceAngle)
                || (sourceAngle < 180F && (targetAngle > d || targetAngle < sourceAngle)) ? -1 : 1);

        return sourceAngle;
    }

    /**
     * Returns the closest entity to the source. It never returns the source entity, even if it is inside the provided
     * list.
     */
    public static <T extends Entity> T getClosestEntity(Entity source, List<? extends T> list) {
        ...
        for (T entity : list) {
            if (!entity.isDead && entity != source
                    && (currentDist = source.getDistanceSqToEntity(entity)) < closestDist) {
    ...
    /**
     * Returns the sorted list of entities based by distance to the source. It never returns the source entity, even if
     * it is inside the provided list.
     */
    ...
    public static Vec3 getRandomVector(Random rand) {
        return Vec3.createVectorHelper(rand.nextDouble() - 0.5D, rand.nextDouble() - 0.5D, rand.nextDouble() - 0.5D)
                .normalize();
    }
    ...
    public static void spawnXP(Entity entity, int amount) {
        for (int a = amount; a > 0;) {
            ...
            a -= split;
            entity.worldObj
                    .spawnEntityInWorld(new EntityXPOrb(entity.worldObj, entity.posX, entity.posY, entity.posZ, a));
        }
    }
    ...
    public static void teleportToOverworld(EntityPlayerMP player) {
        ...
        player.playerNetServerHandler.playerEntity = player.mcServer.getConfigurationManager()
                .respawnPlayer(player, 0, true);
    }

    public static void createMobExplosion(Entity entity, double x, double y, double z, float strength, boolean fire) {
        entity.worldObj.newExplosion(
                entity,
                x,
                y,
                z,
                strength,
                fire,
                entity.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing"));
    }
    ...
    public static int getDayDifference(Calendar cal1, Calendar cal2) {
        return MathUtil
                .floor((double) Math.abs(cal1.getTimeInMillis() - cal2.getTimeInMillis()) / (1000 * 60 * 60 * 24));
    }
