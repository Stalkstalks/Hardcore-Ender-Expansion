...
public class HeeAdminCommand extends BaseCommand {
    ...
    public HeeAdminCommand() {
        ...
        sub.add(new SubCommand("version", 0, false) {
            ...
            void run(ICommandSender sender, String[] args) {
                ...
                sendMessage(
                        sender,
                        LIGHT_PURPLE + "Version: "
                                + RESET
                                + HardcoreEnderExpansion.modVersion
                                + "/"
                                + HardcoreEnderExpansion.buildId);
                sendMessage(
                        sender,
                        LIGHT_PURPLE + "Mod file: "
                                + RESET
                                + (Log.isDeobfEnvironment ? "<deobf>" : HardcoreEnderExpansion.sourceFile.getName()));

                if (UpdateNotificationManager.enableNotifications || UpdateNotificationManager.enableBuildCheck) {
                    sendMessage(
                            sender,
                            LIGHT_PURPLE + "Available for: " + RESET + UpdateNotificationManager.mcVersions);
                    sendMessage(
                            sender,
                            LIGHT_PURPLE + "Release date: " + RESET + UpdateNotificationManager.releaseDate);
            ...
            void run(ICommandSender sender, String[] args) {
                ...
                switch (args[0]) {
                    case "endercrystal":
                        ...
                        break;
                    case "homelandcache":
                        e = new EntityBlockHomelandCache(player.worldObj);
                        break;
                }
        ...
        sub.add(new SubCommand("causatum-check", 0, true) {
            ...
            void run(ICommandSender sender, String[] args) {
                ...
                for (CausatumMeters meter : CausatumMeters.values())
                    sendMessage(sender, LIGHT_PURPLE + "[" + meter.name() + "] " + RESET + file.getLevel(id, meter));
            }
        });
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        int helpPage = args.length == 0 ? 0
                : args[0].equalsIgnoreCase("help") ? DragonUtil.tryParse(args.length == 2 ? args[1] : "1", 1) - 1 : -1;
            ...
            for (SubCommand cmd : sub) {
                if (cmd.name.equalsIgnoreCase(args[0])) {
                    if (args.length <= cmd.argCount) sendMessage(sender, pre + "Invalid amount of parameters.");
                        ...
                        sendMessage(sender, pre + "You need to be in-game to invoke this command.");
