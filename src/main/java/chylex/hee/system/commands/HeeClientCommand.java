...
public class HeeClientCommand extends BaseCommand {
    ...
    public void processCommand(ICommandSender sender, String[] args) {
            ...
            if (prop != null) {
                ...
                for (IConfigElement element : ConfigHandler.getGuiConfigElements()) {
                    if (element.getName().equals(prop) && element.isProperty()) {
                        ...
                        sendMessage(
                                sender,
                                EnumChatFormatting.LIGHT_PURPLE + " [HEE] "
        ...
        sendMessage(
                sender,
                " /hee global <enable|disable>  " + getOnOff(UpdateNotificationManager.enableNotifications));
        sendMessage(sender, EnumChatFormatting.GRAY + "  Setting for all update notifications.");
        sendMessage(
                sender,
                " /hee oncePerUpdate <enable|disable>  "
                        + getOnOff(UpdateNotificationManager.enableOneReportPerUpdate));
        ...
        sendMessage(
                sender,
                " /hee brokenBuild <enable|disable>  " + getOnOff(UpdateNotificationManager.enableBuildCheck));
    ...
    private String getOnOff(boolean state) {
        return "[" + (state ? EnumChatFormatting.GREEN + "ON" : EnumChatFormatting.RED + "OFF")
                + EnumChatFormatting.RESET
                + "]";
    }
}
