...
public class ModelEndermage extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAngle, float entityTickTime, float rotationYaw,
        ...
        leftArm.rotateAngleX = (MathHelper.cos(limbSwing * 0.6662F) * 2F * limbSwingAngle * 0.5F
                - MathHelper.sin(entityTickTime * 0.067F) * 0.05F) * 0.5F;
        rightArm.rotateAngleX = (MathHelper.cos(limbSwing * 0.6662F + (float) Math.PI) * limbSwingAngle
                + MathHelper.sin(entityTickTime * 0.067F) * 0.05F) * 0.5F;
