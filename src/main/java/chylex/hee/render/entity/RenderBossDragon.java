...
public class RenderBossDragon extends RenderLiving {
    ...
    private static final ResourceLocation texCrystalBeam = new ResourceLocation(
            "textures/entity/endercrystal/endercrystal_beam.png");
    private static final ResourceLocation texDeathExplosions = new ResourceLocation(
    ...
    protected void rotateDragonBody(EntityBossDragon dragon, float entityTickTime, float yawOffset,
            float partialTickTime) {
        GL11.glRotatef(-(float) dragon.getMovementOffsets(7, partialTickTime)[0], 0F, 1F, 0F);
        GL11.glRotatef(
                10F * ((float) (dragon.getMovementOffsets(5, partialTickTime)[1]
                        - dragon.getMovementOffsets(10, partialTickTime)[1])),
                1F,
                0F,
                0F);
        ...
        if (dragon.deathTime > 0) {
            GL11.glRotatef(
                    Math.min(1F, MathHelper.sqrt_float((dragon.deathTime + partialTickTime - 1F) / 20F * 1.6F))
                            * getDeathMaxRotation(dragon),
                    0F,
                    0F,
                    1F);
        }
    ...
    protected void renderDragonModel(EntityBossDragon dragon, float limbSwing, float limbSwingAngle,
            float entityTickTime, float rotationYaw, float rotationPitch, float unitPixel) {
    ...
    public void renderDragon(EntityBossDragon dragon, double x, double y, double z, float yaw, float partialTickTime) {
        ...
        BossStatus.bossName = (dragon.isAngry() ? EnumChatFormatting.LIGHT_PURPLE : "")
                + I18n.format(dragon.getCommandSenderName());
        ...
        if (dragon.healingEnderCrystal != null) {
            ...
            float diffX = (float) (dragon.healingEnderCrystal.posX - dragon.posX
                    - (dragon.prevPosX - dragon.posX) * (1F - partialTickTime));
            float diffY = (float) (yCorrection + dragon.healingEnderCrystal.posY
                    - 1D
                    - dragon.posY
                    - (dragon.prevPosY - dragon.posY) * (1F - partialTickTime));
            float diffZ = (float) (dragon.healingEnderCrystal.posZ - dragon.posZ
                    - (dragon.prevPosZ - dragon.posZ) * (1F - partialTickTime));
            ...
            float textureV = MathHelper.sqrt_float(diffX * diffX + diffY * diffY + diffZ * diffZ) * 0.03125F
                    - (dragon.ticksExisted + partialTickTime) * 0.01F;
    ...
    @Override
    protected void renderModel(EntityLivingBase entity, float limbSwing, float limbSwingAngle, float entityTickTime,
            float rotationYaw, float rotationPitch, float unitPixel) {
        renderDragonModel(
                (EntityBossDragon) entity,
                limbSwing,
                limbSwingAngle,
                entityTickTime,
                rotationYaw,
